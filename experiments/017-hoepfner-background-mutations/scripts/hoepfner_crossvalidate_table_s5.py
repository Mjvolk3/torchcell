...
CLUSTER_MUTATION = {
    "CL1": "Chromosome XI aneuploidy",
    "CL2": "Chromosome XI aneuploidy",
    "CL3": "WHI2 nonsense mutation",
    "CL4": "Chromosome V locus amplification",
}
...
def main() -> None:
    base_hip = json.load(open(osp.join(RESULTS_DIR, "summary_stats.json")))["HIP"][
        "baseline_neg_rate"
    ]["neg_rate_4"]
    ...
    print(
        f"Table_S5 flagged strains: {len(aff)} total | {len(positional)} positional "
        f"(paper states 157)"
    )
    ...
    flagged_orfs = set(
        pd.read_csv(osp.join(RESULTS_DIR, "flagged_hip_strains.csv"))["orf"].str.upper()
    )
    ...
    print("=== AUTHORITATIVE Tier A ===")
    print(
        f"157 positional  -> {n_pos} in LMDB, {rec_pos:,} rec "
        f"({rec_pos / TOTAL * 100:.2f}% all / {rec_pos / HIP_TOTAL * 100:.2f}% HIP)"
    )
    print(
        f"188 incl. corr. -> {n_all} in LMDB, {rec_all:,} rec "
    ...
    print(
        f"TP={len(tp)} FP={len(fp)} FN={len(fn)}  precision={prec:.2f} recall={rec:.2f}"
    )
    print(
        f"missed strains' HIP-enrichment percentile (median): {miss_pct:.2f} "
        f"(many affected strains individually look clean; Table_S5 flags by position+lab)"
    )

    out = (
        hip[hip["orf"].isin(all_orfs)]
        .merge(
            aff[
                [
                    "ORF",
    ...
    ].sort_values(["base_cluster", "chrom", "pos"])
    ...
    json.dump(
        dict(
            table_s5=dict(
                ...
                n_total_flagged=len(aff),
                cluster_mutation=CLUSTER_MUTATION,
                lab_counts={k: int(v) for k, v in aff["Lab"].value_counts().items()},
            ),
            tier_a_positional_157=dict(
                n_in_lmdb=n_pos,
                n_records=rec_pos,
                pct_all=rec_pos / TOTAL * 100,
                pct_hip=rec_pos / HIP_TOTAL * 100,
            ),
            tier_a_total_188=dict(
                n_in_lmdb=n_all,
                n_records=rec_all,
                pct_all=rec_all / TOTAL * 100,
                pct_hip=rec_all / HIP_TOTAL * 100,
            ),
            n_absent_from_lmdb=len(absent),
            absent=sorted(absent),
            crossvalidation=dict(
                n_flagged=len(flagged_orfs),
                TP=len(tp),
                FP=len(fp),
                FN=len(fn),
                precision=prec,
        ...
        open(osp.join(RESULTS_DIR, "table_s5_crossvalidation.json"), "w"),
        indent=2,
    )
    print(
