...
"""
Visualization for log-OLS epistatic interaction analysis with WT-differencing.
...
def load_results(results_file: Path = RESULTS_DIR / 'log_ols_results.pkl') -> Dict:
    """Load saved log-OLS results."""
...
def plot_digenic_interactions(results: Dict):
    ...
    for trait in results:
        ...
        ax.set_title(f'Digenic Interactions - {FFA_LABELS.get(trait, trait)} (Log-OLS)', fontsize=14)
        ax.set_xticks(x)
        ...
        plt.tight_layout()
        filename = f'log_ols_digenic_{trait}.png'
...
def plot_trigenic_interactions(results: Dict):
    ...
    for trait in results:
        ...
        ax.set_ylabel('φ (epistatic fold)', fontsize=12)
        ax.set_title(f'Trigenic Interactions - {FFA_LABELS.get(trait, trait)} (Log-OLS)', fontsize=14)
        ax.set_xticks(x)
        ...
        plt.tight_layout()
        filename = f'log_ols_trigenic_{trait}.png'
        ffa_dir = osp.join(ASSET_IMAGES_DIR, "008-xue-ffa")
        os.makedirs(ffa_dir, exist_ok=True)
        save_path = osp.join(ffa_dir, filename)
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
...
def plot_summary_statistics(results: Dict):
    ...
    ax1.set_title('Log-OLS: Significant Interactions by FFA', fontsize=11)
    ...
    ax2.set_title('Log-OLS: Total Interaction Counts', fontsize=11)
    ax2.grid(True, alpha=0.3, axis='y')

    # Plot 3: R-squared by FFA
    ax3 = axes[1, 0]
    r_squared_values = []
    r_squared_labels = []

    for trait in ['C140', 'C160', 'C180', 'C161', 'C181', 'Total Titer']:
        if trait in results and 'r_squared' in results[trait]:
            r_squared_labels.append(FFA_LABELS.get(trait, trait))
            r_squared_values.append(results[trait]['r_squared'])

    if r_squared_values:
        x_pos = np.arange(len(r_squared_labels))
        bars = ax3.bar(x_pos, r_squared_values, color='#34699D', alpha=0.7)
        ...
        ax3.set_xticklabels(r_squared_labels, rotation=45, ha='right')
        ax3.set_ylabel('R²', fontsize=10)
        ax3.set_title('Log-OLS: Model Fit by FFA', fontsize=11)
        ax3.grid(True, alpha=0.3, axis='y')
        ax3.set_ylim(0, max(r_squared_values) * 1.2)
    ...
    if all_pvalues:
        ...
        ax4.set_title('Log-OLS: P-value Distribution', fontsize=11)
        ax4.legend()
        ax4.grid(True, alpha=0.3)

    plt.suptitle('Log-OLS Epistatic Interactions - Summary Statistics', fontsize=14)
    plt.tight_layout()

    filename = 'log_ols_summary.png'
    ffa_dir = osp.join(ASSET_IMAGES_DIR, "008-xue-ffa")
    os.makedirs(ffa_dir, exist_ok=True)
    save_path = osp.join(ffa_dir, filename)
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
...
def main():
    ...
    print("Loading log-OLS results...")
    results_file = RESULTS_DIR / 'log_ols_results.pkl'

    if not results_file.exists():
        print(f"Results file not found at {results_file}")
        print("Please run log_ols_wt_differencing_epistatic_interactions.py first")
        return
