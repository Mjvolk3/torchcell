...
interaction is a structural property of the hypothesis class: for distinct genes $g$,
$h$, $k$ and any $S$ disjoint from them,
%
\begin{equation}
...
differences vanish identically. The direct analog of the additive null of Visani,
Verma and DeWitt (bioRxiv \texttt{2026.04.23.719915}).
...
$\alpha/(n_{gh} + \alpha)$, so B2 is B1 plus one nearly unshrunk offset per screen.
Second-order capacity on observed pairs, no third-order capacity.
...
coefficient, so B4 is structurally zero.
...
The cell graph transformer of experiment 010, run on the 025 build without change in
...
nine interaction graphs reach the prediction only through the trained weights, via a
penalty on layer-1 attention. The long reading of that code is in the 010 document
and is not repeated.
...
Every model is fit on the training part of the arm it is reported under, selects
its free choices on the validation part, and is reported on test. The transformer's
