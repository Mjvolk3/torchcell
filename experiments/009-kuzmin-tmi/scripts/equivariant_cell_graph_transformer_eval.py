...
def main(cfg: DictConfig) -> None:
    ...
    # Derive training_group from checkpoint_path (single source of truth)
    # checkpoint_path format: models/checkpoints/<training_group>/<run_id>/...
    checkpoint_path = wandb_cfg["model"]["checkpoint_path"]
    checkpoint_parts = checkpoint_path.split("/")
    checkpoints_idx = checkpoint_parts.index("checkpoints")
    training_group = checkpoint_parts[checkpoints_idx + 1]
    # Get full relative path after "checkpoints/" for results directory structure
    checkpoint_rel_path = "/".join(checkpoint_parts[checkpoints_idx + 1 :])
    # Remove .ckpt extension for directory name
    if checkpoint_rel_path.endswith(".ckpt"):
        checkpoint_rel_path = checkpoint_rel_path[:-5]

    print(f"Derived training_group: {training_group}")
    ...
    if task.val_samples["predictions"]:
        ...
        # Log all additional metrics in single call (avoids step issues)
        val_metrics_to_log = {f"eval_val/{key}": value for key, value in val_additional_metrics.items()}
        wandb.log(val_metrics_to_log)
        print("Logged eval_val metrics:")
    ...
    if task.test_samples["predictions"]:
        ...
        # Log all additional metrics in single call (avoids step issues)
        test_metrics_to_log = {f"eval_test/{key}": value for key, value in test_additional_metrics.items()}
        wandb.log(test_metrics_to_log)
        print("Logged eval_test metrics:")
