# experiments/008-xue-ffa/scripts/trigenic_interaction_bar_plots_triple_suppression
# [[experiments.008-xue-ffa.scripts.trigenic_interaction_bar_plots_triple_suppression]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/008-xue-ffa/scripts/trigenic_interaction_bar_plots_triple_suppression
# Test file: experiments/008-xue-ffa/scripts/test_trigenic_interaction_bar_plots_triple_suppression.py
...
def create_trigenic_bar_plots(
    ...
    color_eps_ij = color_ij  # Same as f_ij
    color_eps_ik = color_ik  # Same as f_ik
    color_eps_jk = color_jk  # Same as f_jk
    color_tau = color_ijk    # Same as f_ijk
    ...
    for ffa_idx, ffa in enumerate(ffa_types):
        ...
        for (gene1, gene2, gene3), tau_values in trigenic_interactions.items():
            if ffa_idx < len(tau_values):
                ...
                eps_ij = digenic_interactions.get((gene1, gene2), [np.nan]*6)[ffa_idx]
                eps_ik = digenic_interactions.get((gene1, gene3), [np.nan]*6)[ffa_idx]
                eps_jk = digenic_interactions.get((gene2, gene3), [np.nan]*6)[ffa_idx]
                ...
                se_eps_ij = digenic_se.get((gene1, gene2), [0]*6)[ffa_idx] if digenic_se else 0
                se_eps_ik = digenic_se.get((gene1, gene3), [0]*6)[ffa_idx] if digenic_se else 0
                se_eps_jk = digenic_se.get((gene2, gene3), [0]*6)[ffa_idx] if digenic_se else 0

                se_tau = trigenic_se.get((gene1, gene2, gene3), [0]*6)[ffa_idx] if trigenic_se else 0

                # Check if we have valid data
                if not np.isnan(tau) and not np.isnan(f_ijk):
                    ...
                    plot_data.append({
                        ...
                        "f_ijk": f_ijk,
                        # Interaction values
                        "eps_ij": eps_ij, "eps_ik": eps_ik, "eps_jk": eps_jk,
                        "tau": tau,
                        # Standard errors
                        "se_i": se_i, "se_j": se_j, "se_k": se_k,
                        "se_ij": se_ij, "se_ik": se_ik, "se_jk": se_jk,
                        "se_ijk": se_ijk,
                        "se_eps_ij": se_eps_ij, "se_eps_ik": se_eps_ik, "se_eps_jk": se_eps_jk,
                        "se_tau": se_tau,
                    })
        ...
        # Sort by absolute tau value and take top N
        plot_data = sorted(plot_data, key=lambda x: abs(x["tau"]), reverse=True)[:top_n]
        # Re-sort by tau value for visualization (high to low, left to right)
        plot_data = sorted(plot_data, key=lambda x: x["tau"], reverse=True)
        ...
        ax1.bar(x + bar_positions[7], [d["eps_ij"] for d in plot_data], width,
                yerr=[d["se_eps_ij"] if not np.isnan(d["se_eps_ij"]) else 0 for d in plot_data],
                label=r"$\varepsilon_{ij}$",
                color=color_eps_ij, alpha=0.7, capsize=3,
                hatch='///', edgecolor='black', linewidth=0.5,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[8], [d["eps_ik"] for d in plot_data], width,
                yerr=[d["se_eps_ik"] if not np.isnan(d["se_eps_ik"]) else 0 for d in plot_data],
                label=r"$\varepsilon_{ik}$",
                color=color_eps_ik, alpha=0.7, capsize=3,
                hatch='///', edgecolor='black', linewidth=0.5,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[9], [d["eps_jk"] for d in plot_data], width,
                yerr=[d["se_eps_jk"] if not np.isnan(d["se_eps_jk"]) else 0 for d in plot_data],
        ...
        ax1.bar(x + bar_positions[10], [d["tau"] for d in plot_data], width,
                yerr=[d["se_tau"] if not np.isnan(d["se_tau"]) else 0 for d in plot_data],
                label=r"$\tau_{ijk}$",
        ...
        ax1.set_title(f"Top {n_triples} Trigenic Interactions for {ffa} (by |�|)",
        ...
        filename = f"trigenic_interaction_bar_plots_triple_suppression_{ffa.replace(':', '')}_{timestamp()}.png"
        ...
        tau_values = [d["tau"] for d in plot_data]
        print(f"\n{ffa} Summary (Top {n_triples} Trigenic TF interactions):")
        print(f"  Mean �: {np.mean(tau_values):.3f}")
        print(f"  Min �: {np.min(tau_values):.3f}")
        print(f"  Max �: {np.max(tau_values):.3f}")
        print(f"  Negative interactions: {sum(t < 0 for t in tau_values)}/{n_triples}")
        print(f"  Positive interactions: {sum(t > 0 for t in tau_values)}/{n_triples}")
...
def create_recovery_pattern_plots(
    ...
    for ffa, patterns in ffa_groups.items():
        ...
        # Save figure with filename matching script name
        filename = f"trigenic_interaction_bar_plots_triple_suppression_recovery_{ffa.replace(':', '')}_{timestamp()}.png"
        filepath = osp.join(ASSET_IMAGES_DIR, filename)
        fig.savefig(filepath, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    filename = f"trigenic_interaction_bar_plots_triple_suppression_recovery_summary_{timestamp()}.png"
    filepath = osp.join(ASSET_IMAGES_DIR, filename)
    fig.savefig(filepath, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_trigenic_summary_plot(trigenic_interactions, triple_mutants, columns,
    ...
    top_interactions = df.nlargest(top_n, 'abs_tau')
    ...
    pivot_data = top_interactions.pivot_table(
        index='triple', columns='ffa', values='tau', fill_value=0
    )

    # Sort by mean absolute tau
    ...
    # Add colorbar
    plt.colorbar(im, ax=ax, label=r'$\tau$ value')

    ax.set_title(f"Top {min(top_n, len(pivot_data))} Trigenic Interactions by |�|",
                fontsize=14, fontweight="bold")
    ax.set_xlabel("FFA Type", fontsize=12)
    ax.set_ylabel("Gene Triples", fontsize=12)

    # Plot 2: Distribution of tau values
    ax = axes[1]

    # Plot histogram for each FFA
    for ffa in ffa_types:
        ffa_data = df[df["ffa"] == ffa]
        ax.hist(ffa_data["tau"], alpha=0.6, bins=30, label=ffa,
               edgecolor="black", linewidth=0.5)

    ax.axvline(x=0, color="red", linestyle="--", alpha=0.7, linewidth=2)
    ax.set_xlabel(r"Trigenic Interaction Score ($\tau$)", fontsize=14)
    ...
    # Save figure with filename matching script name
    filename = f"trigenic_interaction_bar_plots_triple_suppression_summary_{timestamp()}.png"
    filepath = osp.join(ASSET_IMAGES_DIR, filename)
    fig.savefig(filepath, dpi=300, bbox_inches="tight")
    plt.close()
    ...
    print(f"Mean � across all FFAs: {df['tau'].mean():.3f}")
    print(f"Std �: {df['tau'].std():.3f}")
    print(f"Percentage negative: {100 * (df['tau'] < 0).sum() / len(df):.1f}%")
    print(f"Percentage positive: {100 * (df['tau'] > 0).sum() / len(df):.1f}%")
    print(f"\nTop 5 strongest interactions (by |�|):")
    for _, row in df.nlargest(5, 'abs_tau').iterrows():
        print(f"  {row['triple']} ({row['ffa']}): � = {row['tau']:.3f}")
