...
def main():
    ...
    if set(sample_subgraph["gene"].ids_pert) == set(sample_lazy["gene"].ids_pert):
        ...
        print("  ✗ gene.ids_pert: MISMATCH")

    # Check perturbation_indices
    if torch.equal(
        sample_subgraph["gene"].perturbation_indices,
        sample_lazy["gene"].perturbation_indices,
    ):
        print("  ✓ gene.perturbation_indices: identical")
    else:
        print("  ✗ gene.perturbation_indices: MISMATCH")

    # Check pert_mask
    if torch.equal(sample_subgraph["gene"].pert_mask, sample_lazy["gene"].pert_mask):
        print("  ✓ gene.pert_mask: identical")
    else:
        print("  ✗ gene.pert_mask: MISMATCH")
    ...
    print(
        "Attempting to recover SubgraphRepresentation edge_index from LazySubgraphRepresentation..."
    )
    print()
    ...
    kept_node_indices = (
        sample_lazy["gene"].mask.nonzero().squeeze(1)
    )  # Original indices of kept nodes
    ...
    for et in sample_subgraph.edge_types:
        if et[0] == "gene" and et[2] == "gene":
            ...
            edge_index_relabeled = torch.stack(
                [node_map[edge_index_filtered[0]], node_map[edge_index_filtered[1]]]
            )

            print(f"{et}:")
            print(f"  SubgraphRepresentation edges: {edge_index_subgraph.size(1)}")
            print(
                f"  LazySubgraphRepresentation (after mask): {edge_index_filtered.size(1)}"
            )
            ...
            ei_relabeled_sorted = sort_edge_index(
                edge_index_relabeled, sort_by_row=True
            )

            # Compare
            if torch.equal(ei_subgraph_sorted, ei_relabeled_sorted):
                print("  ✓ Edge indices match after relabeling and sorting!")
            else:
                print("  ✗ Edge indices MISMATCH after relabeling")
                all_match = False

                # Show first few edges for debugging
                print(f"    SubgraphRepresentation: {ei_subgraph_sorted[:, :5]}")
                print(
                    f"    LazySubgraphRepresentation (relabeled): {ei_relabeled_sorted[:, :5]}"
                )

                # Find first mismatch
                if ei_subgraph_sorted.shape == ei_relabeled_sorted.shape:
                    diff_mask = ~torch.all(
                        ei_subgraph_sorted == ei_relabeled_sorted, dim=0
                    )
                    if diff_mask.any():
                        first_diff = diff_mask.nonzero()[0].item()
                        print(f"    First difference at edge {first_diff}:")
                        print(
                            f"      SubgraphRepresentation: {ei_subgraph_sorted[:, first_diff]}"
                        )
                        print(
                            f"      LazySubgraphRepresentation: {ei_relabeled_sorted[:, first_diff]}"
                        )
    ...
    for et in sample_subgraph.edge_types:
        if et[0] == "gene" and et[2] == "gene":
            subgraph_memory += (
                sample_subgraph[et].edge_index.element_size()
                * sample_subgraph[et].edge_index.nelement()
            )
    ...
    for et in sample_lazy.edge_types:
        if et[0] == "gene" and et[2] == "gene":
            ...
            lazy_memory += (
                sample_lazy[et].mask.element_size() * sample_lazy[et].mask.nelement()
            )
