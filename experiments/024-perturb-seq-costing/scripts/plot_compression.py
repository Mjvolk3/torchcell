...
def panel_b(ax, spar, summ) -> None:
    ...
    labels = ["one\nperturbation"] + [
        f"SVD module\n{int(float(m) * 100)}% mass" for m, _ in masses
    ]
    ...
    ax.set_title("Sparse per perturbation, dense per component", loc="left", fontsize=6)
    box(ax)
...
def panel_c(ax, add) -> None:
    ...
    ax.hist(
        slopes, bins=np.linspace(0, 1.6, 25), color=C_POOL, edgecolor="black", lw=0.4
    )
    med = float(np.median(slopes))
    ax.axvline(1.0, color=C_REF, lw=0.7, ls="--")
    ax.annotate(
        "additive",
        (1.0, ax.get_ylim()[1] * 0.94),
        xytext=(3, 0),
        textcoords="offset points",
        fontsize=4.5,
        color=C_REF,
...
def panel_d(ax, summ) -> None:
    ...
    cross = n[
        np.argmin(
            np.abs(
                samples("compressed", n, q, r, m) - samples("conventional", n, q, r, m)
            )
        )
    ]
    ...
    ax.set_title("Compression pays only above a library size", loc="left", fontsize=6)
    box(ax)
...
def panel_e(ax, summ) -> None:
    ...
    for n, ls in ((N_GENOME, "-"), (N_PANEL, "--")):
        ax.plot(ms, np.full_like(ms, n, dtype=float) / ms, lw=1.0, ls=ls, color=C_POOL)
        ax.plot(
            ms,
            np.full_like(ms, (q + r) * np.log(n), dtype=float),
            lw=1.0,
            ls=ls,
            color=C_COMP,
        )
...
def panel_f(ax, summ) -> None:
    ...
    rows = [
        ...
        (
            ...
            q_lo,
            q_hi,
            summ["q_median_genes_moved_per_perturbation"],
            C_POOL,
    ...
    for yi, (_, lo, hi, mid, color, _) in zip(y, rows):
        ax.plot(
            [lo, hi], [yi, yi], lw=3.0, color=color, solid_capstyle="butt", zorder=2
        )
        ax.plot(
            [mid],
            [yi],
    ...
    ax.plot(
        [summ["effective_rank_participation_ratio"]],
        [y[-1]],
        marker="|",
        ms=7,
        color="black",
    ...
    for yi, (_, _, _, _, color, basis) in zip(y, rows):
        ax.annotate(
            basis, (2.6, yi - 0.30), fontsize=4.0, color=C_REF, ha="left", va="center"
        )
    box(ax)
...
def main() -> None:
    ...
    assert_legible(
        fig,
        axes=flat,
        exempt={"additive", "panel", "genome", f"all {summ['n_genes']:,} genes"},
    )
