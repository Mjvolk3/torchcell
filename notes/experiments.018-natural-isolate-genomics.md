...
**Related note — division of labour.** This is the **analysis / bit-accounting** note (the
full derivations + provenance, generated by `make_figures.py` + the six analysis scripts). The
**Fig 4 panel spec** is its sibling
[[experiments.018-natural-isolate-genomics.dataset-comparison]] (generated by
`dataset_comparison.py`), which re-cuts four of the topics here — divergence, regulatory π, the
DE comparison, and the genotype↔transcriptome coupling — as Fig 4 panels and points back here
for their formulas. **This note is the single source of truth for those four.**
...
*Each point is one of the 943 natural isolates. A "mutational-load" baseline — more sequence divergence ⇒ more genes moved — would show up as a rising trend; it does not.*

There is little a-priori reason to expect genotype magnitude to set transcriptome magnitude,
and the data confirm it does not — a **modeling-relevant negative result** rather than a
surprise. Across the 865 isolates with both measurements:
