...
def load_and_process_data(max_size):
    df = pd.read_csv(
        os.path.join(RESULTS_DIR, f"random_forest_processed_df_{max_size}.csv")
    )
...
def humanize_metric(metric):
    # Split the metric name and capitalize each word
    words = metric.split("_")
    if len(words) > 1:
        # If it's a compound metric (e.g., 'val_r2'), keep the first part as is
...
def create_plot(data_dict, metric="val_r2"):
    ...
    for i, node_embedding in enumerate(node_embedding_order):
    ...
    # Humanize the y-axis label
    humanized_metric = humanize_metric(metric)
    plt.ylabel(humanized_metric, fontsize=16)

    plt.title(
        f"Best {humanized_metric} for Node Embeddings across Dataset Sizes", fontsize=18
    )
    ...
    # Explicitly set font sizes for x and y tick labels
    ax = plt.gca()
    ...
    plt.savefig(
        os.path.join(ASSET_IMAGES_DIR, f"node_embedding_performance_{metric}.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
