...
def test_masked_vs_filtered_speed(synthetic_data, gin_mlp, device):
    ...
    print(f"\n{'=' * 60}")
    print("Performance Comparison (yeast genome scale)")
    print(f"{'=' * 60}")
...
def test_pyg_concatenated_batch(device):
    ...
    dataset, batch, _, _ = load_sample_data_batch(
        batch_size=2, num_workers=0, use_custom_collate=True
    )
    ...
    mlp = nn.Sequential(nn.Linear(64, 64), nn.ReLU(), nn.Linear(64, 64)).to(device)
