...
ROUND = {
    "v13": {
        ...
        "pheno": "expression",
        "baselines": "expression_baselines_split",
    ...
    "v14": {
        ...
        "pheno": "proteome",
        "baselines": "baselines_split_fig3_proteome",
...
def _load_dump(
    path: str, keys: list[str], gene_set: set[str]
) -> tuple[np.ndarray, np.ndarray, list[int]]:
    """Per-gene predictions and targets of a dump, in the order of ``keys``.

    The dump lists the phenotype's raw keys, but the per-gene head only predicts the
    keys that are nodes of the cell graph (``build_head_alignments`` in
    train_cgt_multitask.py drops the rest with ``keep_mask``), so its columns are the
    raw keys filtered by ``gene_set`` in raw order; ``keys`` must be exactly that set.
    """
    with open(path) as fh:
        d = json.load(fh)
    head = "per_gene"
    dump_keys = [k for k in d["head_keys"][head] if k in gene_set]
    if set(dump_keys) != set(keys):
        raise ValueError(
            f"{path}: dump names {len(dump_keys)} graph genes, the LMDB {len(keys)};"
    ...
    if len(recs[0]["pred"]) != len(dump_keys):
        raise ValueError(
            f"{path}: {len(recs[0]['pred'])} columns for {len(dump_keys)} genes"
        )
...
def _proteome_reliability(data_root: str, keys: list[str]) -> np.ndarray:
    ...
    meta = pd.read_csv(osp.join(data_root, RAW_DIR, "yeast5k_metadata.csv")).set_index(
        "Filename"
    )
    ...
    log_ko = (
        np.log2(ko.to_numpy(dtype=float))
        - np.nanmean(np.log2(wt.to_numpy(dtype=float)), 1)[:, None]
    )
...
def main() -> None:
    ...
    for spec in RUNS:
        ...
        if key not in cache:
            split = _load_split(
                osp.join(base, "data_module_cache"), spec["split_seed"], rd["label"]
            )
            perts_tr, y_tr, keys = _load_records(base, split["train"], rd["label"])
            perts_va, y_va, keys_va = _load_records(base, split["val"], rd["label"])
            perts_te, y_te, keys_te = _load_records(base, split["test"], rd["label"])
            assert keys == keys_va == keys_te
            # Same gene set for the model and the baselines: the per-gene head predicts
            # only the keys that are nodes of the cell graph (42 of the 6,169 expression
            # keys are snoRNAs and ORFs absent from the genome gene set).
            in_graph = np.array([k in gene_set for k in keys])
            keys = [k for k, ok in zip(keys, in_graph) if ok]
            y_tr, y_va, y_te = y_tr[:, in_graph], y_va[:, in_graph], y_te[:, in_graph]
            ...
            p_te, ok_te = _pert_matrix(perts_te, emb, dim)
            ...
            with open(
                osp.join(results_dir, rd["baselines"], f"seed{spec['split_seed']}.json")
            ) as fh:
                bj = json.load(fh)
            b2 = bj["B2_bilinear"]["by_embedding"][EMB]["selected_on_val"]
            b3 = bj["B3_neighbor_average"]["by_embedding"][EMB]["selected_on_val"]
            e_tr = p_tr[ok_tr] / (
                np.linalg.norm(p_tr[ok_tr], axis=1, keepdims=True) + 1e-12
            )
            base_r: dict[str, dict[str, np.ndarray]] = {}
            for sname, y_s, p_s, ok_s in (
                ("val", y_va, p_va, ok_va),
                ("test", y_te, p_te, ok_te),
            ):
                r_hat2 = _bilinear(
                    r_tr[ok_tr],
                    np.nan_to_num(y_s - mu, nan=0.0),
                    p_tr[ok_tr],
                    p_s,
                    int(b2["k_gene"]),
                    float(b2["ridge"]),
                )
                e_s = p_s / (np.linalg.norm(p_s, axis=1, keepdims=True) + 1e-12)
                nn = np.argsort(-(e_s @ e_tr.T), axis=1)[:, : int(b3["k"])]
                with np.errstate(invalid="ignore"):
                    r_hat3 = np.nan_to_num(
                        np.nanmean((y_tr - mu)[ok_tr][nn], axis=1), nan=0.0
                    )
                base_r[sname] = {
                    "b2": per_gene_pearson(r_hat2[ok_s] + mu, y_s[ok_s]),
                    "b3": per_gene_pearson(r_hat3[ok_s] + mu, y_s[ok_s]),
                }
            rel = (
                _proteome_reliability(data_root, keys)
                if spec["round"] == "v14"
                else None
            )
            cache[key] = {
        ...
        entry: dict[str, Any] = {
            **spec,
            ...
            "sd_train_at_percentile": {
                str(p): float(np.percentile(c["sd_tr"], p))
        ...
        for sname in ("val", "test"):
            dump = osp.join(data_root, f"{sname}-predictions", f"{spec['group']}.json")
            pred, tgt, rec_idx = _load_dump(dump, c["keys"], gene_set)
            # The dump's targets must be the LMDB's rows for that split, in the split's order.
            if rec_idx != list(c["split"][sname]):
                raise ValueError(
                    f"{dump}: record order differs from the split's {sname} indices"
                )
            if not np.allclose(
                np.nan_to_num(tgt), np.nan_to_num(c["y"][sname]), atol=1e-3
            ):
                raise ValueError(f"{dump}: targets differ from the LMDB {sname} rows")
            r_model = per_gene_pearson(pred, tgt)
            entry[sname] = {
                "n_records": int(len(rec_idx)),
                "all_genes_pearson": float(np.nanmean(r_model)),
                "model": _strata_means(r_model, c["pct"]),
                "B2": _strata_means(c["base_r"][sname]["b2"], c["pct"]),
                "B3": _strata_means(c["base_r"][sname]["b3"], c["pct"]),
            }
        out["entries"].append(entry)
        print(
            f"\n{spec['round']} {spec['arm']} seed {spec['seed']} ({spec['run']}): "
            f"all-gene Pearson val {entry['val']['all_genes_pearson']:.4f}"
            f" test {entry['test']['all_genes_pearson']:.4f}"
        )
        print(
            f"  {'stratum':10s} {'val model':>10s} {'B2':>8s} {'B3':>8s} |{'test model':>11s} {'B2':>8s} {'B3':>8s}"
            + ("  ceiling" if "ceiling" in entry else "")
        )
        for name, *_ in STRATA + [(t[0],) for t in TOPS]:
            v, t = entry["val"], entry["test"]
            line = (
                f"  {name:10s} {v['model'][name]:10.4f} {v['B2'][name]:8.4f} {v['B3'][name]:8.4f} |"
                f"{t['model'][name]:11.4f} {t['B2'][name]:8.4f} {t['B3'][name]:8.4f}"
            )
            if "ceiling" in entry:
    ...
    # The imported baseline modules leave their own label and tick sizes in rcParams, so
    # every size is pinned here rather than inherited from font.size.
    plt.rcParams.update(
        {
            "font.family": "Arial",
            "font.size": 6,
    ...
    for ax, rnd in zip(axes, ("v13", "v14")):
        ...
        for i, e in enumerate(ents):
            ax.plot(
                x,
                [e["val"]["model"][s[0]] for s in STRATA],
                marker="o",
                markersize=2.5,
                linewidth=0.8,
                color=PLOT_PALETTE[i],
                label=f"{e['arm']} seed {e['seed']}",
            )
        e0 = ents[0]["val"]
        ax.plot(
            x,
            [e0["B2"][s[0]] for s in STRATA],
            marker="s",
            markersize=2.5,
            linewidth=0.8,
            linestyle="--",
            color=PLOT_PALETTE[4],
            label="B2 ridge ProtT5 (split 0)",
        )
        ax.plot(
            x,
            [e0["B3"][s[0]] for s in STRATA],
            marker="^",
            markersize=2.5,
            linewidth=0.8,
            linestyle="--",
            color=PLOT_PALETTE[5],
            label="B3 kNN ProtT5 (split 0)",
        )
        if "ceiling" in ents[0]:
            ax.plot(
                x,
                [ents[0]["ceiling"][s[0]] for s in STRATA],
                marker="D",
                markersize=2.5,
                linewidth=0.8,
                linestyle=":",
                color="black",
                label="duplicate-strain ceiling",
            )
        ax.set_xticks(x)
