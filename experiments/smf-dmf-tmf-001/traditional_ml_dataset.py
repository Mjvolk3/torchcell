...
def save_data_from_dataloader(dataloader, save_path, is_pert, aggregation):
    ...
    for batch in dataloader:
        ...
        x_unbatched = unbatch(x, batch["gene"].batch)
        ...
        print(f"x_agg shape: {x_agg_np.shape}, y shape: {y_np.shape}")

        # Check and handle mismatch
        if x_agg_np.shape[0] != y_np.shape[0]:
            print("Mismatch in shapes: continuing to next batch")
            continue
    ...
    # Save the arrays
    np.save(osp.join(save_path, "features.npy"), all_features)
    np.save(osp.join(save_path, "labels.npy"), all_labels)
...
def main(cfg: DictConfig) -> None:
    ...
    base_path = osp.join(DATA_ROOT, "data/torchcell/experiments/smf-dmf-tmf_001")
    node_embeddings_path = osp.join(
        base_path,
        "_".join(wandb.config.cell_dataset["node_embeddings"]),
    ...
    save_data_from_dataloader(
        ...
        osp.join(node_embeddings_path, "train"),
        is_pert=wandb.config.cell_dataset["is_pert"],
        aggregation=wandb.config.cell_dataset["aggregation"],
    )
    save_data_from_dataloader(
        data_module.val_dataloader(),
        osp.join(node_embeddings_path, "val"),
        is_pert=wandb.config.cell_dataset["is_pert"],
        aggregation=wandb.config.cell_dataset["aggregation"],
    )
    save_data_from_dataloader(
        data_module.test_dataloader(),
        osp.join(node_embeddings_path, "test"),
        is_pert=wandb.config.cell_dataset["is_pert"],
        aggregation=wandb.config.cell_dataset["aggregation"],
    )
