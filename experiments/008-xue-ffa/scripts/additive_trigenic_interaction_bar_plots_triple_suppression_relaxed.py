# experiments/008-xue-ffa/scripts/additive_trigenic_interaction_bar_plots_triple_suppression_relaxed
# [[experiments.008-xue-ffa.scripts.additive_trigenic_interaction_bar_plots_triple_suppression_relaxed]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/008-xue-ffa/scripts/additive_trigenic_interaction_bar_plots_triple_suppression_relaxed
# Test file: experiments/008-xue-ffa/scripts/test_additive_trigenic_interaction_bar_plots_triple_suppression_relaxed.py
...
def create_recovery_pattern_plots(
    ...
    for ffa, patterns in ffa_groups.items():
        ...
        # Save figure
        filename = f"additive_trigenic_suppression_relaxed_{ffa.replace(':', '')}_{timestamp()}.png"
    ...
    plt.suptitle('Relaxed Trigenic Recovery: Triple Mutant > All Singles & Doubles - ADDITIVE MODEL',
                fontsize=16, fontweight='bold')
    plt.tight_layout()

    # Save summary figure
    filename = f"additive_trigenic_suppression_summary_relaxed_{timestamp()}.png"
    filepath = osp.join(ASSET_IMAGES_DIR, filename)
    fig.savefig(filepath, dpi=300, bbox_inches="tight")
    plt.close()
