...
def create_plot(data_dict, metric="val_r2"):
    ...
    plt.title(
        f"smf-dmf-tmf-001 Best {humanized_metric} for Node Embeddings across Dataset Sizes", fontsize=18
    )
    ...
    plt.savefig(
        os.path.join(ASSET_IMAGES_DIR, f"smf-dmf-tmf-001_node_embedding_performance_{metric}.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
