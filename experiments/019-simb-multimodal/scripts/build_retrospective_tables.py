...
def strand_table(board: pd.DataFrame, ceilings: dict) -> str:
    ...
    cov = _run_coverage(board)
    n_fetched = int(cov["fetched"].sum())
    lines = [
    ...
    lines += [
        ...
        # Two sentences, because a complete pull and a partial one are different claims and
        # the caption must not read as a caveat once the caveat no longer applies.
        + (
            f"\\textbf{{Coverage: complete. All {n_projects} projects and all {n_runs} runs "
            f"have had the validation curve needed to score them read}} "
            r"(\cref{tab:coverage}), so every \emph{best} here is a maximum over the runs "
            r"that were actually run. "
            if n_fetched >= n_runs
            else f"\\textbf{{Coverage: all {n_projects} projects are read and all {n_runs} "
            f"runs are enumerated, but the validation curve needed to score a run was read "
            f"for only {n_fetched} of them}} "
            r"(\cref{tab:coverage}), so every \emph{best} here is a maximum over the runs "
            r"whose curves were read and can only move upward as more are read. "
        )
        + r"\textbf{Every number in this column is \texttt{roll\_max}, and that is why the "
        r"betaxanthin row reads $0.401$ rather than the $0.4301$ quoted elsewhere for the "
        r"same study}: $0.4301$ is the Optuna objective "
        r"\file{val/betaxanthin/pearson_per_feature_max}, a raw running maximum over "
        r"epochs, while \texttt{roll\_max} first averages five neighboring epochs, which "
        r"can only lower a maximum. The two are different scoring rules over the same runs, "
        r"not different runs. \cref{sec:bx-two-numbers} does the reconciliation.}",
...
def paired_table(board: pd.DataFrame) -> tuple[str, dict]:
    ...
    n_runs_total = int(len(joint))
    joint = joint[joint["cell"].notna() & joint["arm"].notna()]
    n_tagged = int(len(joint))

    # A RUN WITH NO CURVE IS EXCLUDED HERE, NOT AVERAGED IN AS A BLANK. `mean()` skips NaN,
    # so leaving these in silently makes a cell's score the mean over whichever of its runs
    # happened to be read while `n` still reports every run, and a cell with no read run at
    # all becomes a NaN that propagates into the headline mean. Both counts are carried into
    # the caption so the exclusion is visible where the number is quoted.
    scored = joint[joint["primary_roll_max"].notna()]
    n_scored = int(len(scored))

    grouped = scored.groupby(["cell", "arm"]).agg(
        n=("run_id", "size"),
    ...
    for cell in sorted(joint["cell"].unique()):
        arms = grouped.loc[cell].index if cell in grouped.index.get_level_values(0) else []
        if "ctrl" not in arms or "m19" not in arms:
            unpaired.append(cell)
            continue
    ...
    summary = {
        ...
        "n_runs_in_strand": n_runs_total,
        "n_runs_tagged": n_tagged,
        "n_runs_scored": n_scored,
        "n_runs_without_curve": n_tagged - n_scored,
        "cells_without_both_arms": unpaired,
    ...
    lines = [
        ...
        r"\begin{table}[htbp]",
        r"\centering\small",
        # \small over seven columns overflows the 182 mm block by ~6 pt on the widest
        # cell name; \footnotesize is the smallest step that fits without narrowing a
        # column, and the table is a reference rather than something read in flow.
        r"\footnotesize",
        r"\begin{tabular}{lcrrrrl}",
        r"\toprule",
        r"grid cell & runs (ctrl/joint) & control & joint & $\Delta$ & aux $r$ & "
        r"epochs (ctrl/joint) \\",
        r"\midrule",
    ]
    ...
    lines += [
        ...
        r"nothing about the auxiliary head. \textbf{Run accounting.} The project holds "
        + f"{n_runs_total} runs, {n_tagged} of which carry both a grid-cell tag and an arm "
        f"tag, and {n_scored} of those logged a betaxanthin validation curve; the "
        f"remaining {n_tagged - n_scored} are excluded rather than averaged in as blanks. "
        + r"\textbf{Within a cell an arm's score is the MEAN over its runs, not the "
        r"maximum}, because a maximum over an unequal number of runs per arm is a selection "
        r"on the quantity being compared; $n$ per arm is given in the cell column. Unlike "
        r"every other number in this document, this table is built at full per-run history "
        r"coverage for its project (\cref{tab:coverage}), since the default candidate rule "
        r"ranks runs by final score and would select one arm over the other.}",
...
def calibration_table(board: pd.DataFrame) -> str:
    ...
    lines += [
        ...
        r"peaked, which is the model anyone would ship. Above 1 means the model loses to the "
        r"mean while still ordering features correctly, which is under-shrinkage rather than "
        r"incapacity. \textbf{The arithmetic behind the last column.} For predictions that "
        r"are a scaled copy of a correlated signal, $\texttt{nmse} = 1 + s^2 - 2rs$, where "
        r"$s$ is the ratio of prediction to target standard deviation. That is a parabola in "
        r"$s$ with its minimum at $s^\star = r$ and value $1 - r^2$ there, so scaling every "
        r"prediction by $r/s$ moves \file{nmse} to $1 - r^2$ and, being a positive rescale, "
        r"leaves every correlation untouched. $1-r^2$ is therefore the floor this rescale can "
        r"reach, and the gap between the two middle columns is what under-shrinkage costs. A "
        r"strand already \emph{below} $1-r^2$ is not a contradiction: its predictions carry "
        r"structure beyond one global scaling, so a single scalar is not the right correction "
        r"there. Only rounds from the v8 era log these metrics; earlier ones are absent, not "
        r"zero.}",
...
def _run_coverage(board: pd.DataFrame) -> pd.DataFrame:
    """Runs enumerated and runs whose validation curve was actually read, per strand.

    `history_fetched` is written per run by the puller and is the only honest record of
    which rows carry a curve: a summary-only row still has a project, a tag list and a
    final value, so it is indistinguishable from a read run in every column except this
    one and `primary_roll_max`.
    """
    fetched = board.get("history_fetched")
    flag = (
        pd.Series(False, index=board.index)
        if fetched is None
        else fetched.fillna(False).astype(bool)
    )
    have_curve = board["primary_roll_max"].notna()
    frame = pd.DataFrame(
        {
            "strand": board["strand"],
...
def coverage_table(board: pd.DataFrame, census_path: str) -> str:
    r"""Coverage at BOTH granularities, because the two are different claims.

    Every project has been read, and that is what the earlier version of this table said.
    It is also not the statement a reader needs: a strand maximum is a maximum over RUNS,
    and the history pass reads a bounded candidate set per project rather than every run,
    so 28 of 28 projects read is compatible with 306 of 2,187 runs read. Reporting only the
    project count reads as complete coverage of something that is 14 percent covered.

    GENERATED rather than hand-listed because the pull is resumable and both splits move
    every time it makes progress.
    """
    ...
    cov = _run_coverage(board)
    n_runs = int(cov["runs"].sum())
    n_fetched = int(cov["fetched"].sum())
    n_curve = int(cov["curve"].sum())

    project_status = (
        f"All {len(read)} projects have been read"
        if not pending
        else f"{len(read)} of {len(every)} projects read, {len(pending)} pending"
    )
    # The caption has to say something different once the run-level pull actually finishes,
    # or it goes on describing a limitation that no longer binds. Both branches are written
    # here rather than left to prose, because prose about coverage goes stale silently and
    # that is the failure this table exists to prevent.
    complete = n_fetched == n_runs
    if complete:
        run_status = (
            r"\textbf{Every run's history has now been read.} An earlier pass read only a "
            r"bounded candidate set per project, the eight highest final values and the "
            r"five longest runs unioned, at most thirteen per project, on the assumption "
            r"that one request per run would not finish against a rate-limited API. It "
            r"does finish: the full pass takes about twenty minutes. Until it was run, "
            r"every strand maximum in this document was a maximum over at most thirteen "
            r"runs per project rather than over the runs that were run."
        )
    else:
        run_status = (
            r"\textbf{Reading a run's best score is a separate request per run, and most "
            r"runs have not had it.} The pull fetches history for a bounded candidate set "
            r"per project, the eight highest final values and the five longest runs "
            r"unioned, so at most thirteen per project. \textbf{Consequence, stated "
            r"plainly: a strand maximum reported anywhere in this document is a maximum "
            r"over the runs whose curves were read, not over the runs that were run.} It "
            r"can only move upward as more are read, so every score and every "
            r"\emph{of ceiling} figure is a lower bound."
        )
    lines = [
        "%% GENERATED by experiments/019-simb-multimodal/scripts/build_retrospective_tables.py",
        ...
        r"\begin{tabular}{lrrrr}",
        r"\toprule",
        r"strand & projects & runs & history read & with a usable curve \\",
        r"\midrule",
    ]
    for strand, row in cov.iterrows():
        runs = int(row["runs"])
        lines.append(
            f"{str(strand).replace('_', ' ')} & {int(row['projects'])} & {runs} & "
            f"{int(row['fetched'])} ({100 * int(row['fetched']) / runs:.0f}\\%) & "
            f"{int(row['curve'])} \\\\"
        )
    lines += [
        r"\midrule",
        f"all & {len(read)} & {n_runs} & {n_fetched} "
        f"({100 * n_fetched / n_runs:.0f}\\%) & {n_curve} \\\\",
        r"\bottomrule",
        r"\end{tabular}",
        r"\caption[]{Coverage at two granularities, which are two different claims. "
        + project_status
        + r", meaning every project's run list and every run's final summary value is in "
        r"hand. Reading a run's best score is a separate request per run. "
        + run_status
        + r" The \emph{history read} column is the runs whose curves were requested; "
        r"\emph{with a usable curve} is the subset that also logged the strand's metric "
        r"under a name the puller knows, and the gap between the two columns is runs that "
        r"logged nothing under it. \textbf{Why the bounded rule was the wrong instrument.} "
        r"It ranks by final value and by length, so it is built to find a maximum and is "
        r"not a random sample: a mean or a median over it is conditioned on a high final "
        r"value or a long run, and a paired difference computed over it selects on the "
        r"very quantity being compared. "
        r"\textbf{It can also miss a maximum, and did.} A run that peaks early and then "
        r"stops short ranks low on final value and low on length, so it is invisible to "
        r"both arms of the rule at once: reading beta-carotene at full per-run coverage "
        r"raised that strand's best from $0.118$ to $0.137$, held by a run that peaked at "
        r"epoch 9 of 49.}",
