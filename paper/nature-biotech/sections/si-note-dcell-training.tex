...
workstation (\suppfig{fig:dcell-training}d) took a step from 82~s to 20~s, and day-to-day
...
optimizer step as bars. Arrows mark the cumulative chain 1 to 4, measured in one sitting;
stages 5 and 6 add \texttt{torch.compile} to stage 4 at batch 500 and 600. Stage 1 is the
first correct build (one forward pass per step, fp32); an earlier build that ran the forward
pass twice (128~s) and two reruns of the stage-4 configuration on later days (99 and 119~s)
are not drawn: the first is a bug fix, and the shared machine drifts from day to day by more
than several optimizations, so only same-sitting stages compare.
\textbf{e},~Where one training step spends its time on one GPU (RTX 6000 Ada, torch
2.11, bf16-mixed, batch 600, the cluster's per-GPU batch; the epoch-142 checkpoint of
\texttt{eni948by} loaded strictly into the rebuilt model before profiling). The five
``Forward'' phases are the forward pass, gene-state gather being the lookup of each term's
deleted genes, done once per term and per strain. Purple bars, share of the step's host
time (Python plus kernel launches) per phase; orange bars, share of the GPU kernel time; the
phases partition the step. The step takes 30.6~s untraced (118~s under the tracer, which is why
shares rather than seconds are drawn), issues 1,731,051 kernel launches, 2,700 more per
added strain, and the GPU runs kernels for 3.1~s of it (10\%); the gather is 92\% of the
host time and 88\% of the kernel time. The batch is built once and held on the GPU, so no
data loading is in the step.
