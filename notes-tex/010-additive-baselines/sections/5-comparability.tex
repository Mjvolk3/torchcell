...
transformer number. Five do. One does not, and it favors the transformer by a
small amount.
...
\notesec{Paired residuals are not available from the recorded runs}
...
stock evaluation script fails, because the 010 LMDB predates the
perturbation-ontology refactor: every record carries
\texttt{perturbation\_type: "deletion"} where the current pydantic schema admits
only \texttt{sga\_kanmx\_deletion} or \texttt{sga\_natmx\_deletion}, which is 204
validation errors on the first batch. Section~\ref{sec:results} reports how far
