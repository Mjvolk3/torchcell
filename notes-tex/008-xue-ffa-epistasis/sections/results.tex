...
\begin{figure*}[t]
\tcfig{figures/panel_network_overlay.pdf}{\textbf{Trigenic interactions on total fatty acid
...
the multiplicative model, 75 negative (brick) and 11 positive (amber), each drawn as the
three edges of its triangle; edge width is the number of interactions the pair takes part
in, from 1 to 8, and positive is drawn over negative. Every one of the 45 pairs is in at
least one negative interaction. Dotted arrows are regulatory links from a factor to a
...
combinations rather than on a graph. The torchcell equivariant cell graph transformer is
trained on the trigenic growth interaction screens of Kuzmin et al.\ (2018, 2020), where
each triple deletion is scored by colony fitness in a wild-type background. All ten
