...
## 2024.03.24 - Multiprocessing on gene_set and wip on experiment_reference_index
...
def extract_gene_names(dataset, data):
    return extract_systematic_gene_names(data["experiment"]["genotype"])
...
def compute_experiment_reference_index_parallel(
    ...
    with mp.Pool(processes=compute_workers) as pool:
        ...
        for batch_hashes in tqdm(
            ...
            reference_hashes.extend(batch_hashes)
    ...
    def compute_gene_set_sequential(self):
        ...
        with self.env.begin() as txn:
            ...
            for key, value in tqdm(cursor):
                ...
                extracted_gene_names = extract_systematic_gene_names(
                    experiment["genotype"]
                )
        ...
        with mp.Pool(processes=compute_workers) as pool:
            for batch in tqdm(data_loader, total=len(data_loader)):
                gene_names_batch = set(
                    gene_name
                    for gene_names in pool.starmap(
                        extract_gene_names, [(self, data) for data in batch]
                    )
