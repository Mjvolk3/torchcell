# experiments/006-kuzmin-tmi/scripts/hetero_cell_bipartite_dango_gi_lazy_preprocessed
# [[experiments.006-kuzmin-tmi.scripts.hetero_cell_bipartite_dango_gi_lazy_preprocessed]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/006-kuzmin-tmi/scripts/hetero_cell_bipartite_dango_gi_lazy_preprocessed
# Test file: experiments/006-kuzmin-tmi/scripts/test_hetero_cell_bipartite_dango_gi_lazy_preprocessed.py
#
# PREPROCESSED VERSION - 100x faster data loading
# Uses Neo4jPreprocessedCellDataset that loads pre-computed masks from disk
# Prerequisites: Run preprocess_lazy_dataset.py first
...
def main(cfg: DictConfig) -> None:
    print("Starting GeneInteractionDango Preprocessed Lazy Training ⚡ (100x faster data loading)")
    ...
    # PREPROCESSED VERSION: No graph processor needed - masks pre-computed
    # This eliminates 10ms/sample processing overhead during training
    ...
    # PREPROCESSED: Choose dataset based on config
    # Read from wandb.config.data_module.use_full_masks
    # true: full masks (847.7GB), false: compressed indices (3GB)
    use_full_masks = wandb.config.get("data_module", {}).get("use_full_masks", False)

    if use_full_masks:
        # Use full masks dataset
        preprocessed_root = osp.join(
            DATA_ROOT, "data/torchcell/experiments/006-kuzmin-tmi/001-small-build-preprocessed-full-masks"
        )
        print(f"Using FULL MASKS preprocessed dataset from: {preprocessed_root}")
        print("Storage: ~847.7GB LMDB with full boolean masks (no reconstruction overhead)")
    else:
        # Use compressed indices dataset
        preprocessed_root = osp.join(
            DATA_ROOT, "data/torchcell/experiments/006-kuzmin-tmi/001-small-build-preprocessed-lazy"
        )
        print(f"Using COMPRESSED preprocessed dataset from: {preprocessed_root}")
        print("Storage: ~3GB LMDB with compressed indices (15ms reconstruction overhead)")
    ...
    if wandb.config.transforms.get("use_transforms", False):
        ...
        # PREPROCESSED: Create source dataset (for cell_graph/phenotype_info only)
        source_dataset = Neo4jCellDataset(
            ...
            graph_processor=None,  # No processing for source dataset
            transform=None,
        )

        # PREPROCESSED: Use appropriate dataset class based on config
        if use_full_masks:
        ...
        # PREPROCESSED: Create datasets without transforms
        # Create source dataset (for cell_graph/phenotype_info only)
        source_dataset = Neo4jCellDataset(
            root=dataset_root,
            query=query,
            gene_set=genome.gene_set,
            ...
            deduplicator=MeanExperimentDeduplicator,
            aggregator=GenotypeAggregator,
            graph_processor=None,  # No processing
            transform=None,
        )

        # Use preprocessed dataset for actual training
        dataset = Neo4jPreprocessedCellDataset(
            root=preprocessed_root,
            source_dataset=source_dataset,
            transform=None,
