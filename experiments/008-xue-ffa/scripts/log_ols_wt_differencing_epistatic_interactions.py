...
# Define paths
BASE_DIR = Path(osp.join(os.getenv("EXPERIMENT_ROOT"), "008-xue-ffa"))
...
def save_results(results: Dict, output_dir: Path = RESULTS_DIR):
    ...
    with open(output_dir / 'log_ols_results.pkl', 'wb') as f:
        pickle.dump(slim, f)
    print(f"Saved {output_dir / 'log_ols_results.pkl'}")
