...
def plot(t: pd.DataFrame, curves: dict[str, pd.DataFrame]) -> None:
    ...
    fig, axes = plt.subplots(
        1, 3, figsize=(mm_to_in(PANEL_WIDTHS_MM["full"]), mm_to_in(55.0))
    )
    ax_loss, ax_pear, ax_sc = axes
    for _, r in t.iterrows():
        h = curves[r.run_id]
        c = color[str(r.dist)]
        style = dict(color=c, lw=0.5, alpha=0.35 if r.collapsed else 0.9)
        ...
        ax_sc.plot(
            ...
            alpha=0.35 if r.collapsed else 1.0,
        )
    ...
    ax_loss.set_title("a  val/loss, marker at the minimum", loc="left", fontsize=6)
    ax_pear.set_xscale("log")
    ax_pear.set_xlabel("epoch + 1")
    ax_pear.set_ylabel(f"val Pearson per feature ({ROLL_WINDOW}-epoch mean)")
    ax_pear.set_title("b  Pearson, the leaderboard metric", loc="left", fontsize=6)
    ...
    ax_sc.set_title("c  loss minimum vs Pearson peak", loc="left", fontsize=6)
    for ax in axes:
