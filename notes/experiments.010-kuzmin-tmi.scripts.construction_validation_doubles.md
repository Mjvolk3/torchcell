...
0.15, `|eps| <= 0.039`, and **zero significant interactions** — so the assay would have almost
...
- **validation (5)** — added for dynamic range + real signal: the doubles with a
  **significant Costanzo interaction** (`P<0.05 & |eps|>0.08`, both signs), the **highest-DMF**
...
validation = red, unused = gray; significant eps ringed) — the tiered companion to
...
Notes: DMF/SD/SE and eps/p are the published Costanzo values (SD = sample SD over 4 colonies,
...
and the strongest-positive-eps YDR057W+YGL087C each also rebuild triples. The set is tunable:
drop the two triple-less validation doubles for an 11-double set, or add more near-significant eps.
...
Costanzo's bar (`P<0.05 & |eps|>0.08`), and all 3 are within-10 and already in the
...
double's eps is a point estimate; whether it counts as significant depends on how big it is
*relative to its error bar*. The error bar on eps is driven by the colony-to-colony scatter of
the double AND of the two singles it is compared against (all n=4 colonies in Costanzo). So a
double sitting on a noisy strain (CBF1, RPS9A) can show a sizable eps that is still inside its
own error bar — insignificant. The reverse also happens: YBR203W+YDR057W has a tiny eps (+0.062)
...
for *interesting values* (fitness extremes, large eps) and drive each to a tight measurement at
...
otherwise), with Costanzo DMF ± SD, derived SE, eps, p, the **Kuzmin2018/2020 DMF ± SD** columns,
