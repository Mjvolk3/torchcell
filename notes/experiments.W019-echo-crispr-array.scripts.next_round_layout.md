...
Generated by `experiments/W019-echo-crispr-array/scripts/next_round_layout.py`.
Output: `results/run4_layout_options.csv`.

Framing settled: the upward fitness ladder is dead
([[experiments.W019-echo-crispr-array.scripts.ladder_feasibility]]); the next round is scored
on **epistasis** `eps = f_ab - f_a*f_b`, and the model's real target is trigenic `tau`.

![Next-round layout: eps precision, pair-selection leverage, culture floor](assets/images/W019-echo-crispr-array/next_round_layout.svg)
...
Generated by `experiments/W019-echo-crispr-array/scripts/assay_precision_benchmark.py`.
Output: `results/run3_precision_benchmark.csv`.

![Assay precision vs published SGA screens](assets/images/W019-echo-crispr-array/assay_precision_benchmark.svg)
...
Generated by `experiments/W019-echo-crispr-array/scripts/plates_explainer.py`.
Output: `results/run4_plates_explainer.csv`.

![What plates buy and why strains are cheap](assets/images/W019-echo-crispr-array/plates_explainer.svg)
...
Generated by `experiments/W019-echo-crispr-array/scripts/replication_structure.py`.
Outputs: `results/run2_positional_residual_check.csv`.

![Replication structure: normalization check, day floor, same-batch requirement](assets/images/W019-echo-crispr-array/replication_structure.svg)
...
Generated by `experiments/W019-echo-crispr-array/scripts/between_day_variance.py`.
...
2. **Fraction of deletion strains scoring above WT.** Only **2.7%** of 7,738 genome-wide
   ...
   ([[experiments.W019-echo-crispr-array.scripts.ladder_feasibility]]), so 50% above WT is
...
### 2026.07.27 - Was P2's anomaly just positional bias? (mostly no, but partly yes)

Generated by `experiments/W019-echo-crispr-array/scripts/p2_reference_diagnosis.py`.
Output: `results/run3_p2_reference_diagnosis.csv`.

![Is the P2 WT anomaly positional?](assets/images/W019-echo-crispr-array/p2_reference_diagnosis.svg)

The three run-3 plates use INDEPENDENT randomized layouts, so P2's 30 WT wells sit in
different places than P1's and P3's. Residual positional bias dragging them down is the
obvious competing explanation, and it is partly right.

**1. Does the gap survive positional correction?** Partly.
...
The row/column correction recovers **+0.081** of P2's gap, so **a real part of the raw deficit
IS positional** -- and the normalization already removes it. But P2 still lands at 0.963
against 1.275 and 1.300 after every correction. Roughly a quarter of the raw gap is
positional; the rest is not.

**2. Shift or spread?** Positional scatter WIDENS a distribution; a bad reference MOVES it.
...
P2's centre drops by ~0.28 while its spread is unchanged (0.203 vs 0.219 and 0.164). That is
a shift, not scatter.

**3. Are the low WT wells clustered?** No -- the low half spans 10 of 16 rows on P2 (9/16 on
P1 and P3), i.e. the whole plate. So not a local agar defect or dry region.

**4. Sign check -- position predicts the OPPOSITE.** Edge colonies grow LARGER
(`rho(size, edge-distance) < 0`, verified earlier), and P2's WT is the MOST edge-weighted of
the three (37% in the outer two rings, mean edge 2.60, versus 17% and mean edge 3.43 on P1).
Position therefore predicts P2's WT should read HIGH. It reads LOW. Whatever is happening runs
against the positional gradient, not with it.

**Fifth, independent line:** the mutants are the same size on all three plates
(2773 / 2703 / 2664 px, within 4%). A plate-level or positional problem would move mutants too.

**Conclusion, stated at the confidence it deserves.** What is OBSERVED: on P2 the wild-type
colonies are ~25% smaller relative to that plate's own mutants than on any other plate, after
full positional correction, uniformly across the plate. What is INFERRED: something specific
to the BY4741 wells on that plate -- most plausibly the WT source well or its dispense. What is
NOT known: whether that was a handling error or genuine variation in that WT aliquot; a single
anomalous plate out of five cannot distinguish them. "Failed" was the wrong word in an earlier
draft -- "anomalous WT reference" is what the evidence supports.

**Practical consequence is unchanged:** P2's fitness values are unusable because they are all
...
### 2026.07.27 - Which picklist belongs to which image? Verified from content, not filenames

Generated by `experiments/W019-echo-crispr-array/scripts/verify_plate_pairing.py`.
Outputs: `results/run3_pairing_verification.csv`, `results/run3_pairing_cross_h.csv`.

`P2_OD1-5nL_TCsingleKO.JPG` was paired with `cherrypick_Plate2_384_5nL.csv` **by naming
convention alone**. Nothing in either file proves the plate photographed as "P2" is the one
the Echo dispensed from Plate2's picklist -- and if two plates were swapped at imaging, every
strain label on them would be wrong, which is exactly the kind of thing that manufactures an
inexplicable wild-type reference. So the pairing was re-derived from content.

Two independent discriminators, crossed over all 3 images x 3 picklists x 4 orientations.
Blank positions were first confirmed to DIFFER between picklists (rows 5/12, 4/13, 6/11), so
they can discriminate.

**Blank wells empty (max 6):**
...
**Kruskal-Wallis H across strains** (null ~12 for 13 groups; only the correct pairing makes a
strain's 29 replicates agree):
...
Every off-diagonal cell sits at the null. Total H for the filename assignment is **369.3**
against **179.6** for the next-best permutation. **PASS -- the naming convention is confirmed
by the data**, and all three plates independently resolve to `identity`, i.e. **A1 at
top-left**, matching how the plates are photographed.

### A real design flaw found on the way: the blanks cannot see a vertical flip

The blank-emptiness test scores 6/6 for BOTH `identity` and `flip_v` on every plate. The
reason is geometric: the blanks are placed at **mirror-symmetric rows** -- (5, 12), (4, 13),
(6, 11), each pair summing to 17 -- so a vertical flip maps the blank set onto itself. Columns
are asymmetric, which is why `rot180` and `flip_h` are correctly excluded.

So the blanks currently resolve 2 of the 3 wrong orientations, not all 3. Nothing went wrong
here (the strain-structure test settles it, and A1-top-left is the physical convention), but
the check is weaker than it looks.

**Fix for the next picklist generator: place the blanks asymmetrically in BOTH axes** -- e.g.
rows that do not pair to `n_rows + 1`. That makes the 6 blank wells alone sufficient to pin
the orientation, without relying on strain structure.

### The general lesson

Four times now an assumption that lived only in a **name or a convention** has needed
verification: cached masks that were post-recovery, committed CSVs from a superseded pipeline,
a blank test that cannot resolve a vertical flip, and an image-to-picklist pairing asserted
only by filename. The provenance rule ("where did this come from and how was it done?") has to
cover the **linkage between artifacts**, not just the artifacts. `verify_plate_pairing.py`
should run as an assertion on every future plate set, not as a one-off.

### 2026.07.27 - Blank placement fixed: picklist generation moves in-repo

Generated by `experiments/W019-echo-crispr-array/scripts/generate_picklist.py`.
Outputs: `experiments/W019-echo-crispr-array/picklists/`.

Randomized plates were previously generated outside this repo by a colleague; from the next
round we generate them here, which is where the blank-symmetry fix lands.

**The bug being fixed.** The old blanks sat at mirror-symmetric rows -- (5, 12), (4, 13),
(6, 11), each pair summing to `n_rows + 1` -- so a vertical flip maps the blank set exactly
onto itself and the blank-emptiness test scores a perfect 6/6 for BOTH `identity` and
`flip_v`. Blanks resolved 2 of the 3 wrong orientations, not 3 of 3.

**The fix.** Blank positions are now chosen so that for every non-identity orientation `f`,
`S ∩ f(S)` is EMPTY -- every blank moves onto a well that should carry a colony. That is
strictly stronger than `S != f(S)`, which would only guarantee *some* blanks move. Placement
is also stratified (one blank per block of the plate) so they probe registration across the
whole surface rather than one corner. The generator audits its own output and exits non-zero
if any plate fails.

Result on three freshly generated plates:

| plate | blank wells | blanks surviving rot180 / flip_v / flip_h |
|---|---|---|
| Plate1 | (1,9) (4,6) (6,18) (9,2) (15,24) (16,10) | 0 / 0 / 0 |
| Plate2 | (5,4) (7,11) (8,22) (9,21) (13,4) (15,11) | 0 / 0 / 0 |
| Plate3 | (3,2) (3,20) (5,11) (12,22) (15,6) (15,15) | 0 / 0 / 0 |

Six blanks now pin the orientation on their own, with no reliance on strain structure.

**These files are PROVISIONAL.** Only the 12 assayed singles exist today, so the wild type
absorbs 222 wells. Once the 14 doubles are built, add them to `SOURCE_WELLS` (continuing the
