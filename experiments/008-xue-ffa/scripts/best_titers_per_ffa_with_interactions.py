# experiments/008-xue-ffa/scripts/best_titers_per_ffa_with_interactions
# [[experiments.008-xue-ffa.scripts.best_titers_per_ffa_with_interactions]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/008-xue-ffa/scripts/best_titers_per_ffa_with_interactions
# Test file: experiments/008-xue-ffa/scripts/test_best_titers_per_ffa_with_interactions.py
...
def find_top_titers(
    ...
    digenic_interactions, trigenic_interactions,
    single_se, double_se, triple_se,
    digenic_se, trigenic_se,
    columns, abbreviations=None, top_n=10
):
    ...
    for ffa_idx, ffa in enumerate(ffa_types):
        ...
        for gene, fitness_values in single_mutants.items():
            if ffa_idx < len(fitness_values) and not np.isnan(fitness_values[ffa_idx]):
                display_gene = letter_to_gene.get(gene, gene) if letter_to_gene else gene
                genotype_data.append({
                    ...
                    'f_j': np.nan, 'f_k': np.nan,
                    'f_ij': np.nan, 'f_ik': np.nan, 'f_jk': np.nan,
                    'f_ijk': np.nan,
                    'eps_ij': np.nan, 'eps_ik': np.nan, 'eps_jk': np.nan,
                    'tau': np.nan,
                    'se_i': single_se.get(gene, [0]*6)[ffa_idx] if single_se else 0,
                    'se_j': 0, 'se_k': 0,
                    'se_ij': 0, 'se_ik': 0, 'se_jk': 0,
                    'se_ijk': 0,
                    'se_eps_ij': 0, 'se_eps_ik': 0, 'se_eps_jk': 0,
                    'se_tau': 0
                })
        ...
        for (gene1, gene2, gene3), fitness_values in triple_mutants.items():
            if ffa_idx < len(fitness_values) and not np.isnan(fitness_values[ffa_idx]):
                ...
                tau = trigenic_interactions.get((gene1, gene2, gene3), [np.nan]*6)[ffa_idx]
...
def plot_top_titers(top_genotypes_by_ffa, columns):
    ...
    for ffa, genotypes in top_genotypes_by_ffa.items():
        ...
        # Singles
        ax1.bar(x + bar_positions[0], [g['f_i'] for g in genotypes], width,
        ...
        ax1.bar(x + bar_positions[1], [g['f_j'] for g in genotypes], width,
                yerr=[g['se_j'] if not np.isnan(g['se_j']) else 0 for g in genotypes],
                label=r"$f_j$ (4$\Delta$)",
                color=color_j, alpha=0.8, capsize=3,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[2], [g['f_k'] for g in genotypes], width,
        ...
        # Triple
        ax1.bar(x + bar_positions[6], [g['f_ijk'] for g in genotypes], width,
        ...
        ax1.bar(x + bar_positions[7], [g['eps_ij'] for g in genotypes], width,
                yerr=[g['se_eps_ij'] if not np.isnan(g['se_eps_ij']) else 0 for g in genotypes],
                label=r"$\varepsilon_{ij}$",
                color=color_eps_ij, alpha=0.7, capsize=3,
                hatch='///', edgecolor='black', linewidth=0.5,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[8], [g['eps_ik'] for g in genotypes], width,
                yerr=[g['se_eps_ik'] if not np.isnan(g['se_eps_ik']) else 0 for g in genotypes],
                label=r"$\varepsilon_{ik}$",
                color=color_eps_ik, alpha=0.7, capsize=3,
                hatch='///', edgecolor='black', linewidth=0.5,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[9], [g['eps_jk'] for g in genotypes], width,
                yerr=[g['se_eps_jk'] if not np.isnan(g['se_eps_jk']) else 0 for g in genotypes],
                label=r"$\varepsilon_{jk}$",
                color=color_eps_jk, alpha=0.7, capsize=3,
                hatch='///', edgecolor='black', linewidth=0.5,
                error_kw={'linewidth': 1, 'alpha': 0.7})

        ax1.bar(x + bar_positions[10], [g['tau'] for g in genotypes], width,
                yerr=[g['se_tau'] if not np.isnan(g['se_tau']) else 0 for g in genotypes],
                label=r"$\tau_{ijk}$",
        ...
        ax1.set_title(f"Top 10 Titers for {ffa} Across All Strains",
        ...
        print(f"  Top genotype: {genotypes[0]['genotype']} ({genotypes[0]['type']}) - fitness: {genotypes[0]['actual_fitness']:.3f}")
...
def main():
    ...
    top_genotypes_by_ffa = find_top_titers(
        ...
        single_se, double_se, triple_se,
        digenic_se, trigenic_se,
        columns, abbreviations, top_n=10
    )
