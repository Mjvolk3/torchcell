...
- [x] Costanzo SMF 26 C = 30 C explained: the source released one temperature-combined value for deletion and DAmP strains (SI line 96); the loader stamps it as two records, 8,454 twins of 20,484; TS alleles are distinct. Issue #410; reader drops the twin, loader fix at the next full rebuild
- [x] 030 query and build designed ([[experiments.030-solid-growth-multi]]): 025 query (all alleles) + 029 build (no merge) + two new query-strain fitness datasets; policy at read time; deletions-only becomes a subset index; order of work and cost projection in the note
