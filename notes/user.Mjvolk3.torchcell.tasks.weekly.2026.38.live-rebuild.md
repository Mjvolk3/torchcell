...
- [x] Fix: restart the container instead of the process, `RESUME_JOB=<old job>` mode that skips generation and import, defaults 24 CPU / 256 G with the measured envelope in the header [[database.slurm.scripts.gilahyper_live_rebuild-slurm_docker]]. Resume of 2032 submitted after landing; validation, swap and manifest bootstrap recorded here on completion.
- [x] Resume 2311 restarted the container and counted the store (99,723,455 nodes, 51 Dataset nodes) but failed reading the CSVs on the host: the image entrypoint sets mode 700 on the mounted `biocypher-out` at every container start. Validate now reopens the directory first; resume accepts a running build container.
- [x] Resume 2325 read the CSVs but the live-count Cypher failed on Neo4j 5.26 (implicit grouping in an aggregation expression). Rewritten as two columns; run by hand, live == CSV for all 51 datasets.
- [x] Resume 2331 (4 CPU / 16 G) validated (live == CSV, 51 datasets) and SWAPPED at 20:35 CDT: serving 99,723,455 nodes / 51 datasets (was 83,066,931 / 36), old store at `data.superseded.2026-09-17_20-35-08`. The job died on the serving poll after the swap (assignment from a failing pipeline under set -e); fixed. Manifest bootstrapped by hand at 7715ee35 (old kept as `.superseded.2026-09-17_20-35-08`); CSVs archived to /bulk.
