...
## 2026.07.10 — Dataset #3: Auesukaree2009 six-stress tolerance screen (L0–L4 PASS)

Built `torchcell/datasets/scerevisiae/auesukaree2009.py`
(`EnvChemgenAuesukaree2009Dataset`, `env_chemgen_auesukaree2009`) on the FROZEN
EnvironmentPerturbation schema — **no schema change needed**. This is the FIRST WS15 dataset
to exercise `PhysicalStressPerturbation` (Vanacloig/Mota/Wildenhain were all SmallMolecule).

**Records: 525** — one categorical `sensitive` record per (haploid single KanMX deletion in
BY4742) × stress. **Per-stress counts: ethanol 95, methanol 55, 1-propanol 125, heat 178,
NaCl 42, H2O2 30.** 6 unique references (one per stress). 333 distinct measured ORFs
(genes recur across stresses; L4 SGD-R64 containment = 1.000 of 333).

**How the physical-stress leaves were used** (task-specified mapping, all realized):

- alcohols (ethanol/methanol/1-propanol) → `SmallMoleculePerturbation(compound_name,
  Concentration(value, unit="percent_v/v"), stress_category="alcohol")`, Environment temp 30 °C.
- heat → `PhysicalStressPerturbation(stress_type=temperature, magnitude=Concentration(37,
  "Celsius"), agent=None)`, with the Environment temperature itself raised to 37 °C.
- NaCl → `PhysicalStressPerturbation(stress_type=osmotic, agent="NaCl",
  magnitude=Concentration(1, "M"))`, temp 30 °C.
- H2O2 → `PhysicalStressPerturbation(stress_type=oxidative, agent="H2O2",
  magnitude=Concentration(5, "mM"))`, temp 30 °C.

All six: `Media("YPD", state="solid")`, aerobic, `duration_hours=72` (3 days). Readout
`EnvironmentResponsePhenotype(measurement_type=categorical, category="sensitive")`; reference
(parental BY4742) `category="tolerant"`. The pair_uniqueness verifier keys physical-stress
records on `agent` (NaCl/H2O2) or `stress_type` (temperature for heat) — 6 distinct compound
keys, no cross-stress collision.

**Data source — born-digital extraction (no external data deposit; the article Tables ARE the
data):** the six per-stress sensitive-gene lists (Tables 1–6) are extracted deterministically
...
(**sha256 `01b945443c0ce41642c76fd737e12b4c31cacb5f384049a5c0a7e4bf9e1eb5a1`**, pinned +
verified in `download()`, copied from the library mirror — PMC file download is JS
proof-of-work, not scriptable) via `pdftotext -layout` (poppler). Each functional-class row
declares its member count in parentheses (e.g. "Vacuolar function (16)"); the parser uses
these declared counts as **per-class self-checksums** against extraction drift, and asserts
each stress's listed total. The MinerU `paper.md` in the same mirror is a lossy fallback (its
heat Table 4 "Unknown function" cell is truncated by two ORFs, `YOR364w`/`YPL144w`); the
born-digital PDF text layer is complete → source of record (matches the project
born-digital-extraction strategy). The functional classification is not stored (only
gene × stress sensitivity is needed).

**Sourced provenance (verbatim quotes, `paper.md`/PDF):**

- **Background (confirmed haploid, NOT HIP/HOP):** *"The 4828 nonessential haploid S.
  cerevisiae deletion strains generated by the Saccharomyces Genome Deletion Project
  (Winzeler et al. 1999) were obtained from Research Genetics/Invitrogen. These strains are
  on a BY4742 (MATα his3Δ1 leu2Δ0 lys2Δ0 ura3Δ0) background (Brachmann et al. 1998)."*
  (Methods, "Strains and media"). Winzeler collection = KanMX cassette ⇒ genotype = one
  `KanMxDeletionPerturbation`; BY4742 auxotrophies = standard collection background, captured
  by `ReferenceGenome(strain="BY4742")` (default ploidy haploid), NOT modeled as
  perturbations (Costanzo/Kuzmin/Mota convention). No constant-background screened genes.
- **Conditions / concentrations:** *"strains were stamped onto YPD plates and YPD plates
  ...
  non-inhibitory levels from the WT spot test, Results/Fig. 1). YPD solid, 30 °C (37 °C heat),
  3 days = 72 h, aerobic.
- **Categorical scoring:** *"Deletion mutants showing significantly reduced growth on plates
  ...
  sensitive mutants."* (Methods) → `category="sensitive"`; parental reference `"tolerant"`.
  The paper reports ONLY sensitive hits (no full screened matrix), so one `sensitive` record
  per (gene, stress); the tolerant majority is the implied reference. No mutant scored as
  INCREASED tolerance → no "resistant" category.
- **n_samples = 3 (STATED):** *"All experiments for the identification of stress-sensitive
  mutants were performed in triplicate."* (Methods). The paper does not specify biological
  vs technical; each triplicate is an independent screening experiment of the collection →
  `sample_unit=biological_replicate` (independent-experiment reading, Vanacloig precedent).
  Categorical readout carries no SE, so no uncertainty fields.
...
- **METHANOL = one internal paper discrepancy:** abstract/Fig. 2 headline says **54**, but
  Table 2 lists **55** distinct genes AND its functional-class parentheticals also sum to 55
  (11+7+4+5+2+5+2+2+4+5+1+7). The Table (the actual per-gene data) is authoritative over the
  summary headline → **55 methanol records stored**; the 1-gene headline-vs-table mismatch is
  flagged as a paper-internal count error.
- **Every one of the 525 listed gene tokens resolves to a unique current SGD R64 systematic
  ORF (zero drops).** One paper-era systematic name, `YGR272c` (heat), is an old identifier
  the genome alias table maps to current R64 `YGR271C-A`; the resolver follows the alias
  (not dropped). Resolver order: current-R64-ID → (old-systematic→alias) → gene name → Alias
  column → alias map.

**L0–L4: PASS** (report at
`…/env_chemgen_auesukaree2009/preprocess/verification_report.json`, sibling of
`experiment_reference_index.json`): L0 525 validated; L1 count=525 + 525 unique (ORF, compound)
pairs; L2 value/SE fidelity (0 numeric values — categorical); L3 single
measurement_type=categorical + reference_zero (0 categorical refs) + all 525 env-perturbed;
L4 1.000 of 333 measured genes are S288C reference genes (≥0.90). Runner entry added to
`ENVIRONMENT_RESPONSE_DATASETS` (`background_genes=frozenset()`, `expected_count=525`).
Tests: 292 passed (datamodels + verification, incl. 8 ontology-integrity invariants).
mypy-strict + ruff clean on all changed files (`auesukaree2009.py`, `datasets/scerevisiae/
__init__.py`, `verification/runners.py`).
