...
## 2026.09.18 - Review round 8: back to six panels, larger level sets

The two schematic panels moved to the top of Fig. 1 (`perspective_motivation_drawio.py` imports `motivation` and `classic` from here). Rows: measured panels at 5 mm (52 tall), level sets at 62 (40 tall, up from 36), schematic at 107, table at 125 with 6.4 mm rows; content to 155.6 mm. The lines under the schematic's boxes name the null they describe ("multiplicative: the second deletion acts only on what the first let through"; "additive: each loss comes off the whole pool, so their overlap is removed twice") and the identity line ends ", what the second deletion would take again from the part the first removed".
