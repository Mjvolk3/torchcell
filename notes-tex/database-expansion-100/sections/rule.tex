...
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt]
  \item \term{S288C-KO} -- the reference genome minus one cataloged ORF (BY4741, BY4742,
    BY4743). Thirty-one of the 150 that reach 200 sit here, the largest single group.
...
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt]
  \item \term{perturb-seq}, 28 rows. A CRISPR interference or activation library whose
  ...
  \item \term{metabolism x expression}, 18 rows. Metabolism and expression on genotypes
    that can be joined, either inside one study or against a supported dataset: a proteome
    or metabolome on genotypes that already carry a transcriptome, or expression under the
    conditions where flux is measured.
  \item \term{scale}, 115 rows. Everything else, ordered as before.
\end{itemize}

Within a band the order is tier, then measurements. The cost of the banding is visible and
is not hidden: de Boer 2020 falls from 2 to 47 and Lee 2014 from 4 to 49. Both remain
inside the first fifty, so nothing large is deferred by it; what changes is that the first
forty-six rows are chosen for what they combine with rather than for how many numbers they
contain. Every move is listed in Table~\ref{tab:swaps}.
...
recoverable per-strain data, which has already happened once to Lee 2014. Wave 1 is 28
Perturb-seq rows, 18 metabolism-and-expression rows and 4 scale rows, and wave 2 is scale
throughout, so the two waves differ in kind and not only in rank.
