...
Figure 1, the ladder under both splits: a and b are the arm R and arm Q ladders (test, B5 with its three-seed sd, transformer validation maxima in the darker line color against light-fill test bars), c is the per-epoch validation curve of the 010 configuration under each split against its additive null, d is the spread of the disjoint null across five held-out screen sets on the 010 build with the arm Q single split marked. Arm Q lies above every fold for B1 (0.185 against 0.088 to 0.151), B2, B3 and B5, so it is a comparatively easy draw of 46 screens.
...
## 2026.09.13 - Shade instead of hatch for validation versus test

The figure standard no longer hatches bars of one series to mark a different scoring surface; lightness carries it. In Figure 1a and 1b every test bar now takes the arm's light fill (`PLOT_PALETTE_FILL`) and the one validation-maximum bar per arm keeps the darker line color (`PLOT_PALETTE`), with the legend in panel a naming the pair. Panel d is a single-series panel with no validation counterpart and keeps the line-color gray. Figure 2 never used hatching. The captions of Figure 1 were reworded to match.

## 2026.09.14 - Long and short versions from one set of sections

The condensed document had dropped the model equations, the reading of the transformer code and two of the 010 report's figures, which was more than the sharing copy should lose. The document now builds two PDFs from one `sections/` tree: `make` builds the long version and `make short` the short one, whose root sets `\longfalse` so every `\iflong ... \fi` block is dropped. Both carry the full equations for B0 to B5 and the transformer readout (Eq. cgt), the provenance and capacity tables, the two 025 panel figures, and the disjoint-run inventory. The long version adds the 010 material that still applies because arm R is the 010 split record for record: what each model sees (encoder at batch 1, the graph penalty and its effective coefficient, the comparability audit, the parameter budget), the reading of the random-split ladder (seed spread, capacity, Spearman), the hand-built forward-pass reproduction, the graph ablation (penalty share, neighbor recall, the matched pair without the penalty), the record-by-record agreement with its hexbin figure and bootstrap, and the five-fold disjoint table. Two 010 figures come in through `PLOT_SLUGS := 025-solid-growth 010-kuzmin-tmi`: `graph_penalty_vs_loss` (four panels) and `paired_prediction_agreement` (three panels). Long 17 pages, short 10, both gates clean.

## 2026.09.15 - Test bar, legends outside the axes, and the tick labels

The panel script now reads `results/cgt_checkpoint_scores_327csnlk.json` and draws GH 1640's epoch 7 test score as a light-fill bar beside its validation maximum in Figure 1b and in the first slot of Figure 2c, which is no longer a placeholder; the three seed slots stay empty. Every legend moved above its axes and the panel titles into the legends or the caption, so nothing but data is drawn inside the axes; the dumbbell panel's tick labels are the job number over the gene input. The summary json carries the test score under `job_1640_epoch7_test`.
