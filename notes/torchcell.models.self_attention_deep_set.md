---
id: ykvpyfrpzum83v5tmvxdedk
title: Self_attention_deep_set
desc: ''
updated: 1712891905647
created: 1712891605573
---
We want the `SelfAttentionDeepSet` to be very similar to `DeepSet` but to use an attention mechanism on the node layers. In the forward method I want to return the attention weights as a list (depending on `num_node_layer`) of the attention weights which should be `[num_heads, num_nodes, num_nodes]`. I should be able to use the batch vector to select out one of the individual graphs that will be `[num_heads, num_nodes_single_indexed_graph, num_nodes_single_indexed_graph]`.
