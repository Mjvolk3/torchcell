...
### Did the other screens run wild type on every plate? No -- and they did not need to

Baryshnikova 2010 SI, line 150, on what the score is referenced to:

> "This is simply the difference between the normalized colony size for a particular double
> mutant and **the median normalized colony size across all query strains** that have been
> [screened against that array mutant]."

SGA's reference is a **median over hundreds-to-thousands of mostly-neutral array mutants**,
not dedicated wild-type wells. On a genome-wide array the plate median IS effectively wild
type, and it is estimated from ~1,536 colonies rather than a handful of controls. Kuzmin
does run explicit control queries -- the `his3` and `ura3` neutral-locus strains (SI line
59) -- but as *separate screens*, not as wells co-plated with the mutants.

**Why we cannot copy that.** Our 26 strains are all real knockouts with a median fitness of
~0.85, so our plate median is dragged well below wild type and is not a neutral reference.
Hence the explicit on-plate BY4741. Note this only affects the *reference*: the plate median
cancels in `median(strain)/median(WT)`, so the choice is not a bias, it is a precision
question -- and it is why WT should take the leftover wells (46 at c = 13), since its error
propagates into every strain.
