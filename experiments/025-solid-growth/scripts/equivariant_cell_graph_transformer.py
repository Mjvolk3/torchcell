# experiments/025-solid-growth/scripts/equivariant_cell_graph_transformer.py
# [[experiments.025-solid-growth.scripts.equivariant_cell_graph_transformer]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/025-solid-growth/scripts/equivariant_cell_graph_transformer
"""Train the equivariant CellGraphTransformer on a subset/split arm of the 025 build.

Ported from ``experiments/010-kuzmin-tmi/scripts/equivariant_cell_graph_transformer.py``,
which trained on a 376,732-record build holding exactly the trigenic triples and exactly
one label. 025 holds 13,525,071 records across three perturbation orders and two labels,
so the same training run has to be expressed rather than assumed. Four config-driven
pieces carry that, and each one silently produces a WRONG run if omitted:

- ``cell_dataset.phenotype_labels`` selects and orders the labels. Without it a triple
  yields fitness AND gene_interaction, so a batch of B records carries 2B targets.
- ``subset.indices`` restricts the record pool through ``CellDataModule.index_subset``.
  Pinning a split assigns records but never excludes them, so without this the 13.1M
...
Run through the SLURM launcher, which sets the DDP rendezvous:
    ...
    sbatch -J <config> experiments/025-solid-growth/scripts/gh_cgt.slurm <config>
"""
...
def load_index_artifact(name: str, key: str | None) -> list[int] | dict[str, list[int]]:
    """Read a committed index artifact from ``025-solid-growth/results``.

    ``key`` selects the field holding the indices, because the two split artifacts store
    them under different names by history: the R transfer writes ``pinned`` beside its
    match report, the Q split writes ``splits`` beside its per-pair assignment. Naming the
    field in config keeps that explicit rather than probing for whichever key happens to
    be present.
    """
    path = osp.join(EXPERIMENT_ROOT, "025-solid-growth", "results", name)
...
def main(cfg: DictConfig) -> None:
    ...
    # The split the model actually trains on must equal the pinned artifact, not merely
    ...
    print(
        f"splits: train={len(data_module.index.train)} "
        f"val={len(data_module.index.val)} test={len(data_module.index.test)}"
    )
