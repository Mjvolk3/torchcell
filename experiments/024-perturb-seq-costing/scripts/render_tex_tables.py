...
Run:  python experiments/024-perturb-seq-costing/scripts/render_tex_tables.py
"""
...
def t8() -> None:
    ...
    note = (
        ...
        r"plasmid. \textbf{Analyzed} = cells surviving QC \emph{and} assigned to a "
...
def t10() -> None:
    ...
    note = (
        r"\textbf{Plex $k$} = gRNAs delivered per cell, each against a "
...
def _provenance_table(spec, recs: list) -> list[str]:
    ...
    lines = [
        ...
        r"judgment call or that was corrected in review; an entry with no "
        r"citation key is not backed by the library mirror and should not be "
        r"quoted. Generated from "
        r"\file{experiments/024-perturb-seq-costing/scripts/figure_sources.py}.}"
