...
\notesec{One name-resolution hazard to check before the comparison is published} Of the 640
comparison genes, 630 carry a betaxanthin measurement, and the pin file holds 639. The
screen reports one gene as \gene{PPA1}, which is an ambiguous alias: it names both
\gene{IPP1} (\gene{YBR011C}) and \gene{VMA16} (\gene{YHR026W}). The split builder resolves it
to \gene{VMA16} on decisive evidence, that Cachera screened the non-essential deletion
collection and \gene{IPP1} is essential, so an \gene{ipp1}$\Delta$ strain cannot be in it.
The training data still inherits the stale resolver (issue \#195) and may carry the other
assignment, which would attach one comparison gene's measurement to the wrong ORF.
\textbf{Verify this specific gene against the training build before publishing the
comparison}; it has not been confirmed either way here.
