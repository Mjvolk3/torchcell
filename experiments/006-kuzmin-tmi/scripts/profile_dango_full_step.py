...
class DangoFullStepProfiler:
    """Profiler for complete training step of Dango model"""
    ...
    def profile_full_training_step(
        ...
        for i in range(warmup):
            optimizer.zero_grad()
            # DANGO returns (predictions, outputs_dict)
    ...
    def print_summary(self, results: Dict[str, ProfileResult], output_file: str):
        """Print and save formatted summary of profiling results"""

        print("\n" + "="*80)
        print("DANGO COMPLETE TRAINING STEP PROFILING")
        print("="*80)
        ...
        with open(output_file, "w") as f:
            f.write("DANGO COMPLETE TRAINING STEP PROFILING\n")
            f.write("="*80 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description="Profile DANGO complete training step")
    ...
    # Edge types for DANGO (STRING12.0 networks)
    edge_types = [
    ...
    # Create profiler
    profiler = DangoFullStepProfiler(model, device)
    ...
    output_file = osp.join(args.output_dir, f"profile_dango_full_step_results_{timestamp_str}.txt")

    # Print summary
    profiler.print_summary(results, output_file)
