...
def create_plots(df: pd.DataFrame, size: int):
    ...
    for metric in metrics:
        ...
        title = f"002-dmi-tmi Random Forest {size_str} - {metric} vs. Number of Parameters"
        ax.set_title(title, fontsize=16)

        plot_name = f"002-dmi-tmi_Random_Forest_{size_str}_{metric}_vs_num_params.png"
        plt.tight_layout()
...
def main():
    ...
    for df, size in zip(dataframes, [1000, 10000, 100000]):
        if not df.empty:
            processed_df = process_raw_dataframe(df, config_columns, summary_columns)
            print(
                f"Number of rows with missing num_params for size {size}: {len(df) - len(processed_df)}"
            )
            processed_df.to_csv(
                osp.join(RESULTS_DIR, f"random_forest_processed_df_{size}.csv"),
                index=False,
            )
            create_plots(processed_df, size)
