...
def print_timing_summary(
    ...
    if filter_class:
        ...
        filtered_timings = {
            k: v for k, v in _TIMINGS.items() if k.startswith(filter_class + ".")
        }
    else:
    ...
    for name, times in sorted(
        filtered_timings.items(), key=lambda x: sum(x[1]), reverse=True
    ):
...
def print_comparison_table(
    ...
    print("=" * 100)
    print(
        f"{'Method':<35} {baseline_class[:12]:>12} {optimized_class[:12]:>12} {'Change':>12} {'Speedup':>10}"
    )
    print("-" * 100)
    ...
    for method in all_methods:
        ...
        if baseline_time > 0 and optimized_time > 0:
            ...
            print(
                f"{method:<35} {baseline_time:>11.2f}ms {optimized_time:>11.2f}ms {change:>11.2f}ms {speedup:>9.2f}x {indicator}"
            )
        elif baseline_time > 0:
            print(
                f"{method:<35} {baseline_time:>11.2f}ms {'N/A':>12} {'N/A':>12} {'N/A':>10}"
            )
        elif optimized_time > 0:
            print(
                f"{method:<35} {'N/A':>12} {optimized_time:>11.2f}ms {'N/A':>12} {'N/A':>10}"
            )
    ...
    print(
        f"{'TOTAL':<35} {baseline_total:>11.2f}ms {optimized_total:>11.2f}ms {change_total:>11.2f}ms {speedup_total:>9.2f}x"
    )
    print("=" * 100 + "\n")
