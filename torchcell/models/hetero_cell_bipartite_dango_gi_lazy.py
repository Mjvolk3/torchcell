...
def main(cfg: DictConfig) -> None:
    ...
    dataset, batch, input_channels, max_num_nodes = load_sample_data_batch(
        ...
        num_workers=0,  # Single worker for debugging (lazy is fast enough!)
        config="hetero_cell_bipartite",
        is_dense=False,
        use_custom_collate=True,  # CRITICAL: Use LazyCollater for zero-copy batching
    )
    ...
    # LAZY VERSION: Initialize model - GIN ONLY (no GATv2)
    ...
    if gene_encoder_config_dict.get("encoder_type") == "gatv2":
        raise NotImplementedError(
            "GATv2 not supported in lazy version. Set encoder_type='gin' in config."
        )
    gene_encoder_config_dict["encoder_type"] = "gin"
    ...
    def save_intermediate_plot(
        ...
        if loss_components_history and loss_type in [
            ...
            # Extract components - handle both MleWassSupCR and MleDistSupCR
            weighted_mse = [comp["weighted_mse"] for comp in loss_components_history]
            # Check which key exists for distance/wasserstein loss
            if "weighted_wasserstein" in loss_components_history[0]:
                weighted_dist = [
                    comp["weighted_wasserstein"] for comp in loss_components_history
                ]
                dist_label = "Weighted Wasserstein"
            else:
                weighted_dist = [
                    comp["weighted_dist"] for comp in loss_components_history
                ]
                dist_label = "Weighted Dist"
            weighted_supcr = [
        ...
        if loss_components_history and loss_type in [
            ...
            # Extract unweighted components - handle both MleWassSupCR and MleDistSupCR
            mse_loss = [comp["mse_loss"] for comp in loss_components_history]
            # Check which key exists for distance/wasserstein loss
            if "wasserstein_loss" in loss_components_history[0]:
                dist_loss = [
                    comp["wasserstein_loss"] for comp in loss_components_history
                ]
                dist_label = "Wasserstein Loss"
            else:
                dist_loss = [comp["dist_loss"] for comp in loss_components_history]
                dist_label = "Dist Loss"
        ...
        if loss_components_history and loss_type in [
            ...
            # Calculate difference between unweighted dist and mse losses - handle both loss types
            if "wasserstein_loss" in loss_components_history[0]:
                dist_minus_mse = [
                    comp["wasserstein_loss"] - comp["mse_loss"]
                    for comp in loss_components_history
                ]
            else:
                dist_minus_mse = [
                    comp["dist_loss"] - comp["mse_loss"]
                    for comp in loss_components_history
                ]
        ...
        if lambda_values_history and loss_type == "icloss":
            ...
            if loss_components_history:
                ...
                # Calculate sum of weighted components - handle both loss types
                weighted_dist_key = (
                    "weighted_wasserstein"
                    if "weighted_wasserstein" in current_components
                    else "weighted_dist"
                )
                components_sum = (
                    current_components["weighted_mse"]
                    + current_components[weighted_dist_key]
                ...
                y_start = 0.57
                # Use appropriate label and key based on loss type
                dist_label = (
                    "Weighted Wasserstein"
                    if weighted_dist_key == "weighted_wasserstein"
                    else "Weighted Dist"
                )
                component_texts = [
        ...
        for epoch in range(num_epochs):
            ...
            if isinstance(criterion, LogCoshLoss):
                ...
                if isinstance(criterion, ICLoss):
                    ...
                    loss_name = "MleWassSupCR"
                    # MleWassSupCR uses different keys
                    print(
                        f"  {loss_name} components: mse={loss_dict['mse_loss']:.4f}, wasserstein={loss_dict['weighted_wasserstein']:.4f}, supcr={loss_dict['weighted_supcr']:.4f}"
                    )
                else:
                    loss_name = "MleDistSupCR"
                    # MleDistSupCR uses dist keys
                    print(
                        f"  {loss_name} components: mse={loss_dict['mse_loss']:.4f}, dist={loss_dict['weighted_dist']:.4f}, supcr={loss_dict['weighted_supcr']:.4f}"
                    )

                # Track loss components - use appropriate keys based on loss type
                    ...
                    loss_components_history.append(
                        {
                            ...
                            "weighted_wasserstein": float(
                                loss_dict["weighted_wasserstein"]
                            ),
                            "weighted_supcr": float(loss_dict["weighted_supcr"]),
                            "mse_loss": float(loss_dict["mse_loss"]),
                            "wasserstein_loss": float(loss_dict["wasserstein_loss"]),
    ...
    with torch.no_grad():
        ...
        if np.sum(valid_mask) > 0:
            ...
            # Loss components evolution (for ICLoss/MleWassSupCR/MleDistSupCR) - final plot
            if loss_components_history and loss_type in [
                "icloss",
                "mle_wass_supcr",
                "mle_dist_supcr",
                ...
                # Set title based on loss type
                if loss_type == "icloss":
                    plot_title = "ICLoss Weighted Components Evolution"
                elif loss_type == "mle_wass_supcr":
                    plot_title = "MleWassSupCR Weighted Components Evolution"
                else:
                    plot_title = "MleDistSupCR Weighted Components Evolution"
                ...
                # Use appropriate label for dist/wasserstein
                final_dist_label = (
                    "Final Wasserstein"
                    if dist_label == "Weighted Wasserstein"
                    else "Final Dist"
                )
                plt.text(
                    0.02,
                    0.90,
                    f"{final_dist_label}: {weighted_dist[-1]:.6f}",
                    transform=plt.gca().transAxes,
                    verticalalignment="top",
                    fontsize=10,
                    bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.5),
