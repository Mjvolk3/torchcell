...
## 2026.09.09 - The CLS Goes Through the Perturbation Operator (`perturb_cls`)

Until now `h_CLS` was sliced off the encoder output BEFORE `EquivariantPerturbationTransform`, so it was the wild-type token for every strain: across-strain sd 0.0 against 0.973 for `z_S` (measured in the 019 expression strand; `notes-tex/019-simb-multimodal-expression/sections/0-strand.tex`). Every head that reads the CLS, `PerturbationHead`, `GlobalHead`, the `pert_cond` half of the per-gene arms, was conditioned on the strain only through its gene-side term, and the paper's Fig 1 spec, which writes `h_pert_CLS` as carrying a global summary of the perturbation, did not match the code.

`CellGraphTransformer(perturb_cls=True)` is the cheapest of the three designs listed in `3-next.tex` (perturb the CLS too; edit tokens before the encoder; typed perturbation tokens): the CLS is query row 0 of the same cross-attention over the same deleted-gene keys, `perturbation_indices + 1` because the keys are gathered from the query matrix. A query's output depends on that query and the keys only, so the gene rows are bit-identical to the flag-off model and no parameter is added (`tests/torchcell/models/test_equivariant_cell_graph_transformer.py::test_perturb_cls_moves_only_the_cls`). The model returns `h_CLS_pert` `[batch, d]`; `GlobalHead` reads it when present, and `GlobalHead(linear=True, use_gene_pool=False)` is then a linear probe of the cell state, `w . h_CLS_pert + b`. `perturbation_head_cls` selects which token the interaction head reads: `wildtype` (default, 010's input exactly) or `perturbed`.

What this does not change: the one-key degeneracy at `|S| = 1` (the CLS query sees the same single key), the pair-rank of the gene rows, and the cost beyond one query row. First use: the 025 fitness arms `cgt_s0_r_kl_fit_010` / `_011` ([[experiments.025-solid-growth.scripts.equivariant_cell_graph_transformer]]). The 019 evidence review behind the choice, operator by operator, is summarized there.
