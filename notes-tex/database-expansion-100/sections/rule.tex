...
are close, so it should be read as a budget rather than a verdict. One row was pinned into
the recommended set because it was named explicitly in the scoping request rather than
because it ranked there: Table~\ref{tab:swaps} names the pin and the row it displaced, so
the recommended set stays auditable.

\input{tables/swaps}

Displacement picks the weakest non-requested row by tier first and measurements second. The
tier term is load-bearing rather than decorative: ranking on measurements alone selected
Puddu 2019 for eviction, and that row is the whole-collection sequencing on which every
\term{S288C-KO} sequence basis in this table rests.
