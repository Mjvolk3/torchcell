...
family and is the assumption behind a log link. \textbf{d}, The two families of null as
statements about the system. A multiplicative null says each deletion keeps a fraction of
what reaches it, which is how independent steps of one flux compose and is the convention
the growth interaction screens use. An additive null says each deletion removes an amount
from a shared pool. The second counts the part of the first deletion's loss that the second
would have taken again, and that double-counted term is the entire difference between them.
\textbf{e}, The four models fit here: what each expects, the scale its residual is measured
on, what it is fit to, and the reading of the system under which it is the natural null.
Equations in the Methods.}{fig:model-intuition}
\end{figure*}
...
chemistry sits in metabolism, on the iPath3 reference map of KEGG's global metabolic
pathways. Colored are the yeast reactions of glycolysis and gluconeogenesis, pyruvate
metabolism, the citrate cycle, fatty acid biosynthesis, elongation and desaturation, and
fatty acid degradation, taken from KEGG's own gene lists for those pathways rather than
chosen here. The two measured species the reference map carries a node for are marked, as
are four named intermediates on the route; the other three measured species have no node
on that map. The map's own labels are not reproduced, because at this size they would
print below the minimum type size. \textbf{b}, The ten deleted transcription
