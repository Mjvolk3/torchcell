...
OBS = (
    0.109  # best expr_002 val/per_gene/pearson_per_gene (controlled _002, 1,161 split)
)
...
def main() -> None:
    ...
    f = osp.join(
        ...
        "data/deleteome/deleteome_all_mutants_ex_wt_var_controls.txt",
    )
    ...
    print(
        f"deleteome: genes={M.shape[0]} mutants={M.shape[1]} (M cols={len(m_cols)} p cols={len(p_cols)})"
    )
    ...
    print(
        f"  median ceiling: {np.median(c):.4f}   |  IQR [{np.quantile(c, 0.25):.3f}, {np.quantile(c, 0.75):.3f}]"
    )
    print(f"  reliability mean: {np.nanmean(rel[ok]):.4f}")
    for thr in (0.05, 0.10, 0.20, 0.30, 0.50):
        print(
            f"    genes with ceiling > {thr:.2f}: {(c > thr).sum():5d}  ({100 * (c > thr).mean():.1f}%)"
        )
