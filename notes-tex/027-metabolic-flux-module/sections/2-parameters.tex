...
default of 10.3\,s$^{-1}$. A catalytic unit here is a \textbf{(reaction, AND-term) pair},
not a distinct enzyme complex: \texttt{parse\_catalytic\_units} assigns an id inside the
per-reaction loop, so one complex catalyzing five reactions counts five times. The
deduplicated AND-term count is 1{,}065. Both numerator and denominator are counted the same
way, so 4.0 percent is internally consistent, but the denominator has to be named precisely
or the reader computes 148/1{,}065 and gets 13.9. $K_M$ had no table at all: the parameter existed as an enum
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{0.16\textwidth}p{0.30\textwidth}p{0.46\textwidth}@{}}
    ...
    $\Delta_f G'^{\circ}$ & \textbf{wired}, but the GEM's shipped table, not the
      eQuilibrator recomputation &
      the standard term of $\Delta_r G_j$, hence the second-law hinge and the dissipation
      bound. \texttt{load\_thermo\_table} reads
      \texttt{data/databases/model\_metDeltaG.csv}; the recomputed parquet with its
      covariance is read by nothing \\
...
\textbf{None of the three tables built in this work is consumed by any measured arm.} The
$k_{cat}$ predictions are not plumbed to \texttt{resolve\_kcat\_table}, the recomputed
formation energies are not plumbed to \texttt{load\_thermo\_table}, and $K_M$ has no
consumer at all. Every arm reported here ran on the Open Enzyme Database's 4.0 percent
measured $k_{cat}$ plus a default, and on yeast-GEM's shipped formation energies.

That is the sharpest thing to carry out of this document, because the module diagram draws
solid edges from the kcat and thermodynamic tables into the box and the potential, and those
edges do not exist in the code. The tables are an audit of what the inputs \emph{could} be
and a demonstration that they can be built reproducibly for any organism. They are not yet
inputs. Wiring them is the next change, and until it happens no result should be described
as testing predicted kinetics or recomputed thermodynamics.
