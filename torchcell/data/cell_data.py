...
def _process_gene_ontology(hetero_data, go_graph, node_idx_mapping):
    ...
    if term_to_genes_list:
        # Create base tensor with [go_idx, gene_idx, stratum, state] columns
        # The level column has been removed as it's redundant with stratum
        go_gene_strata_state = torch.zeros(
            (len(term_to_genes_list), 4), dtype=torch.float
        ).cpu()

        # Fill in values
        go_gene_strata_state[:, 0] = term_to_genes_tensor[:, 0].float()  # go_idx
        go_gene_strata_state[:, 1] = term_to_genes_tensor[:, 1].float()  # gene_idx

        # Get stratum for each GO term
        for i, (term_idx, _) in enumerate(term_to_genes_list):
            go_gene_strata_state[i, 2] = strata_tensor[term_idx].float()  # stratum

        # Set all genes to unperturbed state (1.0)
        go_gene_strata_state[:, 3] = 1.0  # state (unperturbed)

        # Store the unperturbed state tensor in the cell graph
        hetero_data["gene_ontology"].go_gene_strata_state = go_gene_strata_state
