# experiments/010-kuzmin-tmi/scripts/generate_triple_combinations_inference_3.py
# [[experiments.010-kuzmin-tmi.scripts.generate_triple_combinations_inference_3]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/010-kuzmin-tmi/scripts/generate_triple_combinations_inference_3.py
"""
Generate triple gene combinations for inference_3 with relaxed thresholds.

Key differences from inference_2:
- Singles filter: all > 0.90, max > 1.04 (relaxed from 1.10)
- Doubles filter: all > 0.90, max > 1.08 (fixed threshold, not gap-based)
- Uses JT test for validation (gap of 0.04 gives ~96% power at n=8, ~100% at n=16)

Scientific claim: "At least one triple shows significant monotonic fitness improvement"
Statistical test: Jonckheere-Terpstra test for ordered alternatives
"""
...
# =============================================================================
# INFERENCE 3 THRESHOLDS (Relaxed for JT test validation)
# =============================================================================
# The claim: "At least one gene/pair shows iterative improvement"
# We don't require ALL genes to exceed 1.0, just the best one to show the pattern
SMF_THRESHOLD = 1.04  # max(singles) must exceed this (relaxed from 1.10)
SMF_BASELINE = 0.90   # all(singles) must exceed this
DMF_THRESHOLD = 1.08  # max(doubles) must exceed this (= SMF_THRESHOLD + 0.04)
DMF_BASELINE = 0.90   # all(doubles) must exceed this
...
def load_smf_from_dataset(dataset):
    ...
    print(f"  SMF > {SMF_BASELINE}: {sum(1 for f in fitness_values if f > SMF_BASELINE)}")
...
def load_dmf_from_dataset(dataset):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="  Building DMF map"):
...
def build_adjacency_graph(smf_fitness, dmf_fitness):
    ...
    - f_ij > DMF_BASELINE (0.80)
    - f_i > SMF_BASELINE AND f_j > SMF_BASELINE (both singles viable)
    """
    print("\n" + "="*80)
...
def generate_triples_streaming(
    ...
    """
    Generate triples with INFERENCE 3 filtering criteria:
    - All 3 singles > 0.80, max > 1.04
    - All 3 doubles > 0.80, max > 1.08
    """
    print("\n" + "="*80)
    print("Generating triples with INFERENCE 3 thresholds...")
    print(f"  Singles: all > {SMF_BASELINE}, max > {SMF_THRESHOLD}")
    print(f"  Doubles: all > {DMF_BASELINE}, max > {DMF_THRESHOLD}")
    print("="*80)
    ...
    for g1 in tqdm(genes_sorted, desc="Processing genes"):
        ...
        for g2 in neighbors_g1_sorted:
            ...
            for g3 in mutual_neighbors:
                ...
                # Check singles: all > 0.80, max > 1.04
...
def create_visualizations(
    ...
    ax1.set_title('SMF Distribution (Costanzo2016)', fontsize=14)
    ...
    ax2.axvline(DMF_THRESHOLD, color='#B73C39', linestyle='--', linewidth=2,
                label=f'Threshold: {DMF_THRESHOLD}')
    ...
    above_threshold_dmf = sum(1 for f in dmf_values if f > DMF_THRESHOLD)
    ax2.text(0.97, 0.97,
             f'Total: {len(dmf_values):,}\n'
             f'> {DMF_THRESHOLD}: {above_threshold_dmf:,} ({100*above_threshold_dmf/len(dmf_values):.2f}%)',
    ...
    categories = [
        ('Kept (final triples)', kept_count, '#6B8D3A'),
        ('TMI exists (in training)', tmi_removed_count, '#775A9F'),
        ('Missing SMF data', failed_counts["missing_smf"], '#7191A9'),
        ('Missing DMF data', failed_counts["missing_dmf"], '#6D666F'),
        (f'Singles filter\n$\\mathit{{all>{SMF_BASELINE}}}$\n$\\mathit{{max>{SMF_THRESHOLD}}}$',
         failed_counts["singles_threshold"], '#D86E2F'),
        (f'Doubles filter\n$\\mathit{{all>{DMF_BASELINE}}}$\n$\\mathit{{max>{DMF_THRESHOLD}}}$',
         failed_counts["doubles_threshold"], '#B73C39'),
    ]
    ...
    fig.suptitle(
        f'Inference Dataset 3 Filtering (Relaxed Thresholds)\n'
        f'Singles(all>{SMF_BASELINE}, max>{SMF_THRESHOLD}) · '
        f'Doubles(all>{DMF_BASELINE}, max>{DMF_THRESHOLD})',
        fontsize=13, fontweight='bold', y=0.98
    )

    plt.tight_layout(rect=[0, 0, 1, 0.95])
    output_path = osp.join(ASSET_IMAGES_DIR, f"010-kuzmin-tmi/inference_3_filtering.png")
...
def replot_from_saved():
    ...
    same create_visualizations() used by the full pipeline. Skips the billion-candidate
    generate_triples_streaming() and the Neo4j TMI load.
    """
    ts = timestamp()
    results_dir = "experiments/010-kuzmin-tmi/results/inference_dataset_3"
...
def main():
    ts = timestamp()
    print(f"Starting triple generation for INFERENCE 3 at {ts}")
    print(f"Thresholds: SMF > {SMF_THRESHOLD}, DMF > {DMF_THRESHOLD}")
    print(f"Baselines: SMF > {SMF_BASELINE}, DMF > {DMF_BASELINE}")
    ...
    # Load SMF
    print("\n" + "="*80)
    print("Loading SMF dataset (Costanzo2016)")
    print("="*80)
    smf_dataset = SmfCostanzo2016Dataset(
        root=osp.join(DATA_ROOT, "data/torchcell/smf_costanzo2016")
    )
    ...
    with open(summary_file, 'w') as f:
        f.write(f"Inference Dataset 3 - Triple Generation Summary\n")
        f.write(f"Timestamp: {ts}\n\n")
        f.write(f"RELAXED THRESHOLDS (JT test validation):\n")
        f.write(f"  SMF baseline: > {SMF_BASELINE}\n")
        f.write(f"  SMF threshold: max > {SMF_THRESHOLD}\n")
        f.write(f"  DMF baseline: > {DMF_BASELINE}\n")
        f.write(f"  DMF threshold: max > {DMF_THRESHOLD}\n\n")
    ...
    # Final summary
    print("\n" + "="*80)
    print("INFERENCE DATASET 3 - TRIPLE GENERATION COMPLETE")
    print("="*80)
    print(f"Thresholds: SMF > {SMF_THRESHOLD}, DMF > {DMF_THRESHOLD}")
    print(f"Baselines: SMF > {SMF_BASELINE}, DMF > {DMF_BASELINE}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate inference-3 triples; --plot-only redraws the filtering plot."
    )
    parser.add_argument(
        "--plot-only",
        action="store_true",
