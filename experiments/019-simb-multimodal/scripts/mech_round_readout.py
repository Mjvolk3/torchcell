...
def fetch(api: wandb.Api) -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    for run in runs:
        ...
        if key in best:
            keep, drop = (
                (run, best[key])
                if final_epoch > best[key].summary["epoch"]
                else (best[key], run)
            )
            print(
                f"  drop {drop.id} {key} at epoch {drop.summary['epoch']}: superseded by "
...
def figure(
    ...
    fig, axes = plt.subplots(
        1,
        2,
        figsize=(mm_to_in(PANEL_WIDTHS_MM["full"]), mm_to_in(60)),
    ...
    ax.set_title(
        "mechanism round, both seeds; dotted = matched budget", fontsize=6, pad=3
    )
    panel_label(ax, "a")
    ...
    for s in (0, 1):
        ...
        ax.plot(
            [x[a] for a in ARMS if a in sub.index],
    ...
    ax.axhspan(
        ...
        lw=0,
        zorder=0,
        label=f"v9 long-budget arms, n={inc['n']}, {inc['budget_epochs']:,} ep",
    )
    ...
    ax.scatter(
        [], [], s=16, marker="o", facecolor="white", edgecolor="black", label="seed 0"
    )
    ax.scatter(
        [], [], s=16, marker="s", facecolor="white", edgecolor="black", label="seed 1"
    )
    ax.legend(loc="lower left", fontsize=5, borderpad=0.3)
...
def main() -> None:
    ...
    print(
        f"matched budget = {matched}; final epochs "
    ...
    with pd.option_context("display.width", 220):
        print(
            t.drop(columns=["rows_short_by"]).to_string(
    ...
    for b, arms in contrasts.items():
        for arm, c in arms.items():
            print(
                f"  epochs <= {int(b):>5}  {arm:18s} n={c['n_pairs']}  "
                f"mean {c['mean_diff']:+.4f}  "
