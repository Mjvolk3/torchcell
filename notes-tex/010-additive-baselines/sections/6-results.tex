...
agrees for every closed-form model to $3 \times 10^{-13}$, which is the
end-to-end check that the 025 labels and the pinned split are the 010 ones; B5
is a stochastic fit and its run here was on CPU where 010's was on GPU. Arm Q is
...
that exist: the three 010 checkpoints and the arm R replication. The hatched bar
is a validation maximum, not a test number, and the arm Q transformer slot
waits on job 1609.}{fig:ladder025}

\tcfigfit[0.42]{figures/additive_baselines_025_vs_010.pdf}{Arm R test Pearson
on the 025 build against the same model on the 010 build. Every closed-form
model lies on the identity line to numerical precision; the nonlinear baseline
sits 0.006 above it, inside its own three-seed spread.}{fig:parity025}
