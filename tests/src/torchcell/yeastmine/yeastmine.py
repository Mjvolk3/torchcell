...
def get_regulators(gene="YOR202W"):
    # Gene Regulation
    # Retrieve genes that are regulators of a given target gene. Gene(target) -> Gene(Regulators).
    # "YOR202W" == "HIS3"
...
def get_physical_interactions(gene="YFL039C"):
    # act1  == YFL039C
...
def get_gene_interactions(gene="YDR210W"):
    # Gene Interactions
...
def get_protein_abundance(gene="tfc3"):
...
def get_median_protein_abundance(gene="YAL001C"):
...
def get_protein_sequence(gene="rad54"):
...
def get_gene_sequence(gene="rad54"):
...
def get_protein_half_life(gene="act1"):
...
def get_chromosomal_location(gene="act1"):
    service = Service("https://yeastmine.yeastgenome.org/yeastmine/service")
...
def get_feature_type(gene="YIL080W"):
    service = Service("https://yeastmine.yeastgenome.org/yeastmine/service")
...
def get_gene_external_id(gene: str = "act1"):
    service = Service("https://yeastmine.yeastgenome.org/yeastmine/service")
    query = service.new_query("Gene")
    query.add_view(
        "primaryIdentifier",
...
def write_gene_id_translation_table():
...
def get_ym_attrs_list(gene: str):
...
def create_node_ym_attrs(gene_list: list):
