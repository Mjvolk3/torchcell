    ...
    def collect_data(self) -> Tuple[Dict[str, float], Dict[str, float]]:
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                print(f"IPMI command failed (attempt {attempt + 1}/{self.retry_count}): {result.stderr}")
            except subprocess.TimeoutExpired:
                print(f"IPMI command timed out after {self.timeout} seconds (attempt {attempt + 1}/{self.retry_count})")
            except subprocess.SubprocessError as e:
                print(f"IPMI command error (attempt {attempt + 1}/{self.retry_count}): {e}")
...
class SystemMonitor:
    ...
    def export_data(self) -> None:
        ...
        output_path = self.config.output_dir / f"{self.config.title}_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def plot_data(self) -> Optional[Path]:
        ...
        fig.suptitle(f'System Monitoring - {self.config.title}\n' + 
        ...
        for fan in ['CPU_FAN', 'CPU_OPT', 'CHA_FAN2', 'CHA_FAN3', 'CHA_FAN4', 'CHA_FAN6', 'SOC_FAN']:
            device_styles[fan] = {
                'color': colors[color_idx],
                'marker': markers[marker_idx]
            }
        ...
        for sensor in temp_df.columns:
            if any(not np.isnan(y) for y in temp_df[sensor].values):
                group_name = 'CPU' if 'CPU' in sensor else \
                        'Memory' if 'DIMM' in sensor else \
                        'PCIe' if 'PCIE' in sensor else \
                        'Network' if 'LAN' in sensor else 'Other'
                
                style = device_styles[sensor]
                line = ax1.plot(x_values, temp_df[sensor].values,
                            color=style['color'],
                            marker=style['marker'],
                            markersize=4,
        ...
        for fan in rpm_df.columns:
            if any(not np.isnan(y) for y in rpm_df[fan].values):
                group_name = 'CPU Fans' if 'CPU' in fan else \
                        'Chassis Fans' if 'CHA' in fan else 'Other'
                
                style = device_styles[fan]
                line = ax2.plot(x_values, rpm_df[fan].values,
                            color=style['color'],
                            marker=style['marker'],
                            markersize=4,
                            markevery=5,  # Show marker every 5 points to avoid crowding
        ...
        for i, completion_time in enumerate(self.benchmark_times, 1):
            ...
            for ax in [ax1, ax2]:
                ax.axvline(x=cumulative_time,
                        color='r', linestyle='--', alpha=0.3)
                ax.text(cumulative_time, ax.get_ylim()[1],
        ...
        ax1.legend(temp_handles, temp_labels,
                bbox_to_anchor=(1.02, 1),
                loc='upper left',
                title="Temperature Sensors")
        
        # Fan speeds legend
        ax2.legend(rpm_handles, rpm_labels,
                bbox_to_anchor=(1.02, 1),
                loc='upper left',
                title="Fan Speeds")
        
        # Customize plots
        ax1.set_title(f'Configuration: Matrix Size={self.config.matrix_size}, CPUs={self.config.num_cpus}')
        ...
        output_path = self.config.output_dir / f"{self.config.title}_plot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    def _plot_data_series(self, ax: plt.Axes, df: pd.DataFrame, groups: Dict[str, List[str]], 
                         colors: Dict[str, str], markers: List[str], legend_title: str) -> None:
        ...
        for run_num in range(1, self.current_run):
            ...
            x_values = [(t - run_start).total_seconds() for t, mask in zip(self.timestamps, run_mask) if mask]
                    ...
                    if sensor in df.columns:
                        ...
                        if any(not np.isnan(y) for y in y_values):
                            ax.plot(x_values, y_values,
                                  ...
                                  markersize=4,
                                  label=f"{group_name}: {sensor}" if run_num == 1 else "_nolegend_")
    ...
    def _add_completion_marker(self, ax: plt.Axes, completion_time: float) -> None:
        ...
        ax.text(completion_time, ax.get_ylim()[1],
...
class Benchmark:
    ...
    def setup_monitoring(self) -> None:
        ...
        monitor_thread = threading.Thread(
            target=self._run_monitor_on_last_cpu,
            args=(self.monitor,)
        )
    ...
    def run(self) -> List[float]:
        ...
        times = []
        seeds = [self.config.base_seed + i * self.config.num_cpus 
    ...
    def _run_single_benchmark(self, seed: int) -> float:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            max_workers=self.config.num_cpus,
...
def main() -> None:
    ...
    parser.add_argument('--matrix-size', type=int, default=4000,
