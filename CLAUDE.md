...
- **Palette — one ordered source of truth.** Colors come from
  ...
  `notes/assets/scripts/generate_color_palette.py`). It is a single ordered, green-free
  qualitative series of draw.io `(line, fill)` pairs matching Fig 1 — orange
  `#D79B00`/`#FFE6CC`, red `#B85450`/`#F8CECC`, purple `#9673A6`/`#E1D5E7`, yellow
  `#D6B656`/`#FFF2CC`, then blue `#6C8EBF`, gray `#666666`, then darker variants.
  **Assign in order**: the four primaries first, then blue/gray, then extended; blue/
  gray are not used until the four primaries are used up, and a simple N-category series
  takes the first N. Keep it green-free.
