...
desc: >-
  Handoff from the M1 Mac session (branch notes/pert-seq-method-review) to the
  gilahyper session: what notes-tex is, the two-tier Zotero bib policy, and the
  one analysis that must run on gilahyper.
...
notes-tex/
  ...
  024-perturb-seq-costing/ document #1: the perturb-seq method review + costing
```
...
Build: `cd notes-tex/024-perturb-seq-costing && make && make check`.
...
```bash
cd notes-tex/024-perturb-seq-costing
