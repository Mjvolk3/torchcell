...
def compare_hetero_data(reference, current, name="data", rtol=1e-5, atol=1e-8):
            ...
            if torch.is_tensor(ref_val):
                ...
                if ref_val.dtype in [torch.float, torch.float32, torch.float64]:
                    if not torch.allclose(ref_val, cur_val, rtol=rtol, atol=atol):
                        ...
                        diff_mask = ~torch.isclose(
                            ref_val, cur_val, rtol=rtol, atol=atol
                        )
                    ...
                    if not torch.equal(ref_val, cur_val):
                        ...
                        if ref_val.dim() == 1:
                            diff_indices = (ref_val != cur_val).nonzero(as_tuple=True)[
                                0
                            ][:10]
                            assert False, (
                                f"{name}['{node_type}']['{attr}']: Values differ\n"
                ...
                if attr in [
                    ...
                    if ref_val and isinstance(ref_val[0], list):
                        ...
                        for i, (ref_sublist, cur_sublist) in enumerate(
                            zip(ref_val, cur_val)
                        ):
                            assert len(ref_sublist) == len(cur_sublist) and set(
                                ref_sublist
                            ) == set(cur_sublist), (
    ...
    for edge_type in reference.edge_types:
        ...
        assert ref_attrs == cur_attrs, f"{name}[{edge_type}]: Attributes differ"
        ...
        if edge_index_key:
            ...
            for attr in ref_attrs:
                if attr != edge_index_key:
                    ...
                    if torch.is_tensor(ref_val) and torch.is_tensor(cur_val):
                        if (
                            ref_val.size(0) == num_edges
                            and cur_val.size(0) == num_edges
                        ):
            ...
            if edge_attr_keys:
                ...
                ref_edge_attr = torch.cat(
                    [ref_store[k].view(num_edges, -1) for k in edge_attr_keys], dim=1
                )
                ref_edge_index_sorted, ref_edge_attr_sorted = sort_edge_index(
                    ref_edge_index, edge_attr=ref_edge_attr, sort_by_row=True
                )
                ...
                for k in edge_attr_keys:
                    ...
                    ref_attrs_sorted[k] = ref_edge_attr_sorted[
                        :, start_idx : start_idx + num_features
                    ].view(orig_shape)
                    start_idx += num_features

                # Sort current edges and attributes
                cur_edge_attr = torch.cat(
                    [cur_store[k].view(num_edges, -1) for k in edge_attr_keys], dim=1
                )
                cur_edge_index_sorted, cur_edge_attr_sorted = sort_edge_index(
                    cur_edge_index, edge_attr=cur_edge_attr, sort_by_row=True
                )
                ...
                for k in edge_attr_keys:
                    ...
                    cur_attrs_sorted[k] = cur_edge_attr_sorted[
                        :, start_idx : start_idx + num_features
                    ].view(orig_shape)
                    start_idx += num_features
            else:
                # No edge attributes - just sort edge indices
                ref_edge_index_sorted = sort_edge_index(
                    ref_edge_index, sort_by_row=True
                )
                cur_edge_index_sorted = sort_edge_index(
                    cur_edge_index, sort_by_row=True
                )
                ref_attrs_sorted = {}
                cur_attrs_sorted = {}
            ...
            for attr in ref_attrs:
                if attr not in edge_attr_keys and attr != edge_index_key:
                    ...
                    cur_val = cur_store[attr]
                    if torch.is_tensor(ref_val) and torch.is_tensor(cur_val):
                        assert ref_val.shape == cur_val.shape and torch.equal(
                            ref_val, cur_val
...
def test_single_instance_equivalence(reference_data, current_data):
    ...
    compare_hetero_data(
        reference_data["single_instance"], dataset[0], "single_instance"
    )
...
def test_batch_equivalence(reference_data, current_data):
    ...
    compare_hetero_data(reference_data["batch"], batch, "batch")
