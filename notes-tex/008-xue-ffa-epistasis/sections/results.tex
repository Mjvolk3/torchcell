...
transcription factor deletions are additional to those three, so a triple deletion strain
carries six deletions in total. Six quantities were measured per strain: the titers of
myristate (C14:0), palmitate (C16:0), stearate (C18:0), palmitoleate (C16:1) and oleate
(C18:1), and their sum, referred to as total titer. Every titer below is divided by the
base strain's, so a value of 1 is no change and 2 is a doubling.
...
$0.52\times$. Combinations behave very differently: 29 of the 45 doubles and 51 of the 120
triples exceed the base strain, and the best strain in the design is a triple at
$2.05\times$ (Fig.~\ref{fig:inaccessible}a). The gap between what single deletions do and what
combinations do is the first sign that the effects are not composing.

\begin{figure*}[t]
%% The top row is one draw.io export carrying its own letters a and b
%% (perspective_motivation_drawio.py); the three measured panels under it are lettered
%% here, as c, d, e.
\panelrow{\includegraphics{figures/ffa-epistasis-fig1-motivation.pdf}}
\panelrow{%
  \panel{c}{figures/panel_interaction_distribution.pdf}\hfill
  \panel{d}{figures/panel_volcano_total_titer.pdf}\hfill
  \panel{e}{figures/panel_consensus_agreement.pdf}%
}
\caption{\textbf{Interaction is the rule on total fatty acid titer, and the third deletion
is where it bites.} \textbf{a}, Why interactions matter for a factory. A yeast in nature
is robust and cross-coupled, and most of its chemistry is off the curated map:
retrobiosynthesis with predicted enzyme annotation places 55,734 reactions beside the
4,131 of Yeast9, with median predicted Michaelis constants about twofold higher in that
underground network than in the known one~\citep{wuSystematicallyExploringYeast2026}. A
campaign deletes or overexpresses a few genes and keeps the strain that makes more of one
product; the coupling that made the cell robust is still there and is not measured.
\textbf{b}, An interaction is the departure of a measured combination from an expectation,
drawn for the multiplicative null with a negative departure, the common case on growth:
genome-wide, about 550,000 negative and 350,000 positive digenic
interactions~\citep{costanzoGlobalGeneticInteraction2016}, and an estimated hundred times
as many negative trigenic ones, about 25\%
weaker~\citep{kuzminSystematicAnalysisComplex2018}.
Predicted, an interaction is a step a design can climb; unpredicted, a campaign is left to
trial and error. \textbf{c}, Interaction scores against the multiplicative null for all
...
at three. \textbf{d}, Trigenic interaction score $\tau$ against
...
\textbf{e}, How many of the four epistasis models call each triple significant, split by
...
(Fig.~\ref{fig:landscape}c), so adding a third deletion does not damp the interaction, it
...
11 of those 86 depart upward (Fig.~\ref{fig:landscape}d).
...
one, two and three deletions (bars, counts above), with the best titer at each order (blue
...
The obstacle is a valley rather than a plateau. Every route has a worst intermediate, and
how far that intermediate falls below the base strain is the loss a campaign has to accept
to follow the route to its end. Each triple has six routes, so it has a best case and a
worst case. Taking each triple's shallowest route, the one whose worst intermediate is
highest, the median dips 8\% below the starting titer and the deepest such best case dips
22\%: no choice of deletion order avoids a loss. Taking the deepest route of each triple
instead, which is what a campaign that picks badly gets, the median dips 31\% and the
worst 48\%. A campaign built to discard anything worse than its parent discards the route
at the first step either way.
...
multiplicative expectation differ by $(1 - f_i)(1 - f_j)$, which is an identity rather
than a property of this pair. \textbf{b}, All 45 doubles against both expectations;
distance from the dashed line is the digenic interaction under that model. Forty-three of
the 45 sit above both, by a median $+0.40$ against the multiplicative expectation and
$+0.45$ against the additive one, so the two nulls agree on the direction at two deletions
and differ in magnitude. \textbf{c}, Replicate standard deviation of $f$ against $f$ over
the 165 strains with a nonzero replicate spread, on log axes. The fitted line has slope
1.10 in the logarithms; the two reference lines are the noise assumptions the models
divide over, a spread proportional to $f$, which is the Gamma family behind a log link,
and a constant spread. \textbf{d}, What each model expects of a double deletion, as level
sets over the square of single-deletion phenotypes, one panel per model in the order of
\textbf{f}. The square
runs to $1.5$ because single deletions in this design raise titer as well as lower it.
Each panel's contours are evenly spaced on the scale that model measures its residual on:
at $0.3$ to $1.5$ in steps of $0.3$ for the two linear-scale models, and a doubling apart
from $0.1$ to $1.6$ for the two log-scale ones, which is why those crowd near $1$ and open
out at low titer. Three of the four expect the same surface, and the additive null is the
one that does not: its level sets are straight where the others are hyperbolas. The two
surfaces differ by $(1 - f_i)(1 - f_j)$, which is $0.25$ where both singles halve the
titer; that point is marked in every panel with what the model expects there. \textbf{e}, The two families of
...
The readout matters as much as the scale. Thirty-one positive consensus interactions, over
29 distinct triples, clear all four models with the sign agreeing on one of the five
individual species, and 20 of the 31 are on C14:0. None survives on the summed total
...
Nothing is enriched. Across 28 graph-by-model tests, 5 reach an uncorrected $P < 0.05$ and
every one of the 5 is a depletion rather than an excess (Fig.~\ref{fig:models}b); none of
the 5 survives a Benjamini-Hochberg correction over the 28 tests. The largest excess anywhere
...
\begin{figure*}[t]
\panelrow{\panel{a}{figures/ffa_kegg_map.pdf}}
...
chemistry sits in metabolism: KEGG's global map of yeast metabolism, redrawn as vector
from the coordinates KEGG publishes for it, with the yeast genome-scale model overlaid.
Reactions and compounds the model contains are drawn in gray; what KEGG draws but the
model lacks is fainter. The map places 814 yeast genes, 745 of them in the model, and 889
of its 3,878 reaction lines; the model's transport, exchange and per-chain lipid reactions
have no place on any such map and are the part panel \textbf{b} carries. Colored are the
yeast reactions of glycolysis and gluconeogenesis, pyruvate metabolism, the citrate
cycle, fatty acid biosynthesis, elongation and desaturation, and fatty acid degradation,
taken from KEGG's own gene lists for those pathways rather than chosen here. A gene on more
than one list takes the first in the order central carbon, degradation, biosynthesis, which
is what keeps the degradation arm visible: KEGG lists POX1, the acyl-CoA oxidase this
chassis deleted, with the unsaturated fatty acids too. Filled blue
nodes are the four measured species the map draws; gray nodes are five named
intermediates on the route. Oleate has no node on the map, as the free acid or as
oleoyl-CoA, and is attached as an open ring along the shortest path of the same
genome-scale model from stearate, with the genes of that path under its name. Of the ten
deleted regulators, none is among the genes the map draws, which is the point of the
panel: the deletions act on metabolism from outside it. All thirteen pathway genes of
panel \textbf{b} are drawn. The map's own labels are not reproduced, because at this size
they would print below the minimum type size. \textbf{b}, The ten deleted transcription
...
locus, or to the 35 whose three factors were all deleted in training leaves $r$ between
0.12 and 0.15, no further from zero than the full set, and the top quartile by predicted $\tau$ holds
