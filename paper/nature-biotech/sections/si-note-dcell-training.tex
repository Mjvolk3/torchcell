...
points the runs (\supptab{tab:dcell-training-cost}). (d)~Samples per second at each stage of
the DCell speed-up work on a four-GPU workstation (batch 256 per GPU unless labeled), from
the progress-bar logs (elapsed time divided by steps completed, an upper bound on the
steady-state step time); labels give seconds per optimizer step: 128~s for the first working
implementation, 82~s after removing a duplicated forward pass, 23~s after caching the GO
strata tensors, 20~s with bf16 mixed precision, 99--119~s for two reruns of the bf16
configuration on later days, and 49--59~s with \texttt{torch.compile} (recompilation limit
64) at batch 500--600, 40 versus 50 samples per second uncompiled. A rewrite that spreads a
