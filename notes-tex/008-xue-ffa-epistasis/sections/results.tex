...
family and is the assumption behind a log link. \textbf{d}, What each model expects of a
double deletion, as level sets over the square of single-deletion phenotypes, one panel
per model in the order of \textbf{f}. Each panel's contours are evenly spaced on the scale
that model measures its residual on: at $0.2$ to $1.0$ in steps of $0.2$ for the two
linear-scale models, and over the same span in five equal steps of the logarithm for the
two log-scale ones, which is why those crowd near $1$ and open out at low titer. Three of
the four expect the same surface, and the additive null is the one that does not: its
level sets are straight where the others are hyperbolas. The two surfaces differ by
$(1 - f_i)(1 - f_j)$, which is $0.25$ at the center of the square, marked in every panel
with what that model expects there. \textbf{e}, The two families of
