...
what choosing one commits to.} \textbf{a}, Why the null matters for a factory. A yeast in
nature is robust and cross-coupled, and most of its chemistry is off the curated map:
...
trial and error. \textbf{c}, One measured double deletion on a titer axis, MED4 and RPD3,
the pair whose two expectations are furthest apart. The additive and the multiplicative
expectation differ by $(1 - f_i)(1 - f_j)$, which is an identity rather than a property of
this pair. \textbf{d}, All 45 doubles against both expectations; distance from the dashed
line is the digenic interaction under that model. Forty-three of the 45 sit above both, by
a median $+0.40$ against the multiplicative expectation and $+0.45$ against the additive
one, so the two nulls agree on the direction at two deletions and differ in magnitude.
\textbf{e}, Replicate standard deviation of $f$ against $f$ over the 165 strains with a
nonzero replicate spread, on log axes. The fitted line has slope 1.10 in the logarithms;
the two reference lines are the noise assumptions the models divide over, a spread
proportional to $f$, which is the Gamma family behind a log link, and a constant spread.
\textbf{f}, What each model expects of a double deletion, as level sets over the square of
single-deletion phenotypes, one panel per model in the order of \textbf{h}. The square
...
marked in every panel with what that model expects there. \textbf{g}, The two families of
...
second would have taken again. \textbf{h}, The four models fit here: what each expects, the
...
deletions are severe (Fig.~\ref{fig:model-intuition}c,f).
