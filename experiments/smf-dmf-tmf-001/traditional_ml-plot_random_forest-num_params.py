...
def process_raw_dataframe(df: pd.DataFrame, config_columns: list, summary_columns: list) -> pd.DataFrame:
    ...
    # Combine config and summary DataFrames
...
def create_plots(df: pd.DataFrame, max_size: int):
    ...
    for metric in metrics:
        ...
        ax.scatter(df["num_params"], df[f"val_{metric}"], label="Validation", alpha=0.7)
        ax.scatter(df["num_params"], df[f"test_{metric}"], label="Test", alpha=0.7)
        ...
        title = f"Random Forest {max_size_str} - {metric.upper()} vs. Number of Parameters"
...
def main():
    ...
    for df, max_size in zip(dataframes, [1000, 10000, 100000]):
        if not df.empty:
            ...
            print(f"Number of rows with missing num_params for max_size {max_size}: {len(df) - len(processed_df)}")
            processed_df.to_csv(osp.join(RESULTS_DIR, f"random_forest_processed_df_{max_size}.csv"), index=False)
