...
- [ ] Content-addressed interning of constant sub-objects (Environment/reference/publication) into a named LMDB sub-DB so the ~7.9 KB component-Media isn't denormalized per-record (ballooned dmi_costanzo2016 45→159 GB); centralize read-resolve in ExperimentDataset.get_single_item, wire the 4 SGA loaders, verify on smf_baryshnikova2010, then re-rebuild the SGA set on the lean encoding #high [[plan.experiment-dataset-interning.2026.07.15]]
