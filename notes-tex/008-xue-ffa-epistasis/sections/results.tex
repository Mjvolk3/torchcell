...
pathways rather than chosen here. Marked are the two measured species the reference map
places a node for and four named intermediates on the route. The other three measured
species are absent from that map's layout rather than from KEGG: all five have compound
entries, and the global map draws only some of the fatty acids. Panel \textbf{b} has all
five because its species come from the yeast genome-scale model, which resolves acyl
species by compartment. The map's own labels are not reproduced, because at this size they
would print below the minimum type size. \textbf{b}, The ten deleted transcription
