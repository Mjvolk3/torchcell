...
\begin{enumerate}
\item \textbf{The graph-prior probe is DONE, and it came back at chance}
  (\cref{sec:graph-probe}). All nine graphs land between $0.4961$ and $0.5057$ AUC, at most
  $+0.0046$ over a degree-preserving control. \textbf{Free the nine masked heads}; they are
  constrained toward a target that does not predict which reporters respond. The one repair
  with evidence is to stop symmetrizing the two directed relations: TF $\to$ target reaches
  $0.5508$ on tflink and $0.5239$ on regulatory interaction, and
  \file{_build_attention_mask} currently averages that away against its uninformative
  reverse. That is a change to the mask builder, not to the model.
