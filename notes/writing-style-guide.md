...
## 2026.09.15 - The generating script is a comment beside the caption, not a sentence in it

A figure caption no longer ends with "Generated by `experiments/.../foo.py`." The
pointer stays in the `.tex`, on the line directly after the caption's closing
brace, as a `%%` comment:

```latex
\tcfigfit[1.0]{figures/pilot_options.pdf}{%
  \textbf{...takeaway...} \textbf{(a)} ... \textbf{(d)} ...}{fig:pilots}
%% Generated by experiments/024-perturb-seq-costing/scripts/pilot_options.py
```

The reader of the PDF gets a caption that is only about the figure; whoever opens
the source to regenerate it finds the script one line below. The same goes for a
draw.io figure's `Source notes/assets/drawio/...` line. Where a script lives on a
branch that has not landed, name the branch and PR in the comment.

The `%% SOURCE:` header above the figure block is unchanged and still what
`make check` counts; the new comment is the human-facing pointer and sits where a
person editing the caption will see it.

**One sentence per panel.** `\textbf{(g, h, i)} What the agreement tracks. It
rises with...` compresses three panels into one clause and leaves the reader to
work out which panel carries which claim. Each lettered panel gets its own
sentence, carrying the statistic that panel actually shows.
