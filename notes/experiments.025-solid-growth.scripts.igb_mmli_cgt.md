...
## 2026.09.18 - Seed 1 restarted so all three seeds log singles

The user chose to give up the running seed 1 (2408888, 7 h 38 min, epoch 4) rather than carry one seed without per-order single-gene fitness. It was cancelled and resubmitted from worktree `-h` (2e96f24f) as 2409033, which started at once on compute-5-7 since no other job held the mmli GPUs. The four synced runs of 2408888 (W&B group `s3_fit_031`, rank 0 `kj03xx8y`) are an abandoned partial and are not seed 1 of the triplicate. Chain: 2409033 (S3 seed 1), 2408607 (random-vector seed 2), 2409031 (S3 seed 2), 2409032 (S3 seed 3). The pool definition is unchanged: every single-gene record in the build (5,694), the doubles whose gene pair lies inside some triple (739,219), and every triple (376,732); the closure rule applies to the doubles only. Train logs fitness for orders 1, 2 and 3 and interaction for orders 2 and 3; validation and test hold the pinned 010 triples only.

## 2026.09.18 - Disjoint single-region arms to three seeds

CaLM alone, ProtT5 alone and flanks alone each stood at seed 42 only. Queued from worktree `-i` (cfc03dc7) behind the last S3 seed, each arm reaching two seeds before any reaches three: 2409034 CaLM seed 1, 2409035 ProtT5 seed 1, 2409036 flanks seed 1, 2409037 CaLM seed 2, 2409038 ProtT5 seed 2, 2409039 flanks seed 2. The flanks replicates run `cgt_s0_q_kl_fudt_032`, the 100-epoch `_026` with the ceiling at 30, since the readout scores the epoch 10 to 29 window and reads completeness against the config budget. About 9 hours each on four A100s; the whole mmli chain (three S3 seeds at about 3.6 days each, random-vector seed 2, then these six) runs about two weeks.
