        ...
        for j, name2 in enumerate(network_names):
            if i < j:  # Only compute for unique pairs
                ...
                comparisons[(name1, name2)] = {
                    ...
                    "overlap_coefficient1": len(intersection) / len(set1) if set1 else 0,
                    "overlap_coefficient2": len(intersection) / len(set2) if set2 else 0,
                }
...
def visualize_network_sizes(networks, ASSET_IMAGES_DIR):
    """Create a bar chart comparing network sizes"""
    plt.figure(figsize=(15, 8))
    
    networks_data = [(name, G.number_of_nodes(), G.number_of_edges()) 
    ...
    ax.bar(x - width/2, nodes, width, label='Nodes')
    ax.bar(x + width/2, edges, width, label='Edges')
    
    ax.set_title('Network Sizes Comparison', fontsize=16)
    ...
    for i, name1 in enumerate(network_names):
        for j, name2 in enumerate(network_names):
            if i == j:
                ...
                similarity_matrix[i, j] = comparisons[(name1, name2)]["jaccard_similarity"]
            elif (name2, name1) in comparisons:
                similarity_matrix[i, j] = comparisons[(name2, name1)]["jaccard_similarity"]
...
def visualize_overlap_matrix(comparisons, ASSET_IMAGES_DIR):
    ...
    plt.figure(figsize=(14, 12))
    sns.heatmap(shared_edges_matrix, annot=True, fmt=".0f", cmap="YlGnBu",
                xticklabels=network_names, yticklabels=network_names)
    ...
    plt.figure(figsize=(14, 12))
    mask = np.triu(np.ones_like(overlap_coef_matrix, dtype=bool))  # Mask for the upper triangle
    sns.heatmap(overlap_coef_matrix, annot=True, fmt=".3f", cmap="YlOrRd", mask=mask,
                xticklabels=network_names, yticklabels=network_names)
...
def create_network_overlap_charts(edge_sets, ASSET_IMAGES_DIR):
    ...
    for key_network in key_networks:
        ...
        for name, edges in edge_sets.items():
            if name != key_network and name in edge_sets:
                ...
                comparison_data.append({
                    ...
                    "jaccard": len(intersection) / len(edge_sets[key_network].union(edges)) if edges else 0
                })
        ...
        plt.bar(networks, unique_other, bottom=[unique_key[i] + shared[i] for i in range(len(shared))], 
                label='Unique to other network')
        
        plt.title(f'Edge Overlap Between {key_network} Network and Other Networks', fontsize=16)
...
def visualize_tf_regulatory_comparison(networks, ASSET_IMAGES_DIR):
    ...
    # Get the networks
    tflink = networks["TFLink"]
    regulatory = networks["Regulatory"]
    ...
    for tf in tflink_tfs.union(regulatory_tfs):
        tflink_targets = list(v for _, v in tflink.out_edges(tf)) if tf in tflink else []
        reg_targets = list(v for _, v in regulatory.out_edges(tf)) if tf in regulatory else []
    ...
    tf_df['Total_targets'] = tf_df['TFLink_targets'] + tf_df['Regulatory_targets'] - tf_df['Common_targets']
    ...
    plt.bar(indices - bar_width, tf_df['TFLink_targets'], bar_width, 
            label='TFLink Targets', color='red', alpha=0.7)
    plt.bar(indices, tf_df['Common_targets'], bar_width, 
            label='Common Targets', color='purple', alpha=0.7)
    plt.bar(indices + bar_width, tf_df['Regulatory_targets'], bar_width, 
            label='Regulatory Targets', color='blue', alpha=0.7)
    ...
    if common_tfs:
        ...
        for tf in common_tfs:
            ...
            jaccard = overlap / len(tflink_targets.union(reg_targets)) if tflink_targets or reg_targets else 0
        ...
        plt.bar(x - width, [tf['TFLink_targets'] for tf in top_tfs], width, 
                label='TFLink Targets', color='red', alpha=0.7)
        plt.bar(x, [tf['Overlap'] for tf in top_tfs], width, 
                label='Common Targets', color='purple', alpha=0.7)
        plt.bar(x + width, [tf['Regulatory_targets'] for tf in top_tfs], width, 
                label='Regulatory Targets', color='blue', alpha=0.7)
        
        plt.title('TFs with Highest Target Overlap Between TFLink and Regulatory Networks', fontsize=16)
        plt.xlabel('Transcription Factor', fontsize=14)
        plt.ylabel('Number of Target Genes', fontsize=14)
        ...
        for i, tf in enumerate(top_tfs):
            plt.text(i, max(tf['TFLink_targets'], tf['Regulatory_targets']) + 5, 
                     f"J={tf['Jaccard']:.2f}", ha='center')
