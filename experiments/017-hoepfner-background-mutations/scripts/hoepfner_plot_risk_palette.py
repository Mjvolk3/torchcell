...
CLUSTER_LABEL = {
    "CL1": "Cluster 1 · chr XI aneuploidy",
    "CL2": "Cluster 2 · chr XI aneuploidy",
    "CL3": "Cluster 3 · WHI2 nonsense",
    "CL4": "Cluster 4 · chr V amplification",
}
ROMAN = [
    "I",
    "II",
...
def fig01_scores(samples):
    if samples is None:
        ...
        return
    fig, ax = plt.subplots(
        figsize=(mm_to_in(PANEL_WIDTHS_MM["half_plus"]), mm_to_in(56))
    )
...
def fig03_genome(summary, hip, hop):
    ...
    for ci, sub in un.groupby("chrom_idx"):
        ax.scatter(
            ...
            sub["enr"],
            s=2.2,
            color=STEEL if int(ci) % 2 else DENIM,
            alpha=0.6,
            rasterized=True,
            linewidths=0,
        )
    ...
    ax.scatter(
        ...
        edgecolor=BLACK,
        linewidth=0.15,
        label=f"flagged batch artifact (n={len(fl)})",
        rasterized=True,
    )
    ...
    ax.set_xlabel(
        "chromosomal position of deleted ORF (chromosome = construction-lab region)"
    )
...
def fig04_whi2(hip):
    ...
    ax.axvline(
        0.5, color=FLAG_C, ls="--", lw=1.0, label=f"cluster-3 cut (≥0.5, n={n_c3})"
    )
...
def fig05_specificity(hip, hop):
    ...
    fig, ax = plt.subplots(
        figsize=(mm_to_in(PANEL_WIDTHS_MM["half_plus"]), mm_to_in(82))
    )
    nf = m[~m["flagged"]]
    fl = m[m["flagged"]]
    ax.scatter(
...
def fig06_tiers(hip, flagged, s5):
    ...
    fig, ax = plt.subplots(
        figsize=(mm_to_in(PANEL_WIDTHS_MM["half_plus"]), mm_to_in(62))
    )
    xs = range(len(tiers))
    pct = [t[2] / total * 100 for t in tiers]
    ax.bar(
        xs, pct, color=[t[3] for t in tiers], edgecolor=BLACK, linewidth=0.4, width=0.72
    )
...
def fig07_composition(summary, s5):
    ...
    ax.pie(
        [flag_r, hip_clean, hop_r],
        colors=[FLAG_C, HIP_C, HOP_C],
        labels=[
            f"HIP flagged\n{flag_r / tot * 100:.1f}%",
...
def fig08_recovery(summary, hip, s5):
    aff = s5.merge(
        hip[["orf", "cum_pos", "enr"]], on="orf", how="left", suffixes=("", "_h")
    )
    ...
    for cl in ("CL1", "CL2", "CL3", "CL4"):
        ...
        ax.scatter(
            ...
            edgecolor=BLACK,
            linewidth=0.2,
            label=f"{CLUSTER_LABEL[cl]} (n={len(sub)})",
        )
    ...
    ax.scatter(
        ...
        edgecolor=BLACK,
        linewidth=0.6,
        label=f"recovered (n={len(caught)})",
    )
    ax.set_yscale("symlog", linthresh=1)
