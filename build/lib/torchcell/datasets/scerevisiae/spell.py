...
ASSET_IMAGES_DIR = osp.expanduser(
    osp.join("~", "Documents", "projects", "torchcell", "notes", "assets", "images")
)
...
def plot_expression_histograms(
    ...
    if not available_genes:
        print("None of the specified genes found in dataset")
        return
    ...
    for idx, gene in enumerate(available_genes):
        ...
        # Plot histogram
        axes[idx].hist(
            expr_values, bins=30, edgecolor="black", alpha=0.7, color="steelblue"
        )
        axes[idx].set_xlabel("Log2 Expression Ratio")
        axes[idx].set_ylabel("Frequency")
        axes[idx].set_title(
            f"{gene} ({df.loc[gene, 'NAME']}) - {len(expr_values)} conditions"
        )
        axes[idx].axvline(
            x=0, color="red", linestyle="--", alpha=0.5, label="No change"
        )
        axes[idx].legend()
...
def plot_global_expression_distribution(
    ...
    ax.set_title(
        f"{title_prefix} - Global Expression Distribution\n"
...
def extract_and_load_all_spell_studies(
    ...
    if studies_to_load:
        zip_files = [
            z for z in zip_files if any(study in z for study in studies_to_load)
        ]
...
def plot_genes_across_all_studies(
    ...
    for idx, gene in enumerate(gene_list):
        ...
        # Plot histogram
        axes[idx].hist(
            values, bins=100, edgecolor="black", alpha=0.7, color="steelblue"
        )
        axes[idx].set_xlabel("Log2 Expression Ratio")
        axes[idx].set_ylabel("Frequency")
        ...
        axes[idx].set_title(
            f"{gene} ({gene_name}) - {len(values):,} measurements "
            f"across {total_studies} studies"
        )
        axes[idx].axvline(
            x=0, color="red", linestyle="--", alpha=0.5, linewidth=2, label="No change"
        )
        axes[idx].grid(axis="y", alpha=0.3)
    ...
    plt.suptitle(
        f"SPELL Database - Gene Expression Across Multiple Studies\n"
...
def extract_time_info(condition_name: str) -> dict[str, Any]:
    ...
    time_patterns = [
        ...
        (
            r"(\d+\.?\d*)\s*(?:hr|hours?)",
            60,
        ),  # hours to minutes (non-capturing group for alternation)
    ...
    result["is_timeseries"] = any(
        kw in condition_name.lower() for kw in timeseries_keywords
    )
...
def extract_chemical_info(condition_name: str) -> dict[str, Any]:
    ...
    conc_patterns = [(r"(\d+\.?\d*)\s*(mM|µM|uM|nM|M|mg/ml|ug/ml|%)", r"\2")]
...
def extract_replicate_info(condition_name: str) -> dict[str, Any]:
    ...
    if "biological" in condition_name.lower() or "bio rep" in condition_name.lower():
        ...
        result["replicate_type"] = (
            "unknown"  # Has replicate number but type not specified
        )
...
def export_condition_metadata(
    ...
    if output_path is None:
        output_path = osp.join(
            DATA_ROOT, "data/sgd/spell", "spell_conditions_metadata_enhanced.csv"
        )
    ...
    total_conditions = sum(
        metadata["n_conditions"] for _, (_, metadata) in all_data.items()
    )
    processed = 0
    ...
    for (study_name, dataset_name), (df, metadata) in all_data.items():
        for idx, condition in enumerate(metadata["conditions"]):
            ...
            if time_info["time_min"] is not None:
                extracted_fields += 1
            if time_info["is_timeseries"]:
                extracted_fields += 1
            if temp_info["temperature_c"] is not None:
                extracted_fields += 1
            if chem_info["chemical_name"] is not None:
                extracted_fields += 1
            if chem_info["concentration"] is not None:
                extracted_fields += 1
            if nutrient_info["carbon_source"] is not None:
                extracted_fields += 1
            if nutrient_info["nitrogen_source"] is not None:
                extracted_fields += 1
            if nutrient_info["limitation_type"] is not None:
                extracted_fields += 1
            if physical_info["ph"] is not None:
                extracted_fields += 1
            if physical_info["oxygen_level"] is not None:
                extracted_fields += 1
            if stress_info["stress_type"] is not None:
                extracted_fields += 1
            if cell_cycle_info["cell_cycle_phase"] is not None:
                extracted_fields += 1
            if cell_cycle_info["synchronization_method"] is not None:
                extracted_fields += 1
            if replicate_info["replicate_number"] is not None:
                extracted_fields += 1

            # Confidence: higher when more fields extracted
            # 0 fields = 0.1, 5 fields = 0.5, 10+ fields = 0.9+
            extraction_confidence = min(
                0.9, 0.1 + (extracted_fields / total_attempted) * 0.8
            )
            ...
            needs_manual_review = (
                ...
                or "uncategorized" in matched_categories
                or (
                    chem_info["concentration"] is not None
                    and chem_info["chemical_name"] is None
                )
            )
            ...
            condition_records.append(
                {
                    ...
                    "secondary_categories": "|".join(matched_categories[1:])
    ...
    print(
        f"  Time information:         {df_conditions['time_min'].notna().sum():6,} conditions ({100 * df_conditions['time_min'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Time series flags:        {df_conditions['is_timeseries'].sum():6,} conditions ({100 * df_conditions['is_timeseries'].sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Temperature:              {df_conditions['temperature_c'].notna().sum():6,} conditions ({100 * df_conditions['temperature_c'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Chemical compounds:       {df_conditions['chemical_name'].notna().sum():6,} conditions ({100 * df_conditions['chemical_name'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Concentrations:           {df_conditions['concentration'].notna().sum():6,} conditions ({100 * df_conditions['concentration'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Carbon sources:           {df_conditions['carbon_source'].notna().sum():6,} conditions ({100 * df_conditions['carbon_source'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Nitrogen sources:         {df_conditions['nitrogen_source'].notna().sum():6,} conditions ({100 * df_conditions['nitrogen_source'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Nutrient limitations:     {df_conditions['limitation_type'].notna().sum():6,} conditions ({100 * df_conditions['limitation_type'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  pH values:                {df_conditions['ph'].notna().sum():6,} conditions ({100 * df_conditions['ph'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Oxygen levels:            {df_conditions['oxygen_level'].notna().sum():6,} conditions ({100 * df_conditions['oxygen_level'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Stress types:             {df_conditions['stress_type'].notna().sum():6,} conditions ({100 * df_conditions['stress_type'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Cell cycle phases:        {df_conditions['cell_cycle_phase'].notna().sum():6,} conditions ({100 * df_conditions['cell_cycle_phase'].notna().sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Replicate numbers:        {df_conditions['replicate_number'].notna().sum():6,} conditions ({100 * df_conditions['replicate_number'].notna().sum() / len(df_conditions):5.1f}%)"
    )

    # Confidence and review statistics
    print("\nExtraction Quality:")
    print(
        f"  Mean confidence:          {df_conditions['extraction_confidence'].mean():.3f}"
    )
    print(
        f"  Median confidence:        {df_conditions['extraction_confidence'].median():.3f}"
    )
    print(
        f"  High confidence (>0.5):   {(df_conditions['extraction_confidence'] > 0.5).sum():6,} conditions ({100 * (df_conditions['extraction_confidence'] > 0.5).sum() / len(df_conditions):5.1f}%)"
    )
    print(
        f"  Needs manual review:      {df_conditions['needs_manual_review'].sum():6,} conditions ({100 * df_conditions['needs_manual_review'].sum() / len(df_conditions):5.1f}%)"
    )
...
def check_condition_metadata_quality(
    ...
    stats: dict[str, Any] = {
        ...
        "examples": {"empty": [], "generic": [], "numeric": [], "descriptive": []},
    }
        ...
        for cond in metadata["conditions"]:
            ...
            if not cond or cond.strip() == "":
                ...
                if len(stats["examples"]["empty"]) < 5:
                    stats["examples"]["empty"].append(
                        f"{study_name}/{dataset_name}: '{cond}'"
                    )

            # Check for generic labels
            elif (
                any(pattern in cond.lower() for pattern in generic_patterns)
                and len(cond.split()) <= 2
            ):
    ...
    print(
        f"  ✓ Descriptive labels: {stats['descriptive_conditions']:,} "
        f"({100 * stats['descriptive_conditions'] / stats['total_conditions']:.1f}%)"
    )
    print(
        f"  ⚠ Generic labels: {stats['generic_conditions']:,} "
        f"({100 * stats['generic_conditions'] / stats['total_conditions']:.1f}%)"
    )
    print(
        f"  ⚠ Numeric-only labels: {stats['numeric_only_conditions']:,} "
        f"({100 * stats['numeric_only_conditions'] / stats['total_conditions']:.1f}%)"
    )
    print(
        f"  ✗ Empty labels: {stats['empty_conditions']:,} "
