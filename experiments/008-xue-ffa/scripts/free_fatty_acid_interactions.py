...
# [[experiments.008-xue-ffa.scripts.free_fatty_acid_interactions]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/008-xue-ffa/scripts/free_fatty_acid_interactions
# Test file: experiments/008-xue-ffa/scripts/test_free_fatty_acid_interactions.py
...
def compute_interactions_with_error_propagation(normalized_df, normalized_replicates, abbreviations=None):
    """
    Compute TF interactions with p-values using error propagation from replicate standard errors.
    ...
    Implements corrected multiplicative model formulas:
    ε^M_ij = f_ij - f_i*f_j
    τ^M_ijk = f_ijk - f_ij*f_k - f_ik*f_j - f_jk*f_i + 2*f_i*f_j*f_k
    """
    ...
    # Compute digenic interactions with error propagation
    print("Computing digenic interactions with error propagation...")
    for (gene1, gene2), f_ij in tqdm(double_mutants.items(), desc="Digenic interactions"):
        if gene1 in single_mutants and gene2 in single_mutants:
            ...
            # ε^M_ij = f_ij - f_i * f_j (multiplicative model)
            epsilon = f_ij - (f_i * f_j)
            digenic_interactions[(gene1, gene2)] = epsilon

            # SD propagation for ε^M_ij (delta method)
            # SD(ε^M_ij) = sqrt(SD(f_ij)^2 + (f_j * SD(f_i))^2 + (f_i * SD(f_j))^2)
            sd_epsilon = np.sqrt(
                sd_ij**2 +
                (f_j * sd_i)**2 +
                (f_i * sd_j)**2
            )
            digenic_sd[(gene1, gene2)] = sd_epsilon

            # SE propagation for ε^M_ij (delta method)
            # SE(ε^M_ij) = sqrt(SE(f_ij)^2 + (f_j * SE(f_i))^2 + (f_i * SE(f_j))^2)
            se_epsilon = np.sqrt(
                se_ij**2 +
                (f_j * se_i)**2 +
                (f_i * se_j)**2
            )
            digenic_se[(gene1, gene2)] = se_epsilon
    ...
    for (gene1, gene2, gene3), f_ijk in tqdm(triple_mutants.items(), desc="Trigenic interactions"):
        ...
        # τ^M_ijk = f_ijk - f_ij*f_k - f_ik*f_j - f_jk*f_i + 2*f_i*f_j*f_k (simplified form)
        tau = f_ijk - f_ij*f_k - f_ik*f_j - f_jk*f_i + 2*f_i*f_j*f_k
        trigenic_interactions[(gene1, gene2, gene3)] = tau

        # SD propagation for τ^M_ijk (delta method)
        # According to the formula provided:
        # SD(τ^M_ijk) = sqrt(σ²_ijk + (f_k × σ_ij)² + (f_j × σ_ik)² + (f_i × σ_jk)² +
        #                    (-f_jk + 2*f_j*f_k)² × σ²_i + (-f_ik + 2*f_i*f_k)² × σ²_j +
        #                    (-f_ij + 2*f_i*f_j)² × σ²_k)
        sd_tau = np.sqrt(
            sd_ijk**2 +
            (f_k * sd_ij)**2 +
            (f_j * sd_ik)**2 +
            (f_i * sd_jk)**2 +
            ((-f_jk + 2*f_j*f_k) * sd_i)**2 +
            ((-f_ik + 2*f_i*f_k) * sd_j)**2 +
            ((-f_ij + 2*f_i*f_j) * sd_k)**2
        )
        trigenic_sd[(gene1, gene2, gene3)] = sd_tau

        # SE propagation for τ^M_ijk (delta method)
        # Same structure as SD but with SE values
        se_tau = np.sqrt(
            se_ijk**2 +
            (f_k * se_ij)**2 +
            (f_j * se_ik)**2 +
            (f_i * se_jk)**2 +
            ((-f_jk + 2*f_j*f_k) * se_i)**2 +
            ((-f_ik + 2*f_i*f_k) * se_j)**2 +
            ((-f_ij + 2*f_i*f_j) * se_k)**2
        )
        trigenic_se[(gene1, gene2, gene3)] = se_tau
...
def plot_interaction_distributions_and_volcano(digenic_dict, digenic_se, digenic_pvalues,
    ...
    """Create combined distribution and volcano plots for interactions by FFA identity."""
    ...
    for idx, ffa in enumerate(ffa_types):
            ...
            if all_values:
                ...
                if digenic_values:
                    ax_dist.hist(digenic_values, bins=bins, alpha=0.7, color=digenic_color,
                           label=f'Digenic ($\\varepsilon$) (n={len(digenic_values)})', density=True,
                           edgecolor='black', linewidth=0.5)

                if trigenic_values:
                    ax_dist.hist(trigenic_values, bins=bins, alpha=0.7, color=trigenic_color,
                           label=f'Trigenic ($\\tau$) (n={len(trigenic_values)})', density=True,
                           edgecolor='black', linewidth=0.5)
        ...
        if digenic_values and digenic_pvals:
            ...
            if valid_di:
                ...
                ax_volc.scatter(di_vals, [-np.log10(p) for p in di_pvals],
                              alpha=0.6, s=20, label=r'Digenic ($\varepsilon$)', color=digenic_color)
        ...
        if trigenic_values and trigenic_pvals:
            ...
            if valid_tri:
                ...
                ax_volc.scatter(tri_vals, [-np.log10(p) for p in tri_pvals],
                              alpha=0.6, s=20, label=r'Trigenic ($\tau$)', color=trigenic_color)
        ...
        ax_volc.set_xlabel(r'Interaction Score ($\varepsilon$ or $\tau$)', fontsize=14)
        if idx == 0:
    ...
    plt.suptitle('TF Interaction Distributions and Volcano Plots by FFA Type', fontsize=16)
    plt.tight_layout()
...
def plot_significance_summary(combined_df, columns):
    """Create plots showing statistical significance summary."""
    ...
    ax2.set_title('Significant Interactions by Type')
    ...
    ax3.set_title('Q-Q Plot for P-values')
    ax3.legend()
    ax3.grid(True, alpha=0.3)

    plt.suptitle('TF Interaction Statistical Significance Summary', fontsize=16)
    plt.tight_layout()
...
def main():
    """Main function to compute interactions and generate plots."""
    ...
    # Compute interactions with error propagation
    print("\n=== Computing TF Interactions with Error Propagation ===")
    ...
    print("\nSaving results with effect sizes and FDR corrections...")
