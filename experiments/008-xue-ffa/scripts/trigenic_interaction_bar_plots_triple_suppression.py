...
def create_trigenic_bar_plots(
    ...
    for ffa_idx, ffa in enumerate(ffa_types):
        ...
        filename = f"multiplicative_trigenic_interaction_bar_plots_triple_suppression_{ffa.replace(':', '')}.png"
...
def create_recovery_pattern_plots(
    ...
    for ffa, patterns in ffa_groups.items():
        ...
        # Save figure with filename matching script name
        filename = f"multiplicative_trigenic_interaction_bar_plots_triple_suppression_recovery_{ffa.replace(':', '')}.png"
        ffa_dir = osp.join(ASSET_IMAGES_DIR, "008-xue-ffa")
        os.makedirs(ffa_dir, exist_ok=True)
        filepath = osp.join(ffa_dir, filename)
        fig.savefig(filepath, dpi=300, bbox_inches="tight")
    ...
    filename = "multiplicative_trigenic_interaction_bar_plots_triple_suppression_recovery_summary.png"
    ffa_dir = osp.join(ASSET_IMAGES_DIR, "008-xue-ffa")
    os.makedirs(ffa_dir, exist_ok=True)
    filepath = osp.join(ffa_dir, filename)
    fig.savefig(filepath, dpi=300, bbox_inches="tight")
...
def create_trigenic_summary_plot(trigenic_interactions, triple_mutants, columns,
    ...
    # Save figure with filename matching script name
    filename = "trigenic_interaction_bar_plots_triple_suppression_summary.png"
    ffa_dir = osp.join(ASSET_IMAGES_DIR, "008-xue-ffa")
    os.makedirs(ffa_dir, exist_ok=True)
    filepath = osp.join(ffa_dir, filename)
    fig.savefig(filepath, dpi=300, bbox_inches="tight")
