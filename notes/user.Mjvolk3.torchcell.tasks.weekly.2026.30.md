...
- [x] Added the Fig 7 classical-ML justification figure and finalized Supplementary Note 6 with species-aware DNA language model naming and the UCE and species-aware-LM citations [[paper.nature-biotech.si.traditional-ml-baselines]]
- [x] Revised the Fig 1 caption per author notes (reference-cell scope, bit-count symbol, panel-g speedup, panel-h soft constraint), verifying each change against its Supplementary Note [[paper.nature-biotech.fig1.block-diagrams]]
- [x] Set a no-em-dash house rule and swept the manuscript to en-dashes (fixing generated-table captions at their generators), consolidated with the palette and figure standards into a canonical style-guide note referenced from CLAUDE.md [[paper.nature-biotech.style-guide]]
