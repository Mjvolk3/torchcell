...
Eight of this class sit in wave 1, which is the largest change the banding makes.
Knockdown is the modality a pooled per-cell campaign can actually deliver: it needs no
strain construction per gene, it is inducible so the perturbation can be timed, and it
reaches the roughly one gene in five that a deletion collection cannot hold at all. That
last point is Mukherjee 2021's, which covers the essential genes under acetic acid by
CRISPRi. Two rows go further than one knockdown per cell: Jaffe 2019 measures an
interaction between two simultaneous knockdowns, and Crook 2016 grades the knockdown by
dose rather than applying it as a switch, which is the difference between a point and a
curve per gene. Hale 2024 supplies the third dimension by building 8,046 guides into 169
sequenced backgrounds.
...
All 42 items in the microbe-perturb-seq topic collection were read this pass, and the
earlier reading that they carry little data over large genotype numbers holds. Seven have a
real genotype axis in a single-cell host: Kemmeren 2014, Sameith 2015, Lian 2019 and
Nadal-Ribelles 2025 are built, Jackson 2020 and Boocock 2025 are rows 3 and 1, and the
seventh is a preprint with no deposited accession (Sec.~\ref{sec:quadrant}). The other 35
divide into platform papers, mammalian Perturb-seq, bacterial condition atlases and
single-cell statistics, all recorded in Table~\ref{tab:excluded}. Two small yeast platform
rows were added from the sweep, Jariani 2020 and Urbonaite 2021, for the per-cell variance
structure they measure rather than for a genotype axis they do not have.

Two rows already on the list moved a long way up it for the same reason. Hu 2007 profiles
269 transcription-factor deletions and Lenstra 2011 profiles chromatin regulators, so
between them and the built Kemmeren 2014 the bulk deletion-expression coverage of both
halves of transcriptional control is reachable; they were ranked 33 and 89 on scale alone
and are 7 and 18 once the quadrant they sit in is what orders the list. Airoldi 2016, new
from the Jackson 2020 citation list, supplies the wild-type nitrogen-limited baseline for
the media that atlas works in.

Two more deletion-expression rows were added after that. Hughes 2000 is the original
compendium, 300 mutations and chemical treatments on two-color arrays, and Sun 2013
decomposes the transcript level of 46 deletion strains into the synthesis and decay rates
that produce it. With Hu 2007 they make cross-laboratory replication of the built
Kemmeren 2014 testable in three independent directions, and Sun 2013 adds the rate layer
none of the others has.

\subsection{Transcript, translation, protein and turnover\secstatus{todo}}

Steady-state transcript and steady-state protein were both already well covered and the
list had no row for what sits between them, which is where a model asked to infer protein
from RNA loses its footing. Five rows close that.

Grossbach 2022 measures transcript, protein and phosphosite on one panel of 112 sequenced
strains, which is the only row where the RNA-to-protein residual is observable within a
genotype. Teyssonniere 2024 runs the same comparison at species scale, 942 isolate
proteomes against the built Caudal 2024 transcriptomes on the same panel, and it is the
published calibration for that pairing: weak correlation, and 3 percent overlap between the
variants associated with protein and with transcript. Foss 2007 is the same measurement on
the same cross as Grossbach fifteen years earlier and on a different platform, so it is the
replicate rather than the primary.

The other two are gene-level rather than strain-level. McManus 2014 supplies translation
efficiency against mRNA abundance for 5,474 genes, and Martin-Perez 2017 supplies turnover
for 3,160 proteins. Those two coefficients are the reason transcript and protein disagree
at all, and neither varies by strain, so they enter as a per-gene layer over every genotype
rather than as a genotype axis of their own.
...
None of these is a headline dataset and two are load-bearing.
Puddu 2019 is whole-genome sequence for every strain in the deletion collection.
Thirty-one rows inside the long-run cut declare an \term{S288C-KO} sequence basis, and
without Puddu that basis is an assumption that each strain differs from the reference at
exactly the intended locus. Puddu shows it frequently does not. That matters more for a
pooled campaign than for an arrayed screen, because a pooled assay reads a barcode and
infers a genotype, so it is row 4 rather than row 27.

Mulleder 2012 is the second. Jackson 2020 built its own prototrophic strains rather than
use the deletion collection, because auxotrophic markers rule out minimal and
nitrogen-limited media, and Mulleder 2012 is the prototrophic collection that removes that
constraint. It is also the strain resource behind the built Mulleder 2016 amino-acid
metabolome.
