...
The knockout collection is the bridge: Kemmeren 2014 (mRNA, 1,484 deletions), Messner
2023 (protein, 4,699 deletions) and Nadal-Ribelles 2025 (perturb-seq pseudobulk, 2,243
deletions with >= 50 cells) profile the same single-gene deletions that the metabolic
screens score, so a deletion's molecular profile can be set beside its metabolite
readout. Three reads per (molecular source x metabolic panel), all on the deletions the
two share:
...
SOURCES = {
    "kemmeren": "Kemmeren mRNA",
    "messner": "Messner protein",
    "nadal": "Nadal-Ribelles mRNA",
}
...
TABLES_DIR = osp.join(
    ...
    "..",
    "notes-tex",
    "028-knockout-expression-metabolic",
    "tables",
)
GENERATED = (
    "%% GENERATED by experiments/028-knockout-expression/scripts/"
    "expression_metabolic_yko_correlation.py\n"
    "%% from results/expression_metabolic_yko_correlation.json. Do not edit by hand.\n"
    "%% SOURCE: results/expression_metabolic_yko_correlation.json via "
    "expression_metabolic_yko_correlation.py\n"
)
SRC = "\\src{experiments/028-knockout-expression/scripts/expression_metabolic_yko_correlation.py}"
...
def _gene(orf: str, names: dict[str, str]) -> str:
    std = names.get(orf)
    return f"\\gene{{{std}}}" if std else f"\\file{{{orf}}}"


def _write_tables(out: dict[str, Any], names: dict[str, str], tables_dir: str) -> None:
    """Two tables for the notes-tex document: one row per (source, panel) read, and
    the 19 Mulleder amino acids by source.
    """
    os.makedirs(tables_dir, exist_ok=True)
    reads = out["reads"]
    rows = []
    for pk, spec in PANELS.items():
        for sk, sl in SOURCES.items():
            r = reads.get(f"{sk}__{pk}")
            if r is None:
                continue
            mets = r.get("metabolites", {})
            if not mets:
                rows.append(
                    f"    {_tex(spec['label'])} & {sl} & {r['n_shared_deletions']:,} & "
                    f"{r['n_genes']:,} & 0 & -- & -- & -- & -- \\\\"
                )
                continue
            rs = np.array([v["multivariate"]["oof_pearson"] for v in mets.values()])
            nl = np.array([v["multivariate"]["null_oof_p95"] for v in mets.values()])
            best = max(
                mets.items(), key=lambda kv: kv[1]["multivariate"]["oof_pearson"]
            )
            man = r.get("mantel")
            man_s = (
                f"${man['spearman']:+.3f}$ ({man['null_p95_abs']:.3f})" if man else "--"
            )
            rows.append(
                f"    {_tex(spec['label'])} & {sl} & {r['n_shared_deletions']:,} & "
                f"{r['n_genes']:,} & {len(mets)} & ${np.median(rs):.2f}$ & "
                f"{int((rs > nl).sum())} of {len(rs)} & "
                f"{_tex(_short(best[0]))} (${best[1]['multivariate']['oof_pearson']:.2f}$) & "
                f"{man_s} \\\\"
            )
    with open(osp.join(tables_dir, "reads_summary.tex"), "w") as f:
        f.write(GENERATED)
        f.write(
            "\\begin{table}[htbp]\n  \\centering\n  \\footnotesize\n"
            "  \\caption[Every read, by panel and source]{Every (metabolic panel, molecular "
            "source) read on the deletions the two share. Shared is the number of deletions "
            "profiled on both sides; genes is the number of features after the coverage "
            "filter; scored is the number of metabolites with at least 40 deletions. The "
            "ridge score is the out-of-fold Pearson of a ridge from the whole profile to the "
            "metabolite, its median over scored metabolites, and the count above the "
            "permutation 95th percentile of the same statistic. Mantel is the Spearman "
            "between strain-pair profile similarity and strain-pair metabolite distance, "
            "with the permutation 95th percentile of $|\\rho|$ in parentheses; it needs at "
            "least three metabolites and 40 complete deletions. " + SRC + "}\n"
            "  \\label{tab:reads-summary}\n"
            "  \\setlength{\\tabcolsep}{3pt}\n"
            "  \\begin{tabular}{@{}p{2.6cm}p{1.9cm}rrrrp{1.3cm}p{2.6cm}p{2.0cm}@{}}\n"
            "    \\toprule\n"
            "    panel & source & shared & genes & scored & median ridge $r$ & above null "
            "& best metabolite & Mantel $\\rho$ (null) \\\\\n    \\midrule\n"
        )
        f.write("\n".join(rows) + "\n")
        f.write("    \\bottomrule\n  \\end{tabular}\n\\end{table}\n")

    # amino acids by source
    mets = sorted(
        {
            m
            for sk in SOURCES
            for m in reads.get(f"{sk}__mulleder", {}).get("metabolites", {})
        },
        key=lambda m: (
            -reads["messner__mulleder"]["metabolites"][m]["multivariate"]["oof_pearson"]
            if m in reads.get("messner__mulleder", {}).get("metabolites", {})
            else 0.0
        ),
    )
    rows = []
    for m in mets:
        cells = []
        for sk in SOURCES:
            v = reads.get(f"{sk}__mulleder", {}).get("metabolites", {}).get(m)
            if v is None:
                cells += ["--", "--"]
                continue
            g, r = v["univariate"]["top_genes"][0]
            cells += [
                f"${v['multivariate']['oof_pearson']:.2f}$ ({v['multivariate']['null_oof_p95']:.2f})",
                f"{_gene(g, names)} ${r:+.2f}$",
            ]
        rows.append(f"    {_tex(m)} & " + " & ".join(cells) + " \\\\")
    head = " & ".join(f"{sl} ridge $r$ (null) & top gene" for sl in SOURCES.values())
    with open(osp.join(tables_dir, "amino_acids.tex"), "w") as f:
        f.write(GENERATED)
        f.write(
            "\\begin{table}[htbp]\n  \\centering\n  \\footnotesize\n"
            "  \\caption[The Mulleder amino acids by source]{The 19 Mulleder 2016 amino "
            "acids, ordered by the proteome's score. Ridge $r$ is the out-of-fold Pearson "
            "from the whole profile with the permutation 95th percentile in parentheses; "
            "the top gene is the single feature with the largest $|r|$ against the "
            "metabolite across the shared deletions, with its sign. " + SRC + "}\n"
            "  \\label{tab:amino-acids}\n"
            "  \\setlength{\\tabcolsep}{3pt}\n"
            "  \\begin{tabular}{@{}l" + "p{1.7cm}p{2.0cm}" * len(SOURCES) + "@{}}\n"
            "    \\toprule\n"
            f"    amino acid & {head} \\\\\n    \\midrule\n"
        )
        f.write("\n".join(rows) + "\n")
...
def main() -> None:
    ...
    nadal, cells, _ = _recomputed_profiles(
        data_root, "pseudobulk_log2fc", _resolver(genome), genotypes
    )
    sources = {
        "kemmeren": _matrix(kem, sorted(kem)),
        "messner": _messner(_load_lmdb(osp.join(data_root, LMDB_PROTEOME)))[0],
        # Nadal A: pseudobulk log2 fold change per deletion, deletions with >= 50 cells
        "nadal": _matrix(
            nadal, sorted(o for o in nadal if cells.get(o, 0) >= NADAL_MIN_CELLS)
        ),
    }
    panels = {
    ...
    if args.plot_only:
        with open(results_path) as f:
            out = json.load(f)
        for key in ("kemmeren__mulleder", "messner__mulleder", "nadal__mulleder"):
            sk, pk = key.split("__")
    ...
    attr = genome.gene_attribute_table
    names = {
        str(i): str(g)
        for i, g in zip(attr["ID"], attr["gene"])
        if isinstance(g, str) and g and g != "nan"
    }
    _write_tables(out, names, TABLES_DIR)
    ...
    # Yellow, red, blue: the same colors these three panels carry in the 028 figures.
    col = {
        "kemmeren": PLOT_PALETTE[3],
        "messner": PLOT_PALETTE[1],
        "nadal": PLOT_PALETTE[4],
    }
    ...
    if mk is not None:
        ...
        ax.set_xlabel(f"strain-pair profile similarity ({SOURCES[sk]})")
    ...
    # Timestamped copy for the note's iteration history, and a stable name that the
    # notes-tex `make plots` rule can target.
    for stem in (
        osp.join(images, f"expression_metabolic_yko_correlation_{timestamp()}"),
        osp.join(images, "expression_metabolic_yko_correlation"),
    ):
