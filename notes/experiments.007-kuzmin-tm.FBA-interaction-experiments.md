...
## 2026.09.05 - What the Fig. 2d "Yeast9 FBA" baseline actually was, from the frozen files

Written while drafting the Supplementary Note `note:fba` (`paper/nature-biotech/sections/si-note-fba.tex`). Everything measured here comes from [[experiments.007-kuzmin-tm.scripts.fba_baseline_si]], which reads the only committed record of the run, `results/cobra-fba-growth_backup_20250923_134447/` (run 2025-09-15 by `targeted_fba_growth_fast.py` + `match_fba_to_experiments.py`; the slurm driver moved it to the backup name on 2025-09-23 before a re-run whose outputs were never committed; the multi-media and glucose/oxygen sweeps described above also left no committed results).

- **Medium.** The run used `model.medium` of yeast-GEM 9.0.2 unchanged: 16 open exchanges, glucose 1 mmol/gDW/h, ammonium nitrogen, no amino acids or vitamins (`results/fba_baseline_si/yeast9_default_medium.csv`). The "Media Condition Development" section above describes work that came after this run; none of it is behind the Fig. 2d number. The section also states the screen was in YPD; the Kuzmin 2018 SI (local OCR `kuzmin2018_SI.mmd`, not yet checked against the mirror because the tc-lit key in `.env` returned 401 on this date) gives the final selection plates as SD/MSG synthetic medium (0.1% monosodium glutamate, 0.2% amino-acid supplement lacking His/Arg/Lys/Ura, 2% glucose) at 26 C. The KG query that built the dataset filters `m.name = 'YEPD' AND t.value = 30`, which matches neither; flagged in the note as a source annotation to correct.
- **Pearson.** The frozen matched file gives r = 0.0019 (n = 332,313) for tau and r = -0.0046 (n = 299,146) for fitness. The manuscript's 0.0006 does not reproduce from these files and its subset/run is not recorded; the note says so and the scripted value should replace it. (Observation only: the mean predicted tau in the file is 0.00067.)
- **Why near zero.** 97.6% of predicted tau within 1e-6 of zero, 99.93% within 1e-3; 231 nonzero, almost all +1/+2/-1. 93.1% of triple deletions grow at the wild-type rate, 4.9% at zero; 220,392 of 332,313 triples (66%) have no gene in the model, only 7,419 have all three.
- **Solver artifacts.** 253 of 632,616 checkable doubles and 52 of 285,606 checkable triples contradict their own single-deletion fitness (246 of the doubles contain YGR144W: single grows at WT, doubles return 0 with status `optimal`; a local re-solve of the YGR144W single gives WT growth); 173 triples grow faster than the least of their doubles. These produce the +-1/+-2 taus. Cause not determined.
- Figure: `notes/assets/drawio/FigS-yeast9-fba.drawio` from [[experiments.007-kuzmin-tm.scripts.fba_baseline_compose_figure]]; panel f is a placeholder for the rerun on the screen's medium.

## 2026.09.06 - The matched file is misaligned; the triples are Kuzmin 2018 + 2020

- **Bug in `match_fba_to_experiments.py`.** It pairs `dataset[i]`'s gene set with `dataset.label_df.iloc[i]`; the two orders differ, so `matched_fba_experimental_fixed.parquet`'s `experimental` column is the true label set in the wrong row order (sorted stored and raw tau coincide for 99.98% of triples; row by row 1.2% agree). The tau correlation is unaffected (0.0019 either way; the prediction is constant), but the fitness correlation was reported as -0.0046 and is 0.151 with aligned labels (0.254 within triples containing a model gene). Details and the raw-derived labels: [[experiments.007-kuzmin-tm.scripts.fba_baseline_si]]. The "Initial Formula Error" section above is unrelated; this is a row-order bug.
- The 332,313 triples are Kuzmin 2018 (91,050) plus Kuzmin 2020 Tables S1 and S3 (241,746; 483 in both); the overview above calls them Kuzmin 2018 only.
- The figure now has panels a-g (e: evaluable set; f: measured landscape by coverage; g: the medium-rerun placeholder with a red first line).
