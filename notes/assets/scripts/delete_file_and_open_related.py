...
def convert_to_dendron_path(file_path):
    ...
    relative_path = file_path.replace(git_root, "").lstrip("/").lstrip("\\")
...
def open_related_files(src_file_path):
    ...
    md_file_path = os.path.join(get_git_root(), "notes", dendron_path + ".md")
    print(md_file_path)
    ...
    if file_extension == ".py" and src_file_path.startswith(os.path.join(get_git_root(), PYTHON_PKG_REL_PATH)):
        # Construct the test file path
        source_relative_path = os.path.relpath(src_file_path, os.path.join(get_git_root(), PYTHON_PKG_REL_PATH))
        test_file_name = "test_" + os.path.basename(src_file_path)
        test_file_path = os.path.join(get_git_root(), PYTHON_PKG_TEST_REL_PATH, *source_relative_path.split("/")[:-1], test_file_name)
