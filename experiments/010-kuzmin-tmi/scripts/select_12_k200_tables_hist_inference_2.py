# experiments/010-kuzmin-tmi/scripts/select_12_k200_tables_hist_inference_2
# [[experiments.010-kuzmin-tmi.scripts.select_12_k200_tables_hist_inference_2]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/010-kuzmin-tmi/scripts/select_12_k200_tables_hist_inference_2
# Test file: experiments/010-kuzmin-tmi/scripts/test_select_12_k200_tables_hist_inference_2.py

"""
Generate tables and histogram for 12-gene panel (k=200) using inference_2 results.

This version uses inference_2 data which has stricter filtering:
- SMF from Costanzo2016 only (lowest noise σ=0.063)
- Thresholds: SMF > 1.10, DMF > max(SMF) + 0.03, all > 1.0 baseline
- 479,195 triples (vs 275M in inference_1)
...
def load_inference_data() -> pd.DataFrame:
    """Load the inference parquet file for the best model (Pearson=0.4619) from inference_2."""
...
def load_gene_selection_results() -> pd.DataFrame:
    """Load gene selection results from inference_2."""
    results_path = osp.join(
        EXPERIMENT_ROOT, "010-kuzmin-tmi/results/inference_2/gene_selection_results.csv"
    )
...
def load_top_k_constructible_triples(panel_size: int, k: int) -> set[frozenset]:
    ...
    csv_path = osp.join(
        EXPERIMENT_ROOT,
        f"010-kuzmin-tmi/results/inference_2/top_k_constructible_panel{panel_size}_k{k}.csv",
    )
...
def load_constructible_triples(panel_size: int, k: int) -> pd.DataFrame | None:
    ...
    path = osp.join(
        EXPERIMENT_ROOT,
        f"010-kuzmin-tmi/results/inference_2/constructible_triples_panel{panel_size}_k{k}.parquet",
    )
    if osp.exists(path):
...
def plot_overlay_histogram(
    ...
    ax.set_title(
        f"Gene Interaction Distribution (inference_2): {panel_size}-Gene Panel (k={k})\n"
    ...
    output_path = osp.join(
        output_dir, f"overlay_histogram_inference_2_panel{panel_size}_k{k}_{timestamp()}.png"
    )
...
def main():
    ...
    print("Using inference_2 results (479K triples, stricter filtering)")
    print("=" * 60)
    ...
    # Create output directory for inference_2 results
