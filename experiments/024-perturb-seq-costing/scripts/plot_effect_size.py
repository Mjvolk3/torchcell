...
def panel_c(ax) -> None:
    ...
    ax2.set_ylabel("Median fold change among responders", fontsize=5, color="#666666")
...
def panel_d(ax) -> None:
    ...
    ax.plot(
        ...
        markeredgewidth=0.4,
        ls="",
        label="Sameith doubles (observed)",
        zorder=6,
    )
...
def main() -> None:
    ...
    for key, lab, _ in SERIES:
        ...
        print(
            f"{lab:20s} 1.25x={d.loc[1.25]:6.0f}  2x={d.loc[2.0]:5.0f}  "
    ...
    for key, lab, _ in SERIES:
        ...
        print(
            f"  {lab:20s} n={len(v):5d}  range {v.min():.0f}-{v.max():.0f}  "
