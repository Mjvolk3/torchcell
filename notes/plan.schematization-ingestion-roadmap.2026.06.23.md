...
| WS15    | Environmental-perturbation schema + chemogenomic ingestion | ✅ | **8 env×geno→fitness datasets landed** on a new `EnvironmentResponsePhenotype` + `EnvironmentPerturbation` ontology (`SmallMoleculePerturbation` typed `Compound` + `EnvironmentPhysicalPerturbation`; `EngineeredCopyNumberPerturbation` + `ReferenceGenome.ploidy`; streaming sparse reference index). Datasets: Vanacloig 2022 (164k), Mota 2024 (1.3k), Wildenhain 2015 (429k), Auesukaree 2009 (525), Hoepfner 2014 (30M HIP/HOP), Costanzo 2021 condition-SGA (61k), Hillenmeyer 2008 FitDb het (2.9M) + hom (1.2M). L1 keys on the genotype+environment SIGNATURES (strain × condition). See [[environmental-chemogenomic-ingestion-plan]]. **Remaining:** Lee 2014 (awaiting author matrices), Nadal-Ribelles 2025 (scRNA, different modality) |
