# experiments/008-xue-ffa/scripts/create_ffa_species_specific_overlays.py
# [[experiments.008-xue-ffa.scripts.create_ffa_species_specific_overlays]]
# https://github.com/Mjvolk3/torchcell/tree/main/experiments/008-xue-ffa/scripts/create_ffa_species_specific_overlays
# V1: FFA-species-specific interaction overlays - Based on v19/v23 with following changes:
#  - Filter interactions by FFA species (C14:0, C16:0, C16:1, C18:0, C18:1, Total Titer)
#  - Only create 'both' (positive+negative) plots to reduce plot count
#  - Add FFA species to 5-line title format
#  - Process both multiplicative and additive models
...
# Map FFA species to common names for plot titles
FFA_NAMES = {
    "C14:0": "Myristate",
    "C16:0": "Palmitate",
    "C16:1": "Palmitoleate",
...
def improve_pathway_layout_with_interleaving(
    pos, G, vertical_spread_multiplier=4.0, met_x_shift=0.8, met_to_ffa=None
):
    """
    ...
    """
    print(
        f"  Improving pathway layout (v_spread={vertical_spread_multiplier}x, met_x_shift=+{met_x_shift})..."
    )
    ...
    if core_gene_nodes:
        ...
        if n_genes > 1:
            y_positions = np.linspace(
                -vertical_spread_multiplier, vertical_spread_multiplier, n_genes
            )
    ...
    if reaction_nodes:
        ...
        if n_rxns > 1:
            y_positions = np.linspace(
                -vertical_spread_multiplier, vertical_spread_multiplier, n_rxns
            )
    ...
    metabolite_nodes = [
        n for n in G.nodes() if G.nodes[n].get("node_type") == "metabolite"
    ]
    if metabolite_nodes and met_to_ffa:
        # Separate target FFAs and others
        target_ffas = [
            n for n in metabolite_nodes if is_true(G.nodes[n].get("is_target_ffa"))
        ]
        other_mets = [
            n for n in metabolite_nodes if not is_true(G.nodes[n].get("is_target_ffa"))
        ]
        ...
        other_positions = sorted(
            [(n, pos[n]) for n in other_mets if n in pos], key=lambda x: x[1][1]
        )
        ...
        if n_total > 1:
            ...
            all_y_positions = np.linspace(
                vertical_spread_multiplier, -vertical_spread_multiplier, n_total
            )  # REVERSED!
            ...
            print(
                f"    Interleaved {n_targets} target FFAs (C14:0 at TOP) with {n_others} other metabolites"
            )
        ...
        print(
            f"    Spread {n_total} metabolites vertically, shifted RIGHT by +{met_x_shift}"
        )
...
def compute_tf_circle_layout(tf_nodes, radius=1.0, center_x=-8.0):
    ...
    V35: Moved to center_x=-8.0 to move TF network further left.
...
def load_graph_enrichment_data(
    ...
    V1 (FFA-species): Added ffa_species parameter to filter interactions by FFA type.
    """
    print(f"\n  Loading {model} {interaction_type} {sign} {topology} interactions...")
    if ffa_species:
        print(f"    Filtering for FFA species: {ffa_species}")
    ...
    print(f"    Loaded {len(df)} total rows from {filename}")

    # V1 (FFA-species): Filter by FFA species if specified
    if ffa_species and "ffa_type" in df.columns:
        df = df[df["ffa_type"] == ffa_species].copy()
        print(f"    Filtered to {len(df)} rows for {ffa_species}")
    ...
    if "interaction_score" in df.columns and "significant_p05" in df.columns:
        ...
        print(
            f"    Significant interactions in filtered data: {pos_count} positive, {neg_count} negative"
        )

    return df


def load_enrichment_significance(
    model="multiplicative", interaction_type="digenic", sign="positive", topology="edge"
):
    ...
    # Create dict of graph_type -> is_significant
    enrichment_sig = {}
    for _, row in df.iterrows():
        graph_type = row["graph_type"]
        p_value = row["p_value"]
        enrichment_sig[graph_type] = p_value < 0.05
        print(f"      {graph_type}: p={p_value:.4f}, sig={enrichment_sig[graph_type]}")
...
def extract_significant_interactions(
    overlap_df,
    graph_type,
    interaction_type="digenic",
    topology="edge",
    ...
    print(
        f"    Before filtering - Positive: {len(positive_interactions)}, Negative: {len(negative_interactions)}"
    )
    ...
    print(
        f"    After filtering ({sign_filter}) - Positive: {len(positive_interactions)}, Negative: {len(negative_interactions)}"
    )

    return positive_interactions, negative_interactions


def add_tf_to_gene_connections(
    G, tf_nodes, pathway_gene_nodes, regulatory_graph, tflink_graph, genome
):
    """
    ...
    core_pathway_genes = [
        g for g in pathway_gene_nodes if is_true(G.nodes[g].get("is_core"))
    ]
    print(
        f"      Filtering to {len(core_pathway_genes)} core genes (out of {len(pathway_gene_nodes)} total)"
    )

    tf_gene_edges = []

    for graph_name, gene_graph in [
        ...
        for tf in tf_nodes:
            for (
                ...
                core_pathway_genes
            ):  # V18: Use core_pathway_genes instead of pathway_gene_nodes
...
def create_interaction_edges_with_deduplication(
    ...
    print(
        f"    Creating edges from {n_positive_interactions} positive and {n_negative_interactions} negative interactions"
    )
        ...
        if len(genes) >= 2:
            if len(genes) == 2:
                ...
                if len(edges_from_this) < 3:
                    print(
                        f"      Trigenic interaction {i+1} ({genes}) created only {len(edges_from_this)} edges: {edges_from_this}"
                    )
    ...
    print(
        f"    Edge counts after deduplication: {len(positive_edges)} positive, {len(negative_edges)} negative"
    )
    ...
    if n_positive_interactions > len(positive_edges) and n_positive_interactions > 0:
        print(
            f"    NOTE: {n_positive_interactions} positive interactions → {len(positive_edges)} unique edges (trigenic overlap)"
        )
    if n_negative_interactions > len(negative_edges) and n_negative_interactions > 0:
        print(
            f"    NOTE: {n_negative_interactions} negative interactions → {len(negative_edges)} unique edges (trigenic overlap)"
        )
    ...
    return (
        ...
        n_negative_interactions,
    )


def create_multigraph_overlay(
    G_base,
    pos_base,
    tf_nodes,
    positive_interactions,
    ...
    V1 (FFA-species): Added ffa_species parameter for 5-line title format.
    ...
    pos = improve_pathway_layout_with_interleaving(
        pos_base,
        G,
        vertical_spread_multiplier=7.0,
        met_x_shift=0.8,
        met_to_ffa=met_to_ffa,
    )
    ...
    gene_nodes = [
        n
        for n in G.nodes()
        if G.nodes[n].get("node_type") == "gene" and n not in tf_nodes
    ]
    reaction_nodes = [n for n in G.nodes() if G.nodes[n].get("node_type") == "reaction"]
    metabolite_nodes = [
        n for n in G.nodes() if G.nodes[n].get("node_type") == "metabolite"
    ...
    visible_nodes = set(
        core_genes + reaction_nodes + metabolite_nodes + tf_nodes_in_graph
    )
    ...
    # Separate edge types
    gene_rxn_edges = [
        (u, v) for u, v, d in G.edges(data=True) if d.get("edge_type") == "catalyzes"
    ]
    met_rxn_edges = [
        (u, v) for u, v, d in G.edges(data=True) if d.get("edge_type") == "consumed_by"
    ]
    rxn_met_edges = [
        (u, v) for u, v, d in G.edges(data=True) if d.get("edge_type") == "produces"
    ]

    # V18: Filter metabolic edges to only include edges between visible nodes
    gene_rxn_edges = [
        (u, v) for u, v in gene_rxn_edges if u in visible_nodes and v in visible_nodes
    ]
    met_rxn_edges = [
        (u, v) for u, v in met_rxn_edges if u in visible_nodes and v in visible_nodes
    ]
    rxn_met_edges = [
        (u, v) for u, v in rxn_met_edges if u in visible_nodes and v in visible_nodes
    ]

    print(
        f"    Filtered edges: {len(gene_rxn_edges)} gene-rxn, {len(met_rxn_edges)} met-rxn, {len(rxn_met_edges)} rxn-met"
    )
    ...
    tf_gene_edges_filtered = [
        (u, v) for u, v in tf_gene_edges if u in visible_nodes and v in visible_nodes
    ]
    print(
        f"    Filtered TF→gene edges: {len(tf_gene_edges_filtered)} (was {len(tf_gene_edges)})"
    )
    ...
    print(
        f"    Graph type '{graph_type}': arrows={'enabled' if show_arrows else 'disabled'}"
    )
    ...
    # V17: Create interaction edges and get both edge counts and interaction counts
    positive_edges, negative_edges, n_pos_interactions, n_neg_interactions = (
        create_interaction_edges_with_deduplication(
            positive_interactions, negative_interactions, G
        )
    )
    ...
    if negative_edges:
        nx.draw_networkx_edges(
            ...
            edge_color=COLORS["negative_interaction"],
            width=6.0,
            alpha=0.95,
            arrows=False,
            ax=ax,
    ...
    if positive_edges:
        nx.draw_networkx_edges(
            ...
            edge_color=COLORS["positive_interaction"],
            width=2.5,
            alpha=0.95,
            arrows=False,
            ax=ax,
    ...
    if tf_nodes_in_graph:
        nx.draw_networkx_nodes(
            G,
            pos,
            nodelist=tf_nodes_in_graph,
            node_color=COLORS["tf_gene"],
            node_size=400,
            ax=ax,
            alpha=0.95,
        )
    ...
    if tf_labels:
        nx.draw_networkx_labels(
            G, pos, labels=tf_labels, font_size=12, font_weight="bold", ax=ax
        )  # V16: 12 instead of 10
    ...
    if gene_labels:
        nx.draw_networkx_labels(
            G, pos, labels=gene_labels, font_size=12, font_weight="bold", ax=ax
        )  # V16: 12 instead of 10

    # V29: FIXED metabolite labels - FFA type first, then metabolite name, then compartment
    # Format: "(C14:0) myristate [cytoplasm]"
    met_labels = {}
    met_label_pos = {}
    ...
    for node in target_ffas:
        ...
        # Extract compartment from node ID (e.g., "palmitate_c" -> "c")
        # Node IDs typically end with underscore + compartment code
        compartment = ""
        if "_" in node:
            # Get the last part after underscore
            parts = node.split("_")
            potential_comp = parts[-1]
            if potential_comp in comp_map:
                compartment = potential_comp
    ...
    # V20: Updated title - improved formatting with em-dashes and no nested parentheses
    ...
    # V25: Build counts string (removed sign since always showing both)
    counts_parts = []
    ...
    # V25: Build title (removed sign since we always show both positive + negative)
    # 5-line format (FFA-species-specific):
    # Line 1: FFA Metabolic Network + TF Epistatic Interactions (Model Type)
    # Line 2: FFA Species Specific Interactions
    # Line 3: Filtering mode (Significant vs Significant and Enriched)
    # Line 4: Graph Name — Interaction Type (no sign)
    # Line 5: Edge/interaction counts
    model_str = model.replace("_", " ").title()
    title_line1 = (
        f"FFA Metabolic Network + TF Epistatic Interactions ({model_str} Model)"
    )

    if ffa_species:
        # 5-line format with FFA species
        title_line2 = f"{ffa_species} Specific Interactions"
        title_line3 = (
            "Significant and Enriched Interactions"
            if filter_enrichment
            else "Significant Interactions"
        )
        title_line4 = f"{graph_type_name} — {interaction_type_str}{topology_str}"
        title_line5 = counts_str
        ax.set_title(
            f"{title_line1}\n{title_line2}\n{title_line3}\n{title_line4}\n{title_line5}",
            fontsize=14,
            pad=15,
        ...
        # 4-line format (original - should not be used in this script)
        title_line2 = (
            "Significant and Enriched Interactions"
            if filter_enrichment
            else "Significant Interactions"
    ...
    # V14: MUCH LARGER LEGEND (fontsize 14 instead of 12)
    # Always show both positive and negative for combined plots
    ...
    legend_elements = [
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=COLORS["tf_gene"],
            markersize=12,
        ...
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=COLORS["core_gene"],
            markersize=10,
            label="Core FFA Pathway Genes",
        ),
        Line2D(
            [0],
            [0],
            marker="s",
            color="w",
            markerfacecolor=COLORS["reaction"],
            markersize=8,
            label="Reactions",
        ),
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=COLORS["target_ffa"],
            markersize=10,
            label="Target FFAs",
        ),
    ]
    ...
    legend_elements.append(
        Line2D(
            [0],
            [0],
            color=COLORS["positive_interaction"],
            linewidth=2.5,
    ...
    legend_elements.append(
        Line2D(
            [0],
            [0],
            color=COLORS["negative_interaction"],
            linewidth=6,
    ...
    # V26: Create nested directory and add FFA species to filename
    if ffa_species:
        safe_ffa = ffa_species.replace(":", "").replace(
            " ", "_"
        )  # C14:0 -> C140, Total Titer -> Total_Titer
        model_dir = osp.join(
            ASSET_IMAGES_DIR, "008-xue-ffa", "ffa_multigraph_overlays", model, safe_ffa
        )
    else:
    ...
    # V26: Prepend model and add FFA species to filename (along with batch suffix)
...
def create_ffa_multigraph_overlays_comprehensive(
    model="multiplicative", filter_enrichment=False, ffa_species=None
):
    """
    Create comprehensive FFA bipartite network visualizations with multigraph overlays.

    V1 (FFA-species): Added ffa_species parameter to filter interactions by FFA type.
                      Only creates 'both' (positive+negative) plots to reduce plot count.
    ...
    Args:
        ...
        ffa_species: FFA species to filter for (e.g., 'C14:0', 'C16:0', 'Total Titer'). If None, no filtering.
    """
    print("=" * 80)
    print(f"FFA MULTIGRAPH OVERLAYS COMPREHENSIVE (FFA-SPECIES V1): {model.upper()}")
    if ffa_species:
        print(f"FFA SPECIES: {ffa_species} ({FFA_NAMES.get(ffa_species, ffa_species)})")
    if filter_enrichment:
    ...
    pathway_gene_nodes = [
        ...
        if G_base.nodes[n].get("node_type") == "gene" and n not in tf_systematic
    ]
    print(f"  Found {len(pathway_gene_nodes)} pathway genes")

    # Compute COMPRESSED TF circle layout (same for all visualizations)
    # V35: Moved further left to -8.0 to provide more separation from the main network
    ...
    tf_pos = compute_tf_circle_layout(tf_systematic, radius=2.5, center_x=-8.0)
    ...
    graph_configs = [
        ...
        (
            ...
            graph.G_string12_0_coexpression,
            "STRING 12.0 Coexpression",
        ),
        (
            "string12_0_experimental",
            graph.G_string12_0_experimental,
            "STRING 12.0 Experimental",
        ),
        ("string12_0_database", graph.G_string12_0_database, "STRING 12.0 Database"),
    ]

    # V1 (FFA-species): Only create 'both' plots to reduce plot count by 3x
    # Original had 9 configs (3 types × 3 signs), now only 3 configs (3 types × 1 sign)
    interaction_configs = [
        ("digenic", "both", "edge"),
    ...
    for interaction_type, sign, topology in interaction_configs:
        print(f"\n{'='*80}")
        print(
            f"Processing {interaction_type.upper()} {sign.upper()} {topology.upper()}"
        )
        print(f"{'='*80}")

        # V1 (FFA-species): Pass ffa_species parameter to filter data
        overlap_df = load_graph_enrichment_data(
            model, interaction_type, sign, topology, ffa_species=ffa_species
        )

        if overlap_df is None:
            print(
                f"  No enrichment data found for {model} {interaction_type} {sign} {topology}"
            )
            continue
        ...
        if filter_enrichment:
            if sign == "both":
                ...
                enrichment_sig = load_enrichment_significance(
                    model, interaction_type, "positive", topology
                )
            else:
                enrichment_sig = load_enrichment_significance(
                    model, interaction_type, sign, topology
                )
        ...
        for graph_type, gene_graph, display_name in graph_configs:
            ...
            positive_interactions, negative_interactions = (
                extract_significant_interactions(
                    overlap_df,
                    graph_type,
                    interaction_type,
                    topology,
            ...
            if len(positive_interactions) == 0 and len(negative_interactions) == 0:
                print(
                    f"    No significant interactions found - will show base network only"
                )
            ...
            # DEBUG: Check if all interaction genes are in the network
            print("\n  Checking if all interaction genes are in network...")
            all_interaction_genes = set()
            for genes in positive_interactions_sys + negative_interactions_sys:
                all_interaction_genes.update(genes)

            print(
                f"    Total unique genes in interactions: {len(all_interaction_genes)}"
            )
            print(f"    TFs in network: {len(tf_systematic)}")
            print(f"    TF systematic IDs: {sorted(tf_systematic)}")

            missing_genes = [g for g in all_interaction_genes if g not in G.nodes()]
            if missing_genes:
                print(
                    f"    ERROR: {len(missing_genes)} genes from interactions NOT in network:"
                )
                for gene in missing_genes:
                    print(f"      - {gene}")
                print(f"    All interaction genes: {sorted(all_interaction_genes)}")
            else:
                print(
                    f"    ✓ All {len(all_interaction_genes)} interaction genes are in network"
                )

            # Add TF→gene regulatory connections (V18: now filters to core genes only)
            tf_gene_edges = add_tf_to_gene_connections(
                G,
                tf_systematic,
                pathway_gene_nodes,
                graph.G_regulatory,
            ...
            induced_edges = add_induced_subgraph_edges(
                G, tf_systematic, gene_graph, genome
            )

            # V26: Pass graph_type and create visualization with conditional arrow handling
    ...
    print("\n" + "=" * 80)
    print("COMPREHENSIVE MULTIGRAPH OVERLAYS COMPLETE (FFA-SPECIES V1)!")
    print("=" * 80)
    print(f"\nCreated {len(output_paths)} visualizations")
    print("\nKey improvements in V1 (FFA-species):")
    if ffa_species:
        print(
            f"  - FILTERED for FFA species: {ffa_species} ({FFA_NAMES.get(ffa_species, ffa_species)})"
        )
    print(f"  - ADDED model type to title: {model.replace('_', ' ').title()} Model")
    if ffa_species:
        print("  - 5-line title format with FFA species information")
    print(
        "  - ONLY created 'both' (positive+negative) plots (3x reduction in plot count)"
    )
    print("\nV19-V23 improvements retained:")
    print(
        "  - REMOVED zero-interaction skip: Always creates visualization (shows base network)"
    )
    print(
        "  - ADDED batch suffix to filenames: _all vs _enriched (eliminates duplicate confusion)"
    )
    print("  - Fixed phantom edges to hidden non-core genes")
    print("  - Accurate edge count reporting")
    print("  - Thicker negative lines (6.0 width in combined, 4.0 in single)")
    print("  - Bigger text (TF: 12pt, genes: 12pt, metabolites: 13pt)")
    if filter_enrichment:
    ...
    print(
        "      Each plot shows TF relationships in that specific network (STRING Exp vs Coexp, etc.)"
    )
...
if __name__ == "__main__":
    # Create FFA-species-specific visualizations for all FFA species
    # Currently only multiplicative model (for debugging)
    # This creates 216 plots total:
    # - 1 model (multiplicative only - additive, log_ols, glm_log_link commented out for now)
    # - 6 FFA species (C14:0, C16:0, C16:1, C18:0, C18:1, Total Titer)
    # - 2 filtering modes (_all, _enriched)
    # - 6 graph types
    # - 3 interaction configurations (digenic, trigenic triangle, trigenic connected) - only 'both'
    # = 1 × 6 × 2 × 6 × 3 = 216 plots

    print("\n" + "=" * 80)
    print("FFA-SPECIES-SPECIFIC MULTIGRAPH OVERLAYS")
    print("Creating visualizations for multiplicative model only")
    print("=" * 80)
    ...
    for model in ["multiplicative", "additive", "log_ols", "glm_log_link"]:
        ...
        # Loop through all FFA species
        for ffa_species in FFA_SPECIES:
            print("\n" + "-" * 80)
            print(
                f"FFA SPECIES: {ffa_species} ({FFA_NAMES.get(ffa_species, ffa_species)})"
            )
            print("-" * 80)

            # Create visualizations without enrichment filtering
            print("\n  Without enrichment filtering (_all)...")
            output_paths_all = create_ffa_multigraph_overlays_comprehensive(
                model=model, filter_enrichment=False, ffa_species=ffa_species
            )
            all_output_paths.extend(output_paths_all)

            # Create visualizations with enrichment filtering
            print("\n  With enrichment filtering (_enriched)...")
            output_paths_enriched = create_ffa_multigraph_overlays_comprehensive(
                model=model, filter_enrichment=True, ffa_species=ffa_species
            )
            all_output_paths.extend(output_paths_enriched)

    print("\n" + "=" * 80)
    print("ALL FFA-SPECIES-SPECIFIC VISUALIZATIONS COMPLETE!")
    print("=" * 80)
    print(f"Created {len(all_output_paths)} total visualizations")
    print("\nBreakdown:")
    print("  - 1 model (multiplicative only)")
    print("  - 6 FFA species (C14:0, C16:0, C16:1, C18:0, C18:1, Total Titer)")
    print("  - 2 filtering modes (_all, _enriched)")
    print("  - 6 graph types per configuration")
    print(
        "  - 3 interaction configurations (digenic, trigenic triangle, trigenic connected)"
    )
    print("  - Only 'both' (positive+negative) plots")
    print(f"\nExpected: 216 plots | Actual: {len(all_output_paths)} plots")
