# torchcell/models/hetero_cell_bipartite_dango
# [[torchcell.models.hetero_cell_bipartite_dango]]
# https://github.com/Mjvolk3/torchcell/tree/main/torchcell/models/hetero_cell_bipartite_dango
...
class GeneInteractionAttention(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads=8, dropout=0.1):
        ...
        # ReZero parameter (initialized to 0)
...
class GeneInteractionPredictor(nn.Module):
    ...
    def forward(self, gene_embeddings, batch=None):
        ...
        # Get dynamic embeddings through self-attention
...
class HeteroCellBipartite(nn.Module):
    ...
    def forward(
        ...
        # For fitness prediction, use the global embedding difference approach
        batch_size = z_i_global.size(0)
        z_w_exp = z_w_global.expand(batch_size, -1)
        z_p_global = z_w_exp - z_i_global

        # Fitness prediction using first output of prediction head
        fitness = self.prediction_head(z_p_global)[:, 0:1]

        # For gene interaction prediction:
        # 1. Get embeddings of perturbed genes from wildtype
        ...
        # 3. Get interaction component from DANGO predictor
        interaction_component = self.gene_interaction_predictor(
    ...
    #     # 3. Use gene interaction predictor
    #     gene_interactions = self.gene_interaction_predictor(
    #         pert_gene_embs, batch_assign
    #     )

    #     # Combine predictions
    #     predictions = torch.cat([fitness, gene_interactions], dim=1)
    ...
    #         "z_p": z_p_global,
    #         "fitness": fitness,
    #         "gene_interaction": gene_interactions,
    #     }
