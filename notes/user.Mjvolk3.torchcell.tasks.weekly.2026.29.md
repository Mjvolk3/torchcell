...
- [x] Wrote the perturb-seq method review as a six-section typeset document: Perturb-seq primer, full Brettner SPLiT-seq and Jariani droplet explainers, theoretical limits, UIUC costing, bottlenecks [[microbe-perturb-seq.method-review-and-costing]]
- [x] Folded in Yao 2024 compressed Perturb-seq; its 400-cells-per-pair constant validates our multiplex arithmetic (reproduces their worked cases to 39.2 vs ~39 and 374.7 vs 374) and yields the key result that a 200-gene metabolic sub-library at 8-plex covers all 19,900 pairs in ~284k cells [[experiments.019-perturb-seq-costing.scripts.cost_model]]
- [x] Made second-hand provenance a first-class field after catching myself citing Boocock 2023 from a review's summary table rather than the primary paper, which we do not hold; the flag now renders as a dagger in both tables and figures so a screenshot cannot lose it [[experiments.019-perturb-seq-costing.scripts.method_data]]
- [ ] **On gilahyper:** run the knockdown effect-size analysis against the built Kemmeren/Sameith compendia, then feed the measured response distribution into `cost_model.umis_needed_for_fold_change()` to replace the nominal 2x threshold, regenerate the tables and rebuild the document. This is the empirical core of the theoretical-limits section and the last thing resting on an assumed effect size #high [[experiments.019-perturb-seq-costing.scripts.effect_size_analysis]]
- [ ] Add Lian MAGIC, Kemmeren 2014 and Sameith 2015 to the Zotero `microbe-perturb-seq` collection so the effect-size and library-construction sections can cite them properly instead of carrying `\external{}` flags #medium [[microbe-perturb-seq.method-review-and-costing]]
- [ ] Decide how to handle figures reproduced from Brettner/Jariani/Yao in the method explainers - fine for an internal planning doc and a thesis with attribution, not for a published paper without permission #medium [[microbe-perturb-seq.method-review-and-costing]]
