...
Whether the transformer's $+0.04$ margin survives a disjoint split is still not
measured on test, in either direction. Job 1640 trained the replication
configuration on the 025 query-pair-disjoint split and gives a validation
trajectory (Section~\ref{sec:disjoint025}), but the 12 hour wall clock ended it
before any test evaluation, and its best checkpoint has not been scored since.
So the disjoint comparison rests on validation, where the margin appears at
epoch 7 and is gone by epoch 17. What closes the gap is one scoring pass over
the arm Q test split, against the baselines already in
Table~\ref{tab:disjoint025}.
