...
%% Re-run those three and the panels and the numbers move together. Nothing here is
...
45 double (fill) and 120 triple (hatched) deletions. The median moves from $+0.40$ at two
deletions to $-0.72$ at three. \textbf{b}, Trigenic interaction score $\tau$ against
...
\end{equation}
%
This is the trigenic score of the yeast genetic interaction maps, applied to titer in place
of fitness. Standard errors propagate through Eq.~\ref{eq:tau} by the delta method, and each
$t$ statistic is referred to the Welch-Satterthwaite effective degrees of freedom of that
linear combination, which has a median of 4.28 across the 714 testable trigenic scores.
...
choice that removes the positive signal.
...
\begin{figure*}[p]
\tcfig{figures/ffa_network_overlay.pdf}{\textbf{Trigenic interactions on total fatty acid
titer, drawn on the pathway they act through.} The ten deleted transcription factors are
arranged in a circle at left; edges within the circle are the 44 negative and 3 positive
trigenic interactions significant at a 5\% false discovery rate within the total-titer
readout under the multiplicative model, restricted to triples whose three genes are
connected in the yeast genetic interaction graph. A trigenic interaction is drawn as the
three edges of its triangle, so an edge can carry more than one interaction. Dotted edges
are transcription-factor-to-gene regulatory links; the remaining columns are the core fatty
acid pathway genes, the reactions they catalyze, and the metabolites those reactions
produce, with the five measured species and their compartments labeled.}{fig:network}
\end{figure*}
