...
def flag_runs(hip: pd.DataFrame, enr_flag: float) -> pd.DataFrame:
    ...
    h["roll_enr"] = h.groupby("chrom_idx")["enr"].transform(
        lambda s: s.rolling(ROLL_WINDOW, center=True, min_periods=4).median()
    )
...
def plot_score_distributions(samples, base_hip, base_hop):
    ...
    ax.hist(
        ...
        color=BLUEGRAY,
        alpha=0.65,
        label=f"HIP (heterozygous)  n={samples['HIP'].size:,} cells",
        log=True,
    )
    ax.hist(
        samples["HOP"],
        ...
        color=GREEN,
        alpha=0.55,
        label=f"HOP (homozygous)  n={samples['HOP'].size:,} cells",
        log=True,
    )
    ax.axvline(
        PRIMARY_THRESH,
...
def plot_promiscuity_ecdf(hip, hop):
    ...
    ax.axvline(
        ENR_FLAG, color=RED, ls="--", lw=1.5, label=f"flag threshold ({ENR_FLAG:g}x)"
    )
    ax.set_xscale("symlog", linthresh=1)
    ax.set_xlabel(
        "per-strain promiscuity enrichment  (frac hypersensitive / assay baseline)"
    )
...
def plot_genome_map(summary, h, hop):
    ...
    for ci, sub in unflagged.groupby("chrom_idx"):
        ax.scatter(
            ...
            sub["enr"],
            s=9,
            color=BLUE if int(ci) % 2 else BLUEGRAY,
            alpha=0.55,
            rasterized=True,
        )
    ax.scatter(
    ...
    ax.set_xlabel(
        "chromosomal position of the deleted ORF (chromosome = construction-lab region)"
    )
    ax.set_ylabel("HIP promiscuity enrichment (x baseline)")
    ax.set_title(
        "HIP background-mutation map: RUNS of adjacent hypersensitive strains = "
...
def plot_whi2(hip, hop):
    ...
    ax.set_title(
        f"WHI2 co-behaviour: {n_c3} strains share the whi2 background signature"
    )
    ...
    ax.set_title(
        "whi2-linked strains cluster on adjacent chromosomal batches (XII, IX)"
    )
...
def plot_hip_vs_hop_specificity(hip, hop, h):
    ...
    ax.scatter(
        ...
        color=BLUEGRAY,
        alpha=0.4,
        label="HIP strain (in both collections)",
        rasterized=True,
    )
    ax.scatter(
        fl["enr_hop"],
        ...
        edgecolor=BLACK,
        linewidth=0.2,
        label=f"flagged (n={len(fl)})",
    )
    lim = 60
    ax.plot(
        [0, lim], [0, lim], color=BLACK, ls=":", lw=1, label="HIP = HOP (gene biology)"
    )
    ax.set_xscale("symlog", linthresh=1)
    ax.set_yscale("symlog", linthresh=1)
    ...
    ax.set_title(
        "Flagged strains are HETEROZYGOUS-collection specific\n"
...
def plot_risk_tiers(tiers, summary):
    ...
    ax.set_title(
        "Records at risk of an unrepresented HIP background mutation, by definition"
    )
...
def main():
    ...
    tiers = [
        dict(
            short="whi2 cluster\n(mechanism)",
            n_strains=int(len(c3)),
            n_records=c3_records,
            pct_all_records=c3_records / total * 100,
        ),
        dict(
            short=f"empirical runs\n(>={ENR_FLAG:g}x)",
            n_strains=int(len(flagged)),
            n_records=flagged_records,
            pct_all_records=flagged_records / total * 100,
        ),
        dict(
            short="paper stated\n(157 HIP)",
            n_strains=157,
    ...
    risk = dict(
        denominators=dict(
            ...
            hop_records=hop_r,
            hip_strains=hip_s,
            hop_strains=summary["HOP"]["n_strains"],
            hip_pct_records=hip_r / total * 100,
            hop_pct_records=hop_r / total * 100,
        ),
        ...
        tiers=dict(
            ...
            empirical_runs_sweep={
                f"enr_{e:g}x": dict(
                    **v,
                    pct_all=v["n_records"] / total * 100,
                    pct_hip=v["n_records"] / hip_r * 100,
                )
                for e, v in sweep.items()
            },
            paper_characterized_157=dict(
                n_strains=157,
                n_records_estimate=int(157 * mean_hip_cov),
            ...
            conservative_hip_envelope=dict(
                n_strains=hip_s, n_records=hip_r, pct_all=hip_r / total * 100
            ),
    ...
    print(
        f"HIP share of records: {hip_r / total * 100:.1f}%  |  HOP: {hop_r / total * 100:.1f}%"
    )
    print(
        f"flagged HIP strains (>= {ENR_FLAG:g}x runs): {len(flagged)} "
        f"-> {flagged_records:,} records ({flagged_records / total * 100:.2f}% of all)"
    )
    print(
        f"whi2 cluster: {len(c3)} strains -> {c3_records:,} records "
        f"({c3_records / total * 100:.2f}% of all)"
    )
    print(f"suspect runs (>=3 adjacent): {len(runs)}")
    for r in runs[:8]:
        print(
            f"  chr {r['chrom']:<4} {r['pos_start']:>9}-{r['pos_end']:<9} "
