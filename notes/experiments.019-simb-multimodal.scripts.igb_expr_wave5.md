...
## 2026.09.13 - `split` stage: the v13 split round, explicit run list, staggered launch

- **`split` writes its own 24-run list** instead of an arm x seed product (`JOBS_EXPLICIT=1`), because the arm carries the split (`V_ref_s<k>`, `V_concat_s<k>`, `V_*_s0_90`) and each card holds one split's two arms for two seeds; the design is in [[experiments.019-simb-multimodal.conf.cgt_expr_v13_split]]. Six tasks on the `gpu` A40s, four runs per card, `--cpus-per-task=12` (two workers per run), `--mem=120g`, ten-day wall, 6,000 epochs.
- **Co-resident launches are now staggered 30 s apart.** The first submission (2397304) lost 3 of 24 runs at startup: four processes constructing the dataset in the same second raced on the `gene_set.json` write, whose shared `.tmp` name was renamed away by the first process before the second's `replace`. The writer now uses per-process temp names (`torchcell/utils/file_lock.py`), and the stagger keeps the LMDB open, the split-cache read and the W&B init from piling up. Resubmitted as 2397311.
