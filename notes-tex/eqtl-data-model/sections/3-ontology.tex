...
\begin{itemize}[leftmargin=1.2em,itemsep=2pt,topsep=2pt]
  ...
  \item \file{PseudobulkExpressionPhenotype} is the single-cell readout, built for
    Nadal-Ribelles 2025. It carries \file{dispersion} and \file{n\_cells} per genotype,
  ...
  \item The off-graph pointer pattern on that class, \file{sequence\_source},
    \file{sequence\_uri} and \file{sequence\_sha256}, means variant sequences are never
    inlined.
\end{itemize}
...
\file{systematic\_gene\_name} and validates it against

\begin{quote}\small
\file{^(Y[A-P][LR]\\d{3}[WC](-[A-Z])?|Q\\d{4}|YNC[A-Q]\\d{4}[WC])\$}
\end{quote}
...
regulatory sequence exclusively.
...
\notesec{Reference asymmetry} A segregant's natural reference is one of its two parents,
but the convention expresses everything off S288C. BY is near-isogenic to S288C, so
BY-derived blocks are nearly empty perturbation sets while RM-derived blocks carry the full
variant load. Keeping S288C is still right, since it is what makes these records comparable
to everything else, but the lopsidedness reflects the reference choice rather than the
biology and should be documented rather than discovered.

\notesec{Linkage is destroyed by a set} Variants arrive in blocks and co-segregate.
Representing a genotype as an unordered set discards exactly the structure that makes
mapping possible and that limits attribution.
...
rather than on a gene, carrying a probability. That is the same non-gene-keyed requirement
the promoter-variant gap already imposes, so the two should be resolved together.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt,topsep=2pt]
  ...
  \item \textbf{Decide the intergenic question deliberately}, because it gates both this
    class and the regulatory-DNA class. Either a non-gene-keyed perturbation leaf on a
    genomic interval, or gene-keying with the attribution recorded as a
    \file{ProvenanceGap}, or restriction to coding variants. The third is the honest
